...
function set_drop_target(element, drop_handler, drag_in_handler) {
	...
	add_listener( {
		...
		//	在 target 上拖完放开滑鼠。
		drop : function(event) {
			//	处理 types: 将所有设定的 type 一一列出。IE9 没有 dataTransfer.types。
			var i = 0, list = event.dataTransfer.types || ['text', 'url'], l = list.length, data, type;
			for (; i < l; i++) {
				try {
					//	IE10 中拖拉外部档案时，dataTransfer.getData(list[i]) 可能会 throw "Invalid argument."
					...
					if (list[i] in set_drop_target.handler) {
						...
						for ( var h = 0, _t = this,
								...
								function() {
									library_namespace.debug('准备执行。 (' + data + ')', 2, 'set_drop_target.ondrop');
									//library_namespace.debug('' + type[h]);
									type[this].call(_t, data);
									library_namespace.debug('执行完毕。', 2, 'set_drop_target.ondrop');
								}; h < type.length; h++) {
							try {
								if(timeout_binder) {
			...
			//	处理 files. IE9 没有 dataTransfer.files。
...
function read_file(file, handler, encoding, start, end) {
	...
	//	非标准! 但这样设定后，event handler 中即可使用 this.file 取得相关资讯。
	reader.file = file;
...
get_drag_path.dC = '<em>Drop Here</em> （不能拖曳多个物件）';
...
get_drag_path.able = function() {
	...
	try {
		...
		//	呼叫 navigator.* 可能有 -2147024882 "存放装置空间不足，无法完成此操作。"
		//	http://www.dotblogs.com.tw/alonstar/archive/2008/10/09/5625.aspx
		//	结果原来是我的机码不知道为什么多了一大堆.NET CLR 3.0的版本，找了一台正常的比对之后，把多的机码删除就好了。
...
get_drag_path.add = function(p) {
	...
	// 不能计算数目：不准 可能会有 file:///, http:// 等
	get_drag_path.href_hash[this.href = p] = 1;
};

//	每 get_drag_path.interval 执行一次
function get_drag_path() {
	var _s = arguments.callee,
	// 或者直接用 dropT.location.href，不透过 document.getElementById.
	...
	if (w.location.href != _s.href || w.location.href != bU) {
		...
		if (w.location.href != bU) {
			...
			//	drag 的都是完整 path。getPathOnly 应该先于 unescape，预防 # 等字元。
			if (/^[a-z]:\//i.test(i))
				i = getFP(i.replace(/\//g, '\\'));
