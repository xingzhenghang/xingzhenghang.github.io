...
/**
 * @name CeL 下拉式表单 function
 * @fileoverview 本档案包含了下拉选择式表单的 functions。
 * 
 * @example<code>
...
function module_code(library_namespace, load_arguments) {
	...
	/**
	 * 简易型 interact.DOM.XML_node @ interact.form.select_input
	 * @param tag
	 *            p.appendChild tag
	 * @param p
	 *            parent node
	...
	/**
	 * scroll 到可以看到 object TODO: 考虑可能没 scrollbar 包括横向
	 * 
	 * @param o
	 *            object
	 * @param [p]
	...
	<checkbox>
	+<label>
	autocomplete: 假如所有备取 list 都有一样的 prefix，则自动完成。
		把后面的用反白自动完成。
	
	在 list 上下安排三角，onmouseover 即可自动滚动。
	...
	var
		...
		/*	可纪录的 set class name，不过对大多数人来说，更常用的是 instance.setClassName
		
		usage:
		(item[, obj])		set obj to className item, return real className that assigned
		(0,'prefix')	set prefix & 重设（全部重跑）
		*/
		...
		*/
		...
		readyTo = function (e, o) {
			...
			if (e.type === 'mouseover' || e.type === 'mouseout') {
				...
				if (_p.readyItem) {
					library_namespace.debug('readyTo: 以键盘选择: ' + _p.readyItem.innerHTML, 4);
					cK = c, _p.readyItem.onclick();	//	用 .click() 无效!
					return false;
				} else return;
				//	key input 用键盘控制上下	←↑→↓: 37~40
			} else if (c == 38 || c == 40) {
				cK = c;
				o = _p.readyItem;
				library_namespace.debug('readyTo: 以键盘移至: ' + (o && (o.getAttribute("sIndex") + ',' + o.sIndex)), 4);
				if (!o) o = _p.listA[c == 40 ? 0 : _p.listA.length - 1];
				else {
					//	IE 可用 getAttribute，FF 或许在 appendChild 之后属性重设?，得用 o.sIndex
		...
		setList = function (l, force, limit, f) {
			...
			//	add function
			if (c != _t.allListCount) funcButton(_t, _.textSet.allBtn, function () { _t.doFunc = 1; _t.focus(); _t.setList(_t.setAllList(), 1, Number.MAX_VALUE); }, '显示全部 ' + _t.allListCount + ' 列表。');
			if (_t.setValue()) funcButton(_t, _.textSet.clearBtn, function () { _t.doFunc = 2; _t.focus(); _t.onInput(_t.setValue('')); }, '清除输入内容，重新列表。');
		...
		//	简易设定常用的 onInput 型式
		searchInList = function (f, o) {	//	o: 传入 (list, index, key)
			var _t = this;
			if (typeof f === 'string' && (f in _.searchFunctionSet)) f = _.searchFunctionSet[f];

			//	因为允许传入 list，所以不能在这边用 _t.setAllList() 判别函数，而得要写一个泛用的。
		...
		/**
		 * 切换 [input] / inputted [span]
		 * @param {Boolean|undefined} to_input	切换至 input or not. default: 切换至 [input]
		 * @return
		 * @private
		 * @inner
		 * @ignore
		...
		toggleToInput = function (to_input) {
			...
			if (to_input || typeof to_input === 'undefined') {
				...
				if (to_input = library_namespace.get_style
					...
					try {
						//	.get_style(_p.arrowO, 'width') 可能回传 'auto' @ IE8
		...
		*/
		layout = function (o) {
			...
			if (!o || (o.tagName.toLowerCase() in { hr: 1, br: 1 })) return;	//	** 这边应该检查 o 是不是 <hr /> 等不能加 child 的！

			//library_namespace.debug(('layout: use <' + o.tagName + (o.id ? '#' + o.id : '') + '>: ' + o.innerHTML).replace(/</g, '&lt;'));

			//	TODO: 这边应该有一个更完善的删除策略
			if (_t.loaded) {
				t = _p.container;
				//	不必多做功，已经达到所需配置了。
			...
			_p.inputO.onblur = function () {
				...
				/*	设定这项在按 _p.arrowO 的时候会出问题，所以建议在其他地方自订。
				if (_t.setValue() && (_t.setValue() in _t.setAllList()))
					_t.toggleToInput(0);
				*/

				//	TODO: 假如以键盘离开，应该也 showList(0);
			...
			_p.arrowO.onclick = function () {
				...
				if (_t.showList())
					...
					//	没在显示就把他开起来: setValue 设定完 list，onInput 模拟 key-down
		...
		initI = function (o, l, s) {	//	(HTML object, list: Array or Object, onInput handler)
			...
			if (_p.arrowO)
				_p.arrowO.style.display = _t.allListCount ? 'inline' : 'none';	//	无 list 的话先不显示，等有 list 再说。
	...
	*/
	var _// JSDT:_module_
		...
		//	(instance private handle)	不要 instance private 的把这函数删掉即可。
	...
	//	(for inherit)	不要 inherit 的把这段删掉即可。
	...
	//	default 栏位验证 pattern
	//	http://blog.wu-boy.com/2009/06/16/1454/
	//	TODO: ID, Age, 电话, 地址, 性别, ...
	...
	_.prototype = {
		...
		*/
		...
		//	自动于有 list 时 show arrow，无时 hide
		autoShowArrow: 0,

		//	设定要显现的 list，会回传 list，需注意可能被更改！
		...
		*/
		...
		//	每次 input 就会被 call 一次。可用 instance.setSearch('includeKey') 简易设定
		onInput: function (k) {	//	key
		},

		//	设定表单文字栏位的栏位验证	return 1: warning, 2: error ('Suffering from a pattern mismatch'), string: 将输入改为回传值, else OK
		//	另外可设定 onkeypress(){return true/false;} 来对每一次按键作 check。但这不能处理 paste。	http://irw.ncut.edu.tw/peterju/jscript.html#skill
		...
		*/
		...
		//	set inputted value: 转换成输入过的 <span> 时，设定其之值。
		...
		/*
			for Unobtrusive JavaScript: 为未启用JavaScript的情况提供替代方案。
			接上 <input> 或 <select>
		*/
		...
		//	(focus or blur, 不驱动 onfocus/onblur)
		focus: function (f) {	//	,noE
			...
			if (f || typeof f === 'undefined')
				try {
					//	@IE5-8 initial:  Error @CeL: 2110 [Error] (facility code 10): 控制项不可见、未启动或无法接受焦点，因此无法将焦点移到控制项上。
