...
各原始码库设定档: source_repositories.json

TODO:
Sorting message id by reference.
对于只有特定 repository 引用的讯息，可依照储存库分割到不同 .js 档案。
测试原文与翻译讯息首尾的标点符号是否相符。
		...
		.replace(PATTERN_version_stamp,
			function (all, header, v1, v2, _v3, v3, footer) {
				...
				if (v1 === matched[1] && v2 === matched[2]) {
					...
					// 主要 major 版本号或 minor 版本号改变，重新设定 patch 版本号 v3 为 base.js 中的号码。
					;
				}
...
// message_to_localized_mapping['en-US'] = {"Original language message 原文讯息 in source":"localized message"}
...
// qqq_data.get('message_id') = {message: "Original language message 原文讯息 in source", original_message_language_code: 'en-US|cmn-Hant-TW', notes: "notes", scope: "source/", demo: "demo URL / application of the message", additional_notes: "additional notes"}
...
function load_previous_qqq_data(resources_path) {
	...
	for (const [message_id, qqq_data] of Object.entries(contents)) {
		...
		// 因为原始码只要更动过就可能影响行数，所以每次执行重新抓取设定行数。
	...
	locale_messages.forEach(record => {
		...
		for (const [language_code, index] of Object.entries(language_code_to_index_hash)) {
			...
			if (local_message_of_CSV) {
				...
				CeL.error(`${load_CSV_message_to_localized.name}: 设定相异的本地化翻译于 ${resource_file_path}`);
	...
	CeL.storage.traverse_file_system(resources_path, fso_path => {
		...
		try {
			...
			try {
				...
				CeL.error(`${load_message_to_localized.name}: 无法解析在地化讯息档案: ${fso_path}`);
				console.error(e);
			}
		}
...
function parse_qqq(qqq) {
	...
	for (let matched; matched = notes.match(/\s*\(([^():]+):((?:\(\)|[^()])+)\)$/);) {
		...
		if (!qqq_data[property]) {
			...
			// 警告: 每次 .references, .repositories 都会在 adapt_new_change_to_source_file() 中被重新设定!
...
function set_qqq_data(message_id, qqq, options) {
	...
	if (old_qqq_data) {
		...
		// qqq_data.references, qqq_data.repositories 应该在 adapt_new_change_to_source_file() 设定。
		delete qqq_data.references;
		delete qqq_data.repositories;
		// 此属性应由编程者决定。
...
function create__qqq_data_Map() {
	for (const [message_id, qqq] of Object.entries(i18n_message_id_to_message.qqq)) {
		...
		if (qqq_data) {
			// reset qqq_data.references: 舍弃旧的 .references 资讯，将在 adapt_new_change_to_source_file() 重新设定。
			delete qqq_data.references;
			delete qqq_data.repositories;
		}
	}
	...
	for (const message of message_to_id_Map.keys()) {
		...
		if (!qqq_data.message) {
			// set Original language message 原文讯息 qqq_data.message
			qqq_data.message = message;
			...
			if (!matched || qqq_data.message !== matched[2]) {
				...
				CeL.info(`${create__qqq_data_Map.name}: Original language message 原文讯息 changed in translatewiki:`);
				CeL.log(CeL.display_align([
					['id\t', message_id],
					['原\t', qqq_data.message],
					['新→\t', message]
			...
			// 一次性执行的特殊情况: 之前更新了 ISO 639-1 language tag.json 与 IETF language tag.js，却没更新 qqq_data.json 与原始码。
			...
			if (i18n_message && i18n_message !== message && i18n_message.covers(message)) {
				CeL.warn(`${create__qqq_data_Map.name}: 可能忘了更新原始码讯息？若有需要请手动更新：`);
				CeL.log(CeL.display_align([
					['id\t', message_id],
					['原始码\t', JSON.stringify(message)],
		...
		if (!qqq_data.original_message_language_code) {
			// set Original language message 原文讯息之语言代码 qqq_data.original_message_language_code
 ...
 * 增加原始档的在地化语言注记。
 * 
 * @param {String} script_file_path 原始档路径
 */
...
function adapt_new_change_to_source_file(script_file_path, options) {
	...
	for (let line_index = 0; line_index < content_lines.length - 1;) {
		...
		/** 原始程式码 gettext_config 标记列匹配的模式。 matched: [ all line, front, gettext_config, tail ] */
		...
		/** 原始程式码讯息列匹配的模式。 matched: [ all line, front, quote, message 讯息, tail ] */
		...
		if (is_mark_type_ignoring_message) {
			...
			if (!message) {
				CeL.warn(`${adapt_new_change_to_source_file.name}: 新的特殊型态标记 message id 未设定: ${JSON.stringify(gettext_config)}。您必须手动设定 en 或 message 属性！`);
			...
			if (message_line_matched) {
				// 警告: message 不可分割，否则会找不到。且本行的第一个字串就必须是 message，否则会导致误判！
		...
		if (!message_id && !gettext_config.id && (gettext_config.en || !PATTERN_has_invalid_en_message_char.test(message))) {
			...
			gettext('Original language message 原文讯息');
			...
			if (PATTERN_has_invalid_en_message_char.test(gettext_config.id)) {
				CeL.warn(`${adapt_new_change_to_source_file.name}: 自动生成的ID包含非英语字元: ${JSON.stringify(gettext_config.id)}`);
			}
		}
		if (message_id) {
			...
			throw new Error(`${adapt_new_change_to_source_file.name}: [${script_file_path}] 原始码中新增了 message，但未设定且无法自动判别 message id: ${JSON.stringify(message)}`);
			...
			if (message_changed.has(qqq_data.message)) {
				if (message_changed.get(qqq_data.message) !== message) {
					throw new Error(`${adapt_new_change_to_source_file.name}: message 冲突:\n原 message	${JSON.stringify(qqq_data.message)}\n→ i18n或其他原始码中的 message:	${JSON.stringify(message_changed.get(qqq_data.message))}\n→ [${script_file_path}] 原始码中的 message:${JSON.stringify(message)}`);
				}

			} else if (qqq_data.message !== message
				// e.g., 新讯息遇到 'Does not exist: ' 但原先已有 'Does not exist'
				...
				if (!new_message_language_code && !PATTERN_has_invalid_en_message_char.test(message)) {
					...
					CeL.warn(`${adapt_new_change_to_source_file.name}: 无法判别 message 之语言，当作 ${new_message_language_code}:\n[${message_id}] ${JSON.stringify(message)}`);
				}
				if (new_message_language_code && new_message_language_code !== qqq_data.original_message_language_code) {
					...
					CeL.warn(`${adapt_new_change_to_source_file.name}: 改变了[${script_file_path}] ${message_changed ? '原始码中的讯息以及语言' : '原始码中讯息的语言'}:`);
					CeL.log(CeL.display_align([
						['id\t', JSON.stringify(message_id)],
						['原讯息\t', `[${qqq_data.original_message_language_code}] ${message_changed ? JSON.stringify(qqq_data.message) + ` ([${new_message_language_code}] ${locale_data[message_id]})` : ''}`],
						['新→\t', `[${new_message_language_code}] ${message_changed ? JSON.stringify(message) + (message === message_id ? ' (当作为 message id)' : '') : ''}`]
					]));
					qqq_data.original_message_language_code = new_message_language_code;
				} else {
					CeL.info(`${adapt_new_change_to_source_file.name}: 改变了[${script_file_path}] 原始码中的 message:`);
					CeL.log(CeL.display_align([
						['id\t', JSON.stringify(message_id)],
						['原讯息\t', JSON.stringify(qqq_data.message)],
			...
			CeL.info(`${adapt_new_change_to_source_file.name}: [${script_file_path}] 原始码中新增了${is_mark_type_ignoring_message ? '特殊型态标记' : ' message'}: [${message_id}] ${JSON.stringify(message)}`);
			...
			if (gettext_config.id === message_id) {
				if (!language_code) {
					if (PATTERN_has_invalid_en_message_char.test(message)) {
						CeL.error(`${adapt_new_change_to_source_file.name}: 无法判别 message 之语言!\n[${message_id}] ${JSON.stringify(message)}`);
					} else {
						language_code = 'en-US';
						CeL.warn(`${adapt_new_change_to_source_file.name}: 无法判别 message 之语言，当作 ${language_code}:\n[${message_id}] ${JSON.stringify(message)}`);
					}
				}
				if (language_code === 'en-US') {
					/**
					 * 添加讯息的方法: 直接把 Original language message 原文讯息当英文讯息。
					 * e.g., <code>
					...
					if (!gettext_config.en) {
						CeL.error(`${adapt_new_change_to_source_file.name}: 原始码中新增了非 en-US 之 message 却未提供 ${CeL.gettext.get_alias('en-US')} 讯息! 这会造成无法汇入 translatewiki! ${JSON.stringify(message)}`);
					}
				...
				if (!language_code) {
					CeL.warn(`${adapt_new_change_to_source_file.name}: 无法判别 message 之语言! ${JSON.stringify(message)}`);
				} else if (language_code === 'en-US') {
					if (message !== gettext_config.id)
						CeL.error(`${adapt_new_change_to_source_file.name}: 判别 message 与 message id 同为 en-US 但两者不同!\n	${JSON.stringify(message)}\n	${JSON.stringify(gettext_config.id)}`);
				} else {
					let locale_data = i18n_message_id_to_message[language_code];
					if (!locale_data) {
		...
		for (const [property_name, value] of Object.entries(gettext_config)) {
				...
				case 'id':
					...
					if (message_id_changed.has(message_id)) {
						if (message_id_changed.get(message_id) !== value) {
							throw new Error(`${adapt_new_change_to_source_file.name}: message id 冲突:\n原 message	id ${JSON.stringify(message_id)}\n→ 其他原始码中的 message id:	${JSON.stringify(message_id_changed.get(message_id))}\n→ [${script_file_path}] 原始码中的 message:${JSON.stringify(value)}`);
						}

					} else {
						CeL.info(`${adapt_new_change_to_source_file.name}: [${script_file_path}] 原始码中改变了 message id:`);
						CeL.log(CeL.display_align([
							['原\t', JSON.stringify(message_id)],
							['新→\t', JSON.stringify(value)]
						]));
						if (!qqq_data_Map.has(message_id)) {
							CeL.error(`${adapt_new_change_to_source_file.name}: 改变前之 message id 不存在: ${message_id}`);
						}
						if (qqq_data_Map.has(value)) {
							throw new Error(`${adapt_new_change_to_source_file.name}: 改变后之 message id 已存在: ${value}`);
						}
						message_id_changed.set(message_id, value);
						qqq_data.need_to_recheck_all_sources = true;
						// 当原始码中改变 message id 时，不会一同变更 qqq_data_Map, message_to_id_Map, i18n_message_id_to_message。唯一只会纪录于 message_id_changed。
				...
				if (language_code) {
					...
					if (!value) {
						if (i18n_message_id_to_message['en-US'][message_id])
							continue;
						CeL.warn(`${adapt_new_change_to_source_file.name}: 设定 [${language_code}].[${message_id}] 为空字串。`);
					}

					const old_value = i18n_message_id_to_message[language_code][message_id];
					if ((old_value/* || old_value === 0*/) && old_value !== value) {
						CeL.info(`${adapt_new_change_to_source_file.name}: [${script_file_path}] 原始码中改变了 [${message_id}] 的 ${language_code} 讯息:`);
						CeL.log(CeL.display_align([
			...
			if ((old_value || old_value === 0) && old_value !== value) {
				CeL.info(`${adapt_new_change_to_source_file.name}: [${script_file_path}] 原始码中改变了 [${message_id}].${JSON.stringify(property_name)}:`);
				CeL.log(CeL.display_align([
					['原\t', JSON.stringify(old_value)],
					['新→\t', JSON.stringify(value)]
				]));
		...
		// 原始码中仅留存 message id, mark_type，其他全部移到 qqq_data。
		...
		if (message_changed.has(message)) {
			...
			CeL.info(`${adapt_new_change_to_source_file.name}: [${script_file_path}] 更新原始码中的讯息: [${message_id}]:`);
			CeL.log(CeL.display_align([
...
function adapt_message_id_changed_to_Map(map) {
	for (const [old_message_id, new_message_id] of message_id_changed.entries()) {
		...
		if (map.has(new_message_id) && map.get(new_message_id) !== map.get(old_message_id)) {
			...
			throw new Error(`${adapt_message_id_changed_to_Map.name}: 已经有此 message id，无法更名: ${JSON.stringify(new_message_id)} ← ${JSON.stringify(old_message_id)}`);
		}
...
function adapt_message_id_changed_to_Object(object) {
	for (const [old_message_id, new_message_id] of message_id_changed.entries()) {
		...
		if (Object.hasOwn(object, new_message_id)
			// 更改 message id 后，会在先前就写入 object[new_message_id]。
			&& object[new_message_id] !== object[old_message_id]) {
			console.trace([object[new_message_id], object[old_message_id]]);
			throw new Error(`${adapt_message_id_changed_to_Object.name}: 已经有此 message id，无法更名: ${JSON.stringify(new_message_id)} ← ${JSON.stringify(old_message_id)}`);
		}
		object[new_message_id] = object[old_message_id];
...
function write_qqq_data(resources_path) {
	...
	for (const [message_id, qqq_data] of qqq_data_Map.entries()) {
		...
		if (original_message_locale_data && !original_message_locale_data[message_id]) {
			CeL.info(`${write_qqq_data.name}: 填补原语言 [${qqq_data.original_message_language_code}] 之讯息 [${message_id}] ${qqq_data.message}`);
		...
		if (Array.isArray(qqq_data.references) && qqq_data.references.length > 0) {
			...
			if (false && !qqq_data.references) {
				CeL.warn(`${write_qqq_data.name}: 原始码中无明确引用的讯息: [${message_id}] ${qqq_data.message}`);
			}
		...
		if (Array.isArray(qqq_data.repositories) && qqq_data.repositories.length > 0) {
			...
			if (qqq_data.repositories.length === 1) {
				// TODO: 对于只有特定 repository 引用的讯息，依照 repository 分割到不同 .js。 
			}
	...
	if (message_id_without_references.length > 0) {
		CeL.info(`${write_qqq_data.name}: 原始码中无明确引用的讯息: ${message_id_without_references.length}/${qqq_data_Map.size}`);
	...
	for (const [language_code, locale_data] of Object.entries(i18n_message_id_to_message)) {
		if (language_code !== 'qqq') {
			...
			if (untranslated_message_count < 500 || untranslated_ratio < .3) {
				const comments = untranslated_message_count === 0 ? '已翻译完毕的语言'
					: untranslated_message_count < 20 && untranslated_ratio < .01 ? '几近翻译完毕的语言'
						: untranslated_message_count < 100 && untranslated_ratio < .05 ? '翻译得快完成的语言'
							: '可考虑列入选单的语言';
				CeL.info(`${write_i18n_files.name}: ${comments} (${untranslated_message_count}/${qqq_data_Map.size} 未翻译): ${language_code}`);
			}
...
function write_message_script_file({ resources_path, language_code, locale_data, message_id_order }) {
	...
	for (const [message_id, locale_message] of Object.entries(sort_Object_by_order(locale_data, message_id_order))) {
		...
		if (/^function(?:\s|\()/.test(locale_message)) {
			...
			CeL.error(`${write_message_script_file.name}: [${language_code}][${message_id}]: 原讯息与新函数不一致！您必须检核此函数是否有安全疑虑，之后手动将之从字串改为函数 [${fso_path}]！\n原	${original_function}\n新	${locale_message}`);
		}
...
function convert_i18n_plain_header_tail(plain_message) {
	...
	if (/&#/.test(plain_message) && plain_message !== i18n_message) {
		CeL.warn(`${convert_i18n_plain_header_tail.name}: 于首尾有空格并且讯息中含"&#"。仍将强制转换首尾有空格。`);
	}
