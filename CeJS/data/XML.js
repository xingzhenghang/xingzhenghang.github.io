 ...
 * @fileoverview 本档案包含了处理 XML file 的 functions。
 * @since
 */
...
function module_code(library_namespace) {
	 ...
	 * <code>
	 ...
	 getXML():
	 loadXML(getU('全省空白价目表.xml')).getElementsByTagName("Worksheet").length


	 TODO:
	 可参考 JKL.ParseXML, http://doctype.googlecode.com/svn/trunk/goog/dom/xml.js
	...
	function loadXML(XMLtext, flag) {
		...
		try {
			 ...
			 * 
			 * フリースレッド DOM ドキュメントを使用すれば、ファイルを共有アプリケーション状态に取り込むことができます。
			 * 
			 * フリースレッド モデルの欠点の 1
			 * つは、未使用のメモリのクリーンアップにおける待ち时间が増大し、それ以降の操作のパフォーマンスに影响を及ぼすということです
			 * (実际にはクリーンアップが遅れているだけなのに、これをメモリ リークとして报告してくる人もいます)。
			 * 
	...
	/**
	 * 设定 node 之 attributes。
	 * 
	 * @param {Object}node
	 *            已设定过 tag name 之 node
	...
	// XML 之 tag name 应允许 ":"。
	// 无 end-tag 之 node (empty-element tag) pattern。
	// http://www.w3.org/TR/REC-xml/#sec-starttags
	// [ , tag name, attributes ]
	PATTERN_EMPTY = /<([^\0-\,;->\s]+)(\s[^<>]*?)?\/>/,
	// end-tag 之 pattern。必须允许 ":","-"。
	...
	function XML_to_JSON(XML, options) {
		...
		for (var index = 0, matched; index < nodes.length; index++) {
			...
			+ '(\\s[^<>]*)?>([\\S\\s]*?)$'); i >= 0; i--) {
				...
				// 不同 node 且有 tail 则少删一个。
				- (i !== index && tail ? 1 : 0), node);
				// 去掉头尾多切割出的部分。
				if (tag_start_from_head) {
					// head srction 从一开始就是 tag，因此直接将之除掉。
					children.shift();
				}
				if (i !== index && !tail) {
					// end srction 到结尾都是 end tag，因此直接将之除掉。
					children.pop();
				}
