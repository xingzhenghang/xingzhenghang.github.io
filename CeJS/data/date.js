 ...
 * @fileoverview 本档案包含了 date / time 的功能。
 * 
...
function module_code(library_namespace) {
	...
	/** {Number}一整时辰的 time 值。should be 2 * 60 * 60 * 1000 = 7200000. */
	ONE_时辰_LENGTH_VALUE = new Date(0, 0, 0, 2) - new Date(0, 0, 0, 0),

	// e.g., UTC+8: -8 * 60 = -480
	present_local_minute_offset = (new Date).getTimezoneOffset() || 0;

	// ----------------------------------------------------------------------------------------------------------------------------------------------------------//
	// for Julian date. 期能不使用内建 Date 以快速计算日期。
	// @see https://en.wikipedia.org/wiki/Julian_day#Calculation
	// 适用范围: 4717/3/1 BCE 0:0 之后。

	/**
	 * Get Julian day number (JDN) of date.<br />
	 * If type of date is Date, we'll treat date as local date.<br />
	 * 因为得出的是 UTC+0 12:0 之 JDN，UTC+0 0:0 之 JD = JDN - .5。
	 * 
	 * JDN = Math.round(JD);
	 * 
	...
	function Julian_day(date, type, no_year_0, get_remainder) {
		if (typeof date === 'string') {
			...
			if (library_namespace.is_debug(2)) {
				library_namespace.error('Julian_day: 无法解析 [' + date + ']！');
			}
			return;
		}
		...
		if (!isNaN(date)) {
			...
			type = true;
			date -= offset
			// epoch 为 12:0，需要将之减回来以转成 midnight (0:0)。
	...
	Julian_day.from_YMD = function(year, month, date, type, no_year_0) {
		...
		if (type === 'CE') {
			type = year > 1582
			// Julian calendar（儒略历）1582年10月4日的下一日为
			// Gregorian calendar（格里高利历）1582年10月15日。
	...
	Julian_day.to_YMDHMS = function(JD, zone, type, no_year_0) {
		...
		if ((JD %= 1) > 4e-11) {
			// 8.64e-9 = 1e-16 * 86400000: 将之前加的 error 修正补回来。
			// 约精确到 1e-7 ms
	...
	// 数字 32331.06 代表日期和时间 7/7/1988年 1:26:24 a.m。
	var Excel_1900_epoch = Date.parse('1900/1/1') - ONE_DAY_LENGTH_VALUE,
	// The 1904 Date System
	// 依预设，Excel for Mac 使用 1904 日期系统，而 Excel for Windows 使用 1900 日期系统。这表示当您在
	// Excel for Mac 中输入序列值 1 并将其格式化为日期，Excel 会将其显示为 1/2/1904 12:00 a.m。Excel
	// for Windows 则会将序列值 1 显示为 1/1/1900 12:00 a.m。
	...
	function Excel_Date(date_value, is_Mac, get_value) {
		// 0会被转成 1900/1/0，已经不正常。
		if (date_value >= 1) {
			// 这边采用与 function Date_to_Excel() 相对应的判别式。
	...
	// Excel 2010 会将错误值显示为'#VALUE!'，但负数或过大值则会以'#'填满格子(e.g., "#########")。
	...
	function Date_to_Excel(date, is_Mac) {
		...
		return date >= 1 ?
		// Excel 有 1900/2/29 (60)，但现实中没有这天。因此一般转换时，不应出现60之值。
		// Mac 系统以 1904 CE 起始，回避了这个问题。
		// 0: 1900/1/0
	 ...
	 * 
	 * TODO: parse /([今昨明]|大?[前后])天/, '01-03' (相对于当前),
	 * /\d+(分[钟钟]?|小[时时]|毫?秒|[日天周年]|星期|[礼礼]拜|[个个]月)([前后])/; 相对于
	 ...
	 *            {String|Number}zone: 设定 date_string 之 time zone or country
	 *            (e.g., 'CST', 'TW') || 时差 in hour (例如 TW: UTC+8 → 8, 可使用.5).<br />
	 *            {Date}reform: 对某些特殊 paser，如 CE，需要特别设定改历日期时用。<br />
	 *            <br />
	 *            {Boolean}period_end:<br />
	 *            将指定内容视为一时段，并取得此期间之结束（终结）时间，因此 parse 后将得到第一个不属于此范围之时刻。<br />
	 ...
	 * @since 2012/3/22 23:58:38 重构并测试。
	...
	function String_to_Date(date_string, options) {
		...
		if (PATTERN_ISO_DATE.test(date_string)) {
			// 对于有明确指定之 UTC date 如 .toISOString() 之产出或 ISO 8601，
			// 应当不管 time zone 如何设定，直接回传。
			return new Date(date_string);
		}
		...
		// 设定指定 time zone 之 offset in minutes.
		tmp = options.zone;
		library_namespace.debug('设定 time zone / offset hours: ' + tmp, 2);
		...
		library_namespace.debug('最终设定 offset '
		...
		if (library_namespace.is_Function(tmp)) {
			...
			// assert: parser 亦负责 parse time zone offset.
			minute_offset, options)) {
				return tmp;
			}
		}

		library_namespace.debug('无法以 parser 判别。use Date.parse() to parse.', 2);
		if (tmp = Date.parse(date_string)) {
			// TODO: period_end 无效。
			// native parser 会处理 time zone offset.
			tmp = new Date(tmp);
	...
	// e.g., UTC+8: 8 * 60 = +480
	// e.g., UTC-5: -5 * 60
	// 亦为 Date.parse(date_string) 与 new Date() 会自动附上的当地时间差距。
	// assert: String_to_Date.default_offset 为整数。
	String_to_Date.default_offset = -present_local_minute_offset;

	/**
	 * <code>
	主要指是否计算 0 year。
	.no_year_0 = true: 将 astronomical year numbering 转成一般纪年法（1 BCE→1 CE）。
	仅用于计算 Gregorian calendar, Julian calendar。
	normal	astronomical
	2	2
	1	1
	-1	0
	...
	(function() {
		...
		// e.g., for '1616年2月壬午', '7时'
		end_pattern = ')?[^\\d日时]*$',

		// pattern of date. 当今会准确使用的时间，
		// 为 -47xx BCE (Julian day 0) 至 2xxx CE。
		date_pattern = /(?:([\-−‐前]?(?:[0-4]?\d{3}|\d{1,3}))[\/.\-年 ])?\s*([01]?\d)(?:[\/.\-月 ]\s*([0-3]?\d)日?)?/.source,
		// pattern of time. [0-6]: 支持闰秒
		time_pattern = /([0-2]?\d)[:时时]\s*(?:([0-6]?\d)[:分]?\s*(?:([0-6]?\d)(?:\.(\d+))?)?)?秒?\s*(?:([PA])M)?/i.source;
	...
	// U+2212 '−': minus sign
	// 为了 calendar 测试，年分需要能 parse 0–9999。
	// [ all, .*年, \d+, [百千] ]
	...
	function String_to_Date_default_parser(date_string, minute_offset, options) {
		...
		date_string = date_string.trim()
		// 注意:"纪"会转换成结束时间。
		.replace(/世[纪纪]/g, '百年').replace(/千[纪纪]/g, '千年');
		...
		if (matched) {
			...
			if (isNaN(date_value)) {
				...
				library_namespace.debug('无法 parse: [' + date_string + ']', 2,
			...
			// .precision 将会影响 function wikidata_datavalue() @
		...
		if (matched = date_string.match(PATTERN_YEAR_ONLY)) {
			// 仅有 xxx/1xxx/2xxx 年(year) 时。
			precision = matched[3] === '百' ? 'century'
					: matched[3] === '千' ? 'millennium'
					// 注意：这边不会检查如"2016年代"之合理性（应当为"2010年代"）
		...
		if (period_end) {
			...
			// 由小至大，将第一个有指定的加一即可。
			while (tmp-- > 0) {
				// IE 中，String.prototype.match() 未成功时会回传 ''，
				// 而 !isNaN('')===true，因此无法正确判别。
		...
		// 若是未设定，则当作 local time zone。
		if (base_on_UTC) {
			// 否则基于本机当前的时区来调整成基于 UTC 之 `minute_offset`
		...
		if (year < 100 && year >= 0) {
			...
			// 先设定小单位，再设定大单位：设定小单位时会影响到大单位。反之不然。
			// 下两者得到的值不同。
		...
		if (base_on_UTC
			...
			/**
			 * 当基于本机当前的时区来调整成UTC时间时，若是 time zone 和预设的
			 * `present_local_minute_offset` 不同，就必须在以 new Date() 设定时间后，才调整 time
			 * zone。
			 */
		...
		if (options.near && date_value.getFullYear() === 0
			...
			// 判别未输入时预设年份设对了没：以最接近 options.near 的为基准。
	 ...
	 * 
	 * @param {Integer}year
	 * @returns {Boolean} 当年 Julian 与 UTC 为不同闰年规定: Gregorian 当年没有闰日，但 Julian 有。
	 */
	...
	/**
	 * 计算 Gregorian 与 Julian 的日数差距。 the secular difference between the two
	 * calendars.<br />
	 * 会将 date_data: Julian → Gregorian.
	 * 
	 * @param {Array}date_data
	 *            Julian date [year, month, date]
	 * 
	 * @returns {Number} Julian → Gregorian 时，需要减去的日数。（除少数特例外，即 Gregorian →
	 *          Julian 时，需要加上的日数。）
	 * 
	...
	function Julian_shift_days(date_data) {
		...
		// 测试是否为闰日。
		// 闰日前(before Julian calendar leap day)还要算是上一阶段。
		&& date_data[1] < 3) {
			year--;
		}
		// 计算 Gregorian 与 Julian 的 different days。
		// 2: 0年时，差了2日。
		// -701: 8, -700: 7; -601: 7, -600: 6; 99: 2, 100: 1;
		year = 2 + Math.floor(year / 400) - Math.floor(year / 100);
		// 这演算法对差异大至 31+28 日的时段不适用。
		date_data[2] -= year;
		return year;
	}
	 ...
	 * 
	 * 借用系统内建的计时机制。其他 arguments 见 String_to_Date_default_parser()。
	 * 
	 * @param {String}date_string
	...
	/**
	 * 显示格式化日期时间 string：依照指定格式输出日期与时间。<br />
	 * TODO:<br />
	 * 各 locale 有不同 format 与 time zone offset.
	 * 
	 * @param {Date}date_value
	 *            要转换的 date, TODO? 值过小时当作时间, <0 转成当下时间.
	 * @param {Object|String|Function}options
	 *            选择性功能: {<br />
	 *            {String|Function}parser: 格式字串分析器 'strftime',<br />
	 *            {String}format: 格式字串 '%Y/%m/%d %H:%M:%S.%f',<br />
	 *            {String}locale: 地区语系设定<br /> }
	 * 
	 * @returns {String} 依照指定格式格式化后输出的日期与时间.
	 * 
	 * @see<br />
	 *      <a href="http://blog.csdn.net/xzknet/article/details/2278101"
	 *      accessdate="2012/3/24 15:11" title="如何使用Javascript格式化日期显示 -
	 *      虫二的专栏~~在路上~~~ - 博客频道 - CSDN.NET">JsJava中提供了专用的类，专门对日期进行指定格式的字符串输出</a>,<br />
	...
	function Date_to_String(date_value, options) {
		...
		if (false) {
			if (options.parser
				...
				library_namespace.warn('Date_to_String: 无法判断 parser ['
						+ options.parser + ']！');
			}
		}
		...
		library_namespace.warn('Date_to_String: 无法判断 date value [' + date_value
				+ ']！');
	}
	...
	Date_to_String.parser = {
		...
		// accessdate="2012/3/24 17:43">标准日期和时间格式字串</a>
	...
	function strftime(date_value, format, locale, options) {
		...
		function convertor(s) {
			return s.replace(search, function($0, $1, $2) {
				...
				// 来取得另一 conversion 之结果。
				var v = $2 in original_Date ? original_Date[$2]
				// original_Date[$2] 为物件本身之特殊属性，应当排在泛用函数 conversion[$2] 之前。
	...
	/**
	 * 设定支援的 conversion specifications (转换规格).<br />
	 * 将直接使用输入，因此呼叫后若改变 conversion specifications object 将出现问题！<br />
	 * 
	 * @example <code>
	 ...
	 *            conversion specifications (转换规格)。
	 * @param {String}locale
	 *            输出的地区语系设定。
	 * 
	 * @returns {RegExp} 所须搜寻的 conversion specifications (转换规格) pattern。
	 */
	...
	// common year 月初累积日。查表法用。
	// accumulated_days[0] = 到一月月初时累积的天数。
	...
	/**
	 * 自 year/month/date 开始之本年历数（每月日数）。
	 * 
	...
	/**
	 * ordinal date, 年日期.<br />
	 * 本年开始至 year/month/date 之本年年内的天数。
	 * 
	 * @param {Integer}month
	 *            月
	 * @param {Integer}date
	...
	// week date, 周日期, 表示年内的星期数天数，再加上星期内第几天。
	...
	function week_date(date, to_ISO) {
		...
		if (to_ISO) {
			// TODO: 在IS0 8601中星期以星期一开始
			// 一年的首星期必须包含1月4日, 包含一年的首个星期四
			year = year.pad(4);
			week = 'W' + week.pad(2);
		}
	...
	// 除非必要，这边不应用上 options.original_Date。
	strftime.default_conversion = {
		// ----------------------------
		// date

		// 完整年份(非两位数的数字，近十年应为四位数的数字，如2013) 以4位十进制数写年份。
		...
		// 月中的第几天 (1-31) 以十进制数写月的第几日（范围[01,31]）。
		...
		// 2017/8/16 增加在地化的星期名称表示法。之前曾经用过"%w"这个方式的，都需要改成"%A"。
		// 写缩略的星期日期名，例如Fri（本地环境依赖）。
		a : function(date_value, options) {
			return gettext_date.week(
			// (options && options.original_Date || date_value)
			date_value.getDay(), options.locale);
		},
		// 写完整的星期日期名，例如Friday（本地环境依赖）。
		A : function(date_value, options) {
			return gettext_date.full_week(
			// (options && options.original_Date || date_value)
			date_value.getDay(), options.locale);
		},
		// 星期 (0-6) 以十进制数写星期日期，其中星期日是0（范围[0-6]）。
		...
		// 以 ISO 8601 格式（例如 -0430 ）写距 UTC 的偏移
		...
		// 计算距离甲子共有几日，再于 index_to_stem_branch() 取模数。
		// 假定为不间断之循环纪日。
		...
		// 时辰干支: 计算距离甲子共有几个时辰，再于 index_to_stem_branch() 取模数。
		// 时干支不受子初分日(子初换日/子正换日)影响。
		时干支序 : hour_stem_branch_index,
		时干支 : function(date_value, options) {
		...
		// 每刻15分。
		// e.g., 子正初刻
		// 隋后普遍行百刻制，每天100刻。至顺治二年（公元1645年）颁行时宪历后，改为日96刻，每时辰八刻（初初刻、初一刻、初二刻、初三刻、正初刻、正一刻、正二刻、正三刻）[7,13,14]。自此每刻15分，无「四刻」之名。
		时刻 : function(date_value, options) {
	...
	function Date_to_Gregorian(date_value, format, locale, options) {
		...
		if ('no_year_0' in options ? options.no_year_0
			...
			if (FullYear < 1 || UTCFullYear < 1) {
				// 处理 0 year: Gregorian 与 Julian 没有 0 year。
				// 因为公元前之公历闰年出现在 1, 5, 9, 13, .. BC，因此无法以「除以4」计算。
				// 应该直接改年分，而不是将时间直接将其值减掉 (0/1/1 0:-1/1/1 0:),
				// 而当作仿佛没有 0年这段期间的存在，直接从-1年开始。
				//
				// <a
				// href="http://en.wikipedia.org/wiki/Astronomical_year_numbering"
				// accessdate="2013/2/11 15:40" title="Astronomical year
				...
				if (UTCFullYear < 1) {
					...
					// 一般情况 (FullYear === UTCFullYear)。
					// else: 预防 1/1, 12/31 时，getYear()
					// 与 getUTCFullYear() 可能会有不同值。
					// 尽量少带入变数。
					date_value.getFullYear : new Function('return '
							+ (UTCFullYear - 1));
				}
	 ...
	 * 
	 * 以系统内建的计时机制，<br />
	 * 将扩展/外推/延伸的格里历, proleptic Gregorian calendar<br /> → proleptic Julian
	...
	function Date_to_Julian(date_value, format, locale, options) {
		...
		// 计算 Gregorian 与 Julian 的 different days。
		// 0年时，差了 2日。这演算法对差异大至 31+28 日的时段不适用。
		shift_days = 2 + Math.floor(year / 400) - Math.floor(year / 100),
		// 当年 Julian 与 UTC 为不同闰年规定: Gregorian 当年没有闰日，但 Julian 有。
		...
		if (shift_days || is_leap_year) {
			...
			// 因为可能会更改 date_value，因此把本来的 date_value 放在 options
			// 中供有需要的取用。
			...
			// 以新的 UTC Date instance 模拟 Julian calendar。
			...
			if (is_leap_year && date_value.getFullYear() <= year) {
				...
				// 分水岭: 以 Julian date 3/1 0:0 为分界。
				// 在不同闰年当年，Julian date 3/1 前需要特别处理。
				if (month < 2 || date_value.getFullYear() < year) {
					// 因为加至当年没有闰日的 Gregorian，2/29 会变成 3/1。
					if (month === 1 && date_value.getDate() === 28) {
						// is leap day
						// 便宜行事: 不设 delta，直接把 3/1 → 2/28，再强制使 .getDate() = 29。
						// TODO: .getDay() 恐有误。
						// 当 Julian date 2/29 闰日当日，UTC 非闰日的时候，需要特别处理。
						// TODO: 处理其他。
						date_value.getDate = leap_date;
					} else {
						// Julian date 2/29 闰日前。少算的，须更正。
						// 闰日前(before Julian calendar leap day)还要算是上一阶段。
			...
			// 处理 day of the week: 就算以另一个日期模拟 UTC，原先的星期不会改变。
	...
	// 设定 .as_UTC_time 当作 UTC 时，将加上为本地时间调整所需之 offset。
	...
	function Date_to_JDN(date_value, options) {
		// JDN0: JD = -.5 – .5⁻
		// JDN0: JD + .5 = 0 – 1⁻
		// 精神:以 UTC 计算时，当天从头至尾都是相同的 JDN。
		// 基本上世界每个地方在当地当天 12:0 都有相同的 JDN，但不保证世界每个地方在当地当天 0:0 都有相同的 JDN。
		return Math.floor(Date_to_JD(date_value, options) + .5);
	}
	// Time Conversion Tool
	// http://ssd.jpl.nasa.gov/tc.cgi
	function JD_to_Date(JD) {
		// 注意：此输出常显示为系统之 proleptic Gregorian calendar，
		// 而一般天文计算使用 proleptic Julian calendar！
	...
	// Julian Date: 由公元前4713年1月1日，协调世界时中午12时开始所经过的天数。
	// 原点实际设在 -004713-11-24T12:00:00.000Z。
	...
	// 预设的 Gregorian calendar 改历日期:
	// Julian calendar → Gregorian calendar.
	//
	// 这天开始使用 Gregorian calendar。之前使用 Julian calendar。
	// e.g., UTC/Gregorian 1582/10/14 ⇔ Julian 1582/10/4.
	//
	// 西历改历分界点。这天之后采用 Gregorian calendar 表示。
	// 西历以1582年10月15日为改历分界点，Julian calendar（儒略历）1582年10月4日的下一日为 Gregorian
	// calendar（格里高利历）1582年10月15日。
	...
	var reform_by_region = {
		...
		'Denmark' : '1700/3/1',
		// Kingdom of Great Britain, 大不列颠王国, グレートブリテン王国, 英国
	...
	/**
	 * 取得特定区域之 Gregorian calendar 改历日期。
	 * 
	 * @param {String}region
	...
	// 12 地支. Earthly Branches. TODO: 异体字如"夘"
	BRANCH_LIST = '子丑寅卯辰巳午未申酉戌亥',
	// 60: lcm(STEM_LIST.length, BRANCH_LIST.length);
	SEXAGENARY_CYCLE_LENGTH = 60,

	// 当考虑以 CST，或以当地时间为准。
	// 因为八字与经度，以及该经度与太阳的日照相对夹角有关，因此采当地时间为准。

	// CE YEAR_STEM_BRANCH_EPOCH 3-12月为甲子年。
	// 以此计算 new Date(0) 之 offset。
	// 黄帝纪元元年 (year -2696 in proleptic Gregorian calendar) 为甲子年。
	YEAR_STEM_BRANCH_EPOCH = -2696,
	// 1984/3/31 0:0 (甲子年丁卯月)甲子日始: 以此基准点计算 new Date(0) 之 offset。
	DATE_STEM_BRANCH_EPOCH = new Date(1984, 3 - 1, 31),
	//
	DATE_STEM_BRANCH_minute_offset = DATE_STEM_BRANCH_EPOCH.getTimezoneOffset() || 0,
	// 1984/3/31 23:0 (甲子年丁卯月)甲子日甲子时始: 以此基准点计算 new Date(0) 之 offset。
	...
	function hour_stem_branch_index(date_value, options) {
		...
		- (date_value.getTimezoneOffset() - HOUR_STEM_BRANCH_minute_offset)
				* ONE_MINUTE_LENGTH_VALUE;
		return date_value / ONE_时辰_LENGTH_VALUE;
	}

	// 极大程度依赖 date_pattern 的写法！
	...
	function convert_stem_branch_date(stem_branch, date_value, end_date_diff) {
		...
		if (!isNaN(stem_branch)
			...
			// assert: 加入干支之 date 可被正常 parse，但日干支本身会被忽略。
	...
	String_to_Date.parser.Chinese = function(date_string, minute_offset,
		...
		if (stem_branch
		// 确定真有 match 到干支。
		// 若没 match 到，IE 中为 ""，其他为 undefined。
	...
	function index_to_stem_branch(index) {
		// NaN | 0 → 0, 过大的数 | 0 可能成负数。
		//
		// 当 < 0: 需要修正对负小数之模数计算。
		// 原因出自上面计算负小数时，其实应采用 Math.floor()。
		// Math.floor(-1.1) → -2。
		// 但以 "-1.1 | 0", "-1.1 >> 0", "≈-1.1" 会 → -1。
		// 效能:
	...
	/**
	 * guess the year-stem-branch (岁次/年干支) of the date.
	 * 
	 ...
	 *            of year-stem-branch.
	 * 
	 * @returns {String} year-stem-branch (岁次/年干支)
	 */
	function guess_year_stem_branch(date_value, options) {
		...
		if (month < 2)
			...
			if ((date = date_value.getDate()) < 19 && month === 0) {
				...
				// 无法判别岁次：需要 include 'data.date.era'!
	...
	// ----------------------------------------------------------------------------------------------------------------------------------------------------------//
	// timevalue → {String}日期及时间表达式

	/**
	 * 计算大略的时间间隔，以适当的时间单位缩写简略显示。 count roughly duration, count date.<br />
	...
	function age_of(start, end, options) {
		...
		// 将数字四舍五入到指定的小数位数。 TODO: 把时间表示方式改为60进位。
		...
		if (diff) {
			...
			if (options.岁) {
				...
				// + 1: 一出生即虚岁一岁(YO, years old, "Y/O.")。之后过年长一岁。
				// else: 计算周岁（又称实岁、足岁）几岁。
				+ (options.岁 === '虚' ? 1 : 0);
	...
	age_of.get_new_year = function(year, 月, 日, era_key) {
		// 取平均值。因无法准确判别春节（农历正月初一）日期，此方法尚有误差！
	...
	function indicate_date_time(date, options) {
		...
		if (date_value_diff <= 3) {
			...
			if (message > recent_date_name.length) {
				// assert: (new Date) 于本月初，date 于上月底。
	...
	// ------------------------------------------
	// {String}日期及时间表达式 → {Natural}timevalue in milliseconds

	// https://en.wikipedia.org/wiki/ISO_8601#Durations
	var PATTERN_ISO_8601_durations = /^P(?:(\d+)Y)?(?:(\d+)M)?(?:(\d+)D)?T(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?$/,
	// 日期及时间 由小到大排列。
	PATTERN_time_units = [ [ '(?:ms|milliseconds?|毫秒|ミリ秒)', 1 ],
			[ '(?:s(?:ec)?|秒[钟钟]?)', 1000 ], [ '(?:m(?:in)?|分[钟钟]?)', 60 ],
			[ '(?:h(?:r|ours?)?|[个个]?小?[时时]|[个个]?时间)', 60 ],
			// T: for ISO 8601 Durations. e.g., 'P21DT3H'
			[ '(?:d(?:ays?|T)?|[日天])', 24 ],
			[ '(?:w(?:eeks?)?|周|[个个]?星期|个?礼拜|个?礼拜)', 7, true ],
			// 以下仅仅给出约略大小。 1 month: 30 days
			[ '(?:mon(?:ths?)?|[个个ヶ]?月)', 30 ],
			// https://en.wikipedia.org/wiki/Year#SI_prefix_multipliers
			// NIST SP811 和ISO 80000-3：2006 支持将符号a作为一年的时间单位。在英语中，还使用缩写y和yr。
	...
	// parse durations. period 是比较久的时间
	function parse_period(period) {
		var matched = period.match(parse_period.PATTERN);
		if (matched && (!/[日时]/.test(matched[2])
		// 预防 "10月22日夜7-8时"
		|| !/[时分秒\/]/.test(matched[2].match(/^(?:.*?)([年月日时分秒\/])/)[1]))) {
	...
	// "[^a-z]": 避免类似 "Neo-Babylonian" 被当作 period。
	//
	...
	// count=5: 1–6, ...
	/**
	 * 将`date`所属的月份以日子为单位平均切割成`count`个slice，回传`date`所在slice的首尾日期。
	 * 
	 * @param {Array}date
	 *            基准日期: [year, month, date]。assert: 必须为合理日期，不可输入[2001,1,32]之类。
	 * @param {Natural}count
	 *            将`date`所属的月份以日子为单位平均切割成`count`个slice。 assert: count<=31
	 * @param {Object}[options]
	 *            附加参数/设定选择性/特殊功能与选项
	 * 
	 * @returns `date`所在slice的首尾日期: {Array}[起始日期, 结束日期]
	 */
	...
	mode:
		...
		+2:显示时间至分,
		+1:显示时间至秒,
		+0:显示时间至毫秒(milliseconds)

		+32(4<<3):不显示日期,
		+24(3<<3):显示日期mm/dd,
		+16(2<<3):显示日期yyyy/mm,
		+8(1<<3):显示日期yyyy/mm/dd(星期),
		+0:显示日期yyyy/mm/dd
	...
	NOTE:
	在现有时制下要转换其他时区之时间成正确time:
	d=_其他时区之时间_;
	diff=其他时区之时差(例如 TW: UTC+8)
	 ...
	 * 
	 * @param date_value
	 *            要转换的 date, 值过小时当作时间, <0 转成当下时间
	 * @param {Number}
	 *            mode 要转换的 mode
	 * @param {Boolean}
	 *            zero_fill 对 0-9 是否补零
	 * @param {String}
	 ...
	 * @since 2010/4/16 10:37:30 重构(refactoring)
	 * 
	 * @requires to_fixed
	 * 
	...
	function format_date(date_value, mode, zero_fill, date_separator,
		...
		// time_mode > 4 && mode > 3: error mode: 没啥好显示的了
		...
		if (b === 'number' && date_value >= 0) {
			...
			if (!a && isNaN(a = format_date.UTC_offset)) {
				// input UTC 时之差距(milliseconds)
				// .getTimezoneOffset() is in minute.
				a = format_date.UTC_offset = ONE_MINUTE_LENGTH_VALUE
						* present_local_minute_offset;
			}

			// 值过小时当作时间: d < 90000000 ≈ 24*60*60*1000，判别为当天，只显示时间。不允许 d < 0！
		...
		// 引数がオブジェクトを要求してくる场合は instanceof 演算子を使用します
	...
	function detect_serial_pattern(items, options) {
		...
		Object.values(pattern_hash).forEach(function(lists) {
			...
			for (var index = 1; index < lists.length; index += 2) {
				...
				index_of_lists < lists.length; index_of_lists++) {
					...
					// 有相同 digits 的不应归为 serial。
					// e.g., "A1B1", "A1B2"
					// 仅有 A1B<b>1</b>, A1B<b>2</b> 可归为 serial。
					// A<b>1</b> ["A", "1", ... ] 应改为 ["A1", "", ... ]
		...
		Object.keys(pattern_hash).forEach(function(pattern) {
			...
			if (lists.length === 0) {
				// Nothing left after "有相同 digits 的不应归为 serial。".
	...
	/**
	 * 组织 period_list，以做后续检索。
	 * 
	 * @param {Object|Array}period_list
	 *            period list
	 * @param {Number|Function}start_index
	 *            start index. 若为 function，回传须转为 number，不应回传 boolean, false。
	 * @param {Number|Function}end_index
	 *            end index. 若为 function，回传须转为 number，不应回传 boolean, false。
	...
	function sort_periods(period_list, start_index, end_index, unit_length) {
		...
		// contemporary 若存在，应该按照 end 时间由前至后 sort。
		contemporary = [],
		//
