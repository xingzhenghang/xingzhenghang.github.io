 ...
 * @fileoverview 本档案包含了文字/字元编码用的 functions。文字コード変换ライブラリ
 * 
 * @example <code>
...
function module_code(library_namespace) {
	...
	HEX_BASE = 0x10,
	// 取得正式名称。预先设定以供load_code_map()使用。
	// [经过normalize_encoding_name(encoding)之前期处理的key]
	// = module/file name below data/encoding/
	code_of_alias = {
		big5 : 'Big5',
		gbk : 'GBK',
		// 因为以实用性为主，因此全部导向到扩张至最大的最新字码。
		gb2312 : 'GBK',
		eucjp : 'EUC-JP',
		shiftjis : 'Shift_JIS',
		sjis : 'Shift_JIS'
	...
	encoding.map.json规格书:包含map:
	{
		...
		// 这边的count表示中间有count个字元，分别是自char开始，unicode编码之后的序列。
	...
	to_multi的不能跨越to_single的范围。
	e.g.,
	{'A1FF':[0xFF,'abcde'],'A2FF':'12','A4B3':'~'}
	'A2FF','A4B3': 不在'A1FF'范围内: A1FF:a, A2FF:b, A3FF:c, ...
	实作将直接以+1的方式配入 convert_map 中，因此A2FF之第二组"2"将被配入A300!
		...
		for ( var key in map_data) {
			...
			if (typeof char_list === 'string') {
				char_list.chars(true).forEach(function(character) {
					...
					// 为了能辨识，无论哪种都还是得设定这个对应 to Unicode。
			...
			if (!('start_byte_code' in config)) {
				if (char_code === 0 && char_list[0] === '\u0000'
						&& char_list[1] > 0) {
					char_list.shift();
					// e.g., ['\0',2] → byte code < 2+1 的都能直接转string。
	...
	function String_to_code(encoding, options) {
		...
		// TODO: 对于不是以character分割，以及双/多位元却是0x0000的情况需要特别处理（这里会被当作0x00而非0x0000）!
	...
	if (library_namespace.platform.nodejs) {
		...
		// 把 Buffer 物件的内容当作是 encoding 编码，并解析成 {String}UTF-8 string。
	...
	if (false) {
		...
		CeL.character.load('Big-5', function() {
			console.assert('作辈' === Buffer.from('A740BDFA', 'hex').toString(
					'Big-5'));
			var text = '做基本检测。';
	...
	function code_array_to_String(encoding, options) {
		...
		byte_index = 0, length = this.length; byte_index < length; byte_index++) {
			...
			if (++code_index === max_byte) {
				// 自这次搜寻开始，无法找到能mapping的character。
				converted += UNKNOWN_CHARACTER;
				// rollback至自这次搜寻开始后的下一个byte。
	...
	function encode_URI_component(string, encoding) {
		...
		if (false) {
			// for pure Big5, no 香港增补字符集
			string = string.replace(/[―喰蔃瀞靔鼗弌𨱎蠏覩瑨牐]/g, function(char) {
