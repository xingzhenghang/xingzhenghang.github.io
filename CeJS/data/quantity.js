 ...
 * @fileoverview 本档案包含了 value of a quantity/物理量/度量衡（长度、时间、重量等）标准/国际单位/国际单位制词头
 *               单位换算用的 functions。<br />
 * 
 * TODO:<br />
 ...
 * 中国度量衡/中国古代货币 http://thdl.ntu.edu.tw/suzhou/
 * 
 * @since
 * @example <code>
...
CeL.assert([(new CeL.quantity('54cm')).multiple('8.5公尺').toString('繁体中文'),'4.59平方公尺'],'');
...
CeL.assert([(new CeL.quantity('500平方公尺')).convert_to('a').toString('繁体中文'),'5 公亩'],'');
...
function module_code(library_namespace) {
	...
	// 度量衡单位 : 数量级 (prefix exponent / integer powers of 10)
	// 度量衡单位 : [ 数量级, 单位数量级 (unit power) ]
	// e.g., m : 0 → m
	// e.g., m : 3 → km
	// e.g., m : [ , 2 ] → m^2
	// e.g., m : [ 3 , 2 ] → km^2
	function multiple_unit(quantity, unit_symbol, prefix_exponent, unit_power) {
		...
		if (unit_symbol in units) {
			...
			if (prefix_exponent !== array[0]) {
				...
				// 微调：尽量取到不会 overflow 或 underflow 的 prefix。
	...
	function Quantity(amount, units, options) {
		...
		// 数值/量值的大小 / number / amount/count of base unit /
		...
		if (options) {
			// 度量衡单位分类 type. e.g., 'length' (长度)
			if (options.type)
				this.type = options.type;

			// 有效数字/the appropriate number of significant
			// figures
	...
	// 同一个物理量的两种不同的单位之间，是靠著转换因子（conversion
	// factor）从一个单位转换到另一个单位。例如，1 in = 2.54
	// cm，注意到在这里「2.54 cm/in」是转换因子，不具有因次，其数值等于1。
	parse_factor('min=60s,hr=60min,a=100m2,Newton=kg*m/s2');

	// 常数/constants/factors
	var constants = {
		PI : Math.PI,
		c : 299792458
	};
