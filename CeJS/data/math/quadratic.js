 ...
 * @fileoverview
 * 本档案包含了二次无理数 (quadratic irrational, quadratic irquadratic, also known as a quadratic irquadraticity or quadratic surd) 的 functions。<br />
 * TODO: 可充作 Gaussian rational (简易 complex number)、Gaussian integer、Eisenstein integer、dual number、split-complex numbers。<br />
...
if (typeof CeL === 'function')
	CeL.run(
	{
		...
		code: function (library_namespace) {
			...
			//{Boolean|Undefined}最简, GCD(multiplier, integer, denominator) = 1
			...
			/**
			 * 任意大小、带正负号的有理数。quadratic irrational number instance.<br />
			 *
			 * @example
			 * <code>
			 * </code>
			...
			function sign(negative) {
				if (this[KEY_RADICAND].is_positive()) {
					...
					if (si * sm < 0)
						// KEY_MULTIPLIER, KEY_INTEGER 正负相异时须比较大小。
			...
			// TODO: https://www.alpertron.com.ar/METHODS.HTM Solve the equation: a x2 + b xy + c y2 + dx + ey + f = 0 圆锥曲线/二元二次方程
			...
			function solve_Pell(d, n, limit, return_Integer) {
				...
				// limit() return true if 到达界限 / reach the limit / out of valid range.
			...
			function assignment(radicand, multiplier, integer, denominator) {
				...
				//	为了允许 二元数/dual numbers，因此不对 radicand.is_0() 时作特殊处置，而当作 ε = √0。
			...
			// 倒数, multiplicative inverse or reciprocal
			function reciprocal() {
			...
			*/
			...
			function valueOf(precision) {
				// 2: (default base of Integer) ^ 2 > JavaScript 内定 precision.
