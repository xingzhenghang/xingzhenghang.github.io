 ...
 * @fileoverview 本档案包含了处理 source code/text 的 functions。
 * @since
 */
...
function module_code(library_namespace) {
	 ...
	 * 
	 * @class 处理 source code 的 functions
	 */
	var _// JSDT:_module_
	= function() {
		// null module constructor
	...
	/**
	 * 类似 sprintf，处理 escape sequence 字串之 function。
	 * 
	 ...
	 *            {Function}handler: 处理 source text (非 escape sequence) 之
	 *            function,<br />
	 *            {Function}escape_handler: 处理 escape sequence 之 function.<br /> }
	 * 
	...
	function parse_escape(string, options) {
		 ...
		 * default: 1.<br />
		 * 为处理不定长 escape sequence. 这里仅列出需要特别注意的。
		 * 
		 * @type {Object}
		 */
		e_l = {
		...
		/**
		 * handle function.<br />
		 * 处理 source text (非 escape sequence) 之 function。
		 * 
		 * @type {Function}
		 */
		handler = undefined,
	...
	/**
	 * 模仿样板字面值（Template literals）
	 * 
	 * TODO: extract_literals('${ ({$:1})["$"] }')
	 * 
	 * @param {String}template_string
	 *            样板字串（template strings）
	...
	function set_toString(Class, format_parser, special_condition) {
		...
		// 以指定 format 转换 Class 之内容成 string。
		var old_toString = Class.prototype.toString;
		// format 用途：i18n|不同领域、不同产业采用不同 format
	...
	// 将改变 status，不改变 controller, parameters.
		...
		for (; index < length; index++) {
			...
			if (is_controller(item)) {
				if (controller) {
					...
					// 不直接 clone，减少 copy 次数。
					controller = item;

			} else {
				// 连续的 controller 只有在必要时（最后一个），才处理。
				if (controller) {
