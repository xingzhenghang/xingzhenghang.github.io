 ...
 *       (TSV) data
 * @fileoverview 本档案包含了处理 CSV, TSV data 的 functions。 TODO: delimiter-separated
 *               values (DSV)
 * @since
 */
...
function module_code(library_namespace) {
	...
	/**
	 * default config/设定/选项/options/flag.
	 */
	var default_config = {
		/**
		...
		has_title : false,
		/**
		 * {Boolean} 是否将 [0] 设定为第一笔资料。若为 true，则 [0] 为 title。
		 */
		skip_title : false,
		...
		/**
		 * {Integer} 将每笔资料依 title column index 存成 Object。<br />
		 * 设定之后 table = {title_name: [row data]}
		 */
		to_Object : undefined,
		/**
		 * {Boolean} 如何存每笔资料。<br />
		 * select_column=true<br />
		 * 设定之后 data[row index] = {title_name: data}<br />
		 * TODO:<br />
		...
		// table[row index][column index] 将以 handle_array[column index] 预先处理。
		handle_array : undefined,

		// 处理 every row。row = handle_row(row, row_count);
		handle_row : undefined,

		// bool|'auto'
		/**
		 * {Boolean} 是否使用 text delimiter。<br />
		 * for to_CSV_String()，表示是否强制加上 text delimiter。
		 */
		no_text_qualifier : 'auto',
		/**
		 * {String} 栏位文字辨识符号。<br />
		 * Excel 之 Unicode 文字输出不包括 "'"。<br />
		 * //text_qualifier : '"\'',
		 */
		text_qualifier : '"',
		/**
		...
		field_delimiter : '\t,',
		/**
		 * {String} row delimiter (row 分隔符号) / record delimiters: usually only
		 * \n.<br />
	 ...
	 * 
	 * @since 2012/4/2 15:26:06 重写 parse_CSV()，功能加强。效能恐降低。
	 */
	function parse_CSV(data, config) {
		...
		if (select_column !== undefined && select_column !== false) {
			...
			if (Array.isArray(select_column)) {
				...
				// 既然指定 select_column，蕴含应当有 title。
				has_title = true;
			}
		}
		...
		if (!config('no_text_qualifier') && text_qualifier) {
			...
			// ["']
			'[', text_qualifier, ']',
			// |: 无 text qualifier 的部分。
			'|',
			// 无 text qualifier 的部分。
			'(',
			// [^\t,\n]*
			'[^', config('field_delimiter'), config('line_separator'), ']*',
			// ending of 无 text qualifier 的部分。
			')',
			// ending
			')');
		} else {
		...
		while (last_index_of_data < data_length
			...
			if (library_namespace.is_debug() && matched[1]) {
				...
				library_namespace
						.warn('parse_CSV: Cannot parse data (无法辨识的资料): ['
								+ matched[1] + ']');
			}
			...
			if (matched[auto_text_qualifier ? 4 : 3]) {
				...
				if (title_passed) {
					library_namespace.debug('纪录资料栏数 ' + column_count + '。', 4,
							'parse_CSV');
					...
					library_namespace.debug('初始化资料栏数纪录 ' + column_count + '。',
							4, 'parse_CSV');
				...
				// 因为 add_row() 要用到 title_passed，因此这边才设定。
				if (!title_passed)
					title_passed = row;
	...
	// 依照栏位标题, key 合并两资料表格. title_mapper 可选
	...
	/**
	 * 将 {Array|Object} 依设定转成 CSV text。<br />
	 * 
	 * @example <code>
	 ...
	 * @param {Object}config
	 *            自订设定/选项。 see to_CSV_String.config.
	 * 
	 ...
	 * 
	 * @since 2012/4/3 18:26:06 重写 to_CSV()，改名 to_CSV_String()，功能加强。效能恐降低。
	 */
	...
	/**
	 * parse CSV data to JSON 读入 CSV 档
	 * 
	 * @deprecated 废弃/弃用: use parse_CSV() instead.
	 * @param {String}
	 *            _t CSV text data
	 * @param {Boolean}
	...
	function old_parse_CSV(_t, doCheck, has_title) {
		...
		for (; _i <
		// 这里不加 _f.text_qualifier 可以 parse 更多状况
	 ...
	 * @see 可参考 JKL.ParseXML.CSV.prototype.parse_CSV 2007/11/4 15:49:4
	 * @deprecated 废弃: use parse_CSV() instead.
	 * @param FP
	 *            file path
	 * @param FD
	 *            field delimiter([,;: ]|\s+)
	...
	.
	/**
	 * @deprecated 废弃: use to_CSV_String() instead.<br />
	 *             config =
