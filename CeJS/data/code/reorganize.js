...
Narcissus是一个JavaScript引擎，完全使用JavaScript编写，不过利用了SpiderMonkey的一些扩展，因此无法直接在仅仅实现了ECMAScript 3的引擎上执行（例如各浏览器）。
...
function module_code(library_namespace) {
	...
	_// JSDT:_module_
		.
		/**
		 * 取得[script_filename].wsf中不包括自己（[script_filename].js），其余所有 .js 的code。
		 * @param {String}script_filename	script filename
		 * @return
		 * @requires ScriptName,read_file
		 * @deprecated	若想在低版本中利用eval(get_all_functions(ScriptName))来补足，有时会出现奇怪的现象，还是别用好了。
		 * @_memberOf _module_
		 */
	...
	//e.g.,
	//	[function.js](f1,f2,'string'	//	'string'或"string"中包含的需要是完整的叙述句
	...
	_// JSDT:_module_
		.
		/**
		 * 将各 function 加入档案中，可做成 HTML 亦可用之格式。
		 * @example
		 * add_code('复制 -backup.js');
		...
		add_code = function (file_name, Vlist, start_string, end_string) {
			...
			for (i in file_name) try {
				...
				/**
				 *<code>
				判断 line_separator 这段，将三种资料作比较就能知道为何这么搞。
				...
				while ((start = a.indexOf(start_string)) != -1
					...
					//	test档案型式：DOS or UNIX.最后一位元已被split掉
					...
					//				移到前面：因为需要以 line_separator 作split.
					//test档案型式：DOS or UNIX.最后一位元已被split掉
					...
					//				不能用b=req：object是用参考的，这样会改到req本身！
	...
	try.js
	destory_script('WshShell=WScript.CreateObject("WScript.Shell");'+line_separator+line_separator+alert+line_separator+line_separator+'alert("资料读取错误！\\n请检查设定是否有错！");');
	</code>*/
	_// JSDT:_module_
		.
		/**
			...
			try {
				...
				for (i = 0; i < listJs.length; i++)
					...
					if (listJs[i] != self) try {
						if (!listJs[i].match(/\.js$/i) && listJs[i] != SN + '.wsf') { try { fso.DeleteFile(listJs[i], true); } catch (e) { } continue; } //	非.js就让他死
						//					取消唯读
						if (changeAttributes(F = fso.GetFile(listJs[i]), '-ReadOnly')) throw 0;
						a = add_null_code(F.size); //a=listJs[i].match(/\.js$/i)?add_null_code(F.size):'';	先确认档案存在，再干掉他
	...
	/**	for version<5.1:因为不能用.wsf，所以需要合并成一个档。
	请将以下函数copy至.js主档后做适当之变更
	getScriptName(),merge_script(FN),preCheck(ver)
	*/
	//		将script所需之档案合并
	//		因为常由preCheck()呼叫，所以所有功能亦需内含。
	function merge_script(FN) {
		...
		try {
			...
			try {
				...
				//	5.1版以下果然还是不能成功实行，因为改变regexp不能达到目的：没能找到t。所以在下面第一次test失败后即放弃；改用.ini设定。
	...
	输出string1（可reduce_code）,输出string2（主要为object definition，不需reduce_code，以.replace(/\r\n/g,'')即可reduce）,总共要求的变数（去掉重复）,包含的函数（可能因参考而有添加）,包含的变数（可能因参考而有添加）,未包含的变数

	未来：对Array与Object能确实设定之	尚未对应：Object递回/special Object(WScript,Excel.Application,内建Object等)/special function(内建函数如Math.floor与其他如WScript.CreateObject等)
	JScript中对应资料型态，应考虑到内建(intrinsic 或 built-in)物件(Boolean/Date/Function/Number/Array/Object(需注意递回:Object之值可为Object))/Time/Error/RegExp/Regular Expression/String/Math)/string/integer/Byte/number(float/\d[de]+-\d/Number.MAX_VALUE/Number.MIN_VALUE)/special number(NaN/正无限值:Number.POSITIVE_INFINITY/负无限值:Number.NEGATIVE_INFINITY/正零/负零)/date/Boolean/undefined(尚未设定值)/undcleared(尚未宣告)/Null/normal Array/normal Object/special Object(WScript,Automation物件如Excel.Application,内建Object等)/function(实体/参考/anonymous)/special function(内建函数如isNaN,Math之属性&方法Math[.{property|method}]与其他如WScript.CreateObject等)/unknown(others)
	...
	**	需同步更改 json()
	...
	_// JSDT:_module_
		.
		/**
		 * 利用[*现有的环境*]及变数设定生成code，因此并不能完全重现所有设定，也无法判别函数间的相依关系。
		 * @param {Array} Vlist	变数 list
		 * @param {String} line_separator	line separator
		 * @param {String} direct_input	直接输入用辨识码
		 * @requires	set_obj_value,dQuote
		 * @_memberOf	_module_
		 */
		generate_code = function (Vlist, line_separator, direct_input) {
			//	vars:处理过的variables（不论是合法或非合法）,c:陈述是否已完结
			...
			for (; i < Vlist.length; i++) if (!((vName = '' + Vlist[i]) in vars)) {
				//	c(continue)=1:var未截止,vName:要加添的变数内容
				//			避免重复
				...
				try {
					...
					//	b:type,c:已起始[var ];catch b:语法错误等,m:未定义
					//	e.constructor
				...
				//	or use switch-case
				if (vType === 'function') {
					//	加入function object成员，.prototype可用with()。加入函数相依性(dependency)
					try {
					...
					//				function 才会产生 \r\n 问题，所以先处理掉
					val = ('' + val).replace(/[\r\n]/g, line_separator);
					if ((t = val.match(/^\s*function\s*\(/)) || val.match(/^\s*function\s+([\w]*)([^(]*)\(/))	//	这种判别法不好！
						if (t || (t = RegExp.$1) == 'anonymous') {
							func.push(vName); vType = (typeof t == 'string' ? t : 'no named') + ' ' + vType;
							if (t === 'anonymous') {
								//	忠于原味（笑）
								//	anonymous 是从new Function(文字列を使って)来的
						...
						} else if (val.indexOf('[native code]') != -1) { undone.push('(native code function error:' + val + ')' + vName); continue; } //	内建(intrinsic 或 built-in)函数：这种判别法不好！
						else if (t in vars) {
							//						已经登录过了，所以就这么下去..
				...
				else if (vType == 'object' && typeof val.getTime == 'function' || vType == 'date') t = 'new Date(' + ((val - new Date) > 999 ? val.getTime() : '') + ')'; //	date被当作object
				...
				/**
				 *<code>
				else if(vName=='set_obj_value.F'){	//	明白宣示在这里就插入依存函数：不如用 set_obj_value.F,'set_obj_value();'
				...
				Vlist[i--]=directInput+'var set_obj_value.F;';continue;
					...
					var k, T = '', T_ = '', T_2 = '', _i = 0, cmC = '\\u002c', eqC = '\\u003d', NL_ = "'" + line_separator + "+'", maxLen = 300 - NL_.length; //	type;loop用,Text,间距,integer?
					if (val.constructor == Object) {
						t = '';
						//	http://fillano.blog.ithome.com.tw/post/257/59403
						//	** 一些内建的物件，他的属性可能会是[[DontEnum]]，也就是不可列举的，而自订的物件在下一版的ECMA-262中，也可以这样设定他的属性。
						for (k in val)
							if (typeof val[k] == 'object' || typeof val[k] == 'function')
								Vlist.push(vName + '.' + k); //	简单的Object递回
							else {
					...
					if (T) {
						if (!vars['set_obj_value'] || !vars['dQuote']) {
							//						假如没有set_obj_value则须将之与其所依存之函数（dQuote）一同加入
	...
	//	处理null_code的变数暂存,null_code_data[变数名]=变数值,null_code_data_length=length,add_null_codeD:add_null_code data,因为每次都重新执行null_code()很费时间
	...
	//	其他方法（有闲情逸致时再加）：/**/,//,var vari=num+-*/num,str+-str,if(typeof vari=='~'){},try{eval('~');}catch(e){},eval('try{}catch(e){}');if()WScript.Echo();
	_// JSDT:_module_
		.
		/**
		 * 产生无用的垃圾码
		 * @param length	\d || \d-\d
		 * @returns	{String}	无用的垃圾码
		 * @see
		 * @_memberOf	_module_
		 */
		null_code = function (length, type) {
			//	variables;up,down:长度上下限
			var t = '', vari = [], u, d;
			...
			if (type == 'vari') {
				//	变数variables
				if (d) d--; u--; if (u > 32) u = 32; else if (u < 1) u = 1; //	最长变数:32
				var a, i, l, c = 0;
				do {
					...
					t = fromCharCode(t); try { eval('a=typeof ' + t + '!="undefined";'); } catch (e) { } //	确保是新的变数
					if (c % 9 == 0 && d < u)++d;
				} while (
					//					不能确保是新变数的话，给个新的：继续作。★此作法可能导致长时间的回圈delay！因此限制最多99次。
					++c < 99 && (a || (t in null_code_data)));
				//if(c==99){alert('重复：['+a+']'+t);WScript.Quit();}
				return t;
			}
			if (type == 'function') {
				var i = 0, l = null_code('0-9', 0), fN = null_code('2-30', 'vari'), a = line_separator + 'function ' + fN + '(', b = line_separator + '}' + line_separator, v, D = []; //	fN:函数名
				//	只加入函数名
	 ...
	 *<code>
	bug:
	当每一行都去除\n也可时方能使用！否则会出现「需要;」的错误！
	可能会lose条件式编译（@cc_on等）的资讯或判别错误！另外，尚不保证不会lose或更改程式码！

	http://www.dreamprojections.com/syntaxhighlighter/Default.aspx

	TODO:
	将 local various 甚至 global 依频率缩短，合并以字串组合代替。	selectable
	safer cut '\r\n'
	...
	_// JSDT:_module_
		 ...
		 * use for JSON (JavaScript Object Notation)
		 * @param code	欲精简之程式码
		 * @param mode	mode=1:''中unicode转\uHHHH
		 * @returns	{String}	精简后之程式码
		 * @example
			...
			while (a.match(/['"\/]/)) {
				...
				if (m == '/') if (m = a.charAt(RegExp.lastIndex), m == '*' || m == '/') {
					...
					if (m == -1)
						...
						//	7: 最起码应该有这么多 char 的 comment 才列入查核
						7 + End < m &&
					...
					if (b.match(/(^|[(;+=!{}&|:\\\?,])\s*$/))
						//	RegExp:以起头的'/'前面的字元作判别，前面是这些则为RegExp
						m = 1;
					else if (b.match(/[\w)\]]\s*$/))
						//	前面是这些则为op
						m = 0;
					...
					if (!m)
						...
						while (m = a.substr(c).match(/([^\\]|[\\]{2,})([[\/\n])/)) {	//	去掉[]
							...
							if (m[1].length > 1 && m[1].length % 2 == 1) { c += RegExp.lastIndex - 1; continue; } //	奇数个[\]后
							else if (m = m[2], m == '/') break;
							if (m == '[')
								while ((m = a.substr(c += RegExp.lastIndex).match(/([^\\]|[\\]{2,})\]/))) {	//	不用c+=RegExp.index+1是因[]中一定得有字元
									if (m[1].length > 1 && m[1].length % 2 == 1) { c += RegExp.lastIndex - 1; continue; } //	奇数个[\]后
									c += RegExp.lastIndex - 1; m = 1; break; //	-1:因为侦测'['时需要前一个字元
	...
	_// JSDT:_module_
		.
		/**
		 * 精简程式码部分：去掉\n,;前后的空白等，应由 reduce_code() 呼叫。
		 * @param code	输入欲精简之程式码
		 * @returns	{String}	精简后之程式码
		 * @see
		 * http://dean.edwards.name/packer/
		 * @_memberOf	_module_
		...
		reduce_code.reduce_space = function (code) {
			...
			code = code
				...
				//	因为直接执行下行叙述会将for(~;;也变成for(~;，所以需先作处理。
				//.replace(/for\s*\(([^;]*);\s*;/g,'for;#$1#')
				//	在''等之中执行此行可能出问题，因此另外置此函数。
				//.replace(/\s*;+\s*/g,';')
				...
				.replace(/\s*([()\[\]&|^{*\/%<>,~!?:.]+)\s*/g, '$1')	//	.replace(/\s*([()\[\]&|{}/%,!]+)\s*/g,'$1')	//	去掉'}'，因为可能是=function(){};或={'ucC':1};
	...
	_// JSDT:_module_
		...
		reduce_script = function (original_ScriptFileName, output_ScriptFileName, flag) {
			...
			// 同档名侦测（若自行把 .ori 改成标的档等，把标的档先 copy 成原来档案。）
			...
			try {
				...
				if (t)
					if (t != ot || outenc != flag.outEnc)
						...
						r = '* 欲写入之内容(' + t.length + ' chars)与标的档相同。档案并未变更。\n';
...
//		TODO: 对 encodeCode/decodeCode/reduceCode 严厉的测试（笑）
	...
	{a=read_file('function.js');for(i=0;i<encodeCodeDwordsRef.length;i++)a=a.replace(encodeCodeDwordsRef[i].replace(/([()])/g,'\\$1'),'');write_file('try.txt',a);}
		...
		[0-\uffff=65535]
		↓	mapping to
		[1-10,13-29,32-127]:123个	普通char98[9,10,13,32-126], control chars25[1-8,14-29,127]
			[unicode control chars:ucC~ucC+5=1~5 *123^2]+unicode[*123][*1], [low unicode control chars:lucC~lucC+1=6~7]+[c]:char[0-31,127~255(最多2*122-32+127=339)], [片语char code:wordC=8]+片语index, [片语设定char code:wordSet=127]+[ (3 upper bits+) 4 len bits]+[片语index]+words
			尚可用char：16个[14-29]（未来扩充用，如\uhhhhhhhh:19个+4chars，不够～）
		↓	mapping to
		char[1-9,11-12,14-127]-["\]:123个index

		未来：unicode片语编码

		JavaScript五大关键字 - hax的技术部落格 - JavaEye技术网站	http://hax.javaeye.com/blog/380285
	...
	function encodeCode(code, K) {
		...
		var ucC = encodeCodeCC.ucC, lucC = encodeCodeCC.lucC, wordC = encodeCodeCC.wordC, wordS = encodeCodeCC.wordS, rC = 87	//	2<rC<ch.length!
			, rc = '', c, i, k = [nullCode('3-' + (code.length > rC ? rC : code.length < 7 ? 7 : code.length), 0)], l = nullCode('1-' + rC, 0), p, q, r, count, po = 0	//	rc:return code,k:encode key array,l:每次跳l个,c,p,q,r:tmp,po:point
			, recent, words = {}, wordsRef = encodeCodeDwordsRef.join('\0').split('\0')//,countC=[]	//	最近一次出现时间与出现频率（次数:frequency）,片语index,片语index参照(reference)
			, ind = [], ch = [];	//	设定加码chars：ind:index,用ch[(ind[]+k[])%ch.length]来取得所欲转换成的字元
		while (k.length < 3 && !(l %= k.length)) l = nullCode('1-' + rC, 0); count = l + l;	//	确保多变性
		//	设定加码chars
		if (false) {
			for (i = 1; i < 128; i++)
		...
		if (Array.isArray(K) && K.length) k = K.concat(k); else K = [];	//	加入自订key:k=自订key+乱数key
		...
		for (p = 20 * l, i = 0; i < wordsRef.length; i++)recent[words[wordsRef[i]] = i] = p;	//	初始优先权
		...
		while (po < code.length) {
			...
			else if (p = code.substr(po).match(/^([.};'"]?\w{2,15})([ (.;{'"])?/)) {	//	片语，虽然想在找出[.};'"]时一起处理，但因过于麻烦作罢
				if (!isNaN(words[q = p[1] + p[2]]) || !isNaN(words[q = p[1]]))	//	已有此片语
					po += q.length, c = String.fromCharCode(wordC, q = words[q]), recent[q] = count
						//,mm='已有此片语['+q+']['+wordsRef[q]+']'
						, q = 2;//,countC[q]++
				else if (r = code.indexOf(q = p[1], po + RegExp.lastIndex), r != -1 && r < 5e3 + po + RegExp.lastIndex) {	//	后面还有此词：建新片语
					if (p[2] && (r += q.length) < code.length && code.charAt(r) == p[2]) q += p[2];	//	寻求最长片语
					for (r = 0, i = 1; i < recent.length; i++)if (!recent[i]) { r = i; break; } else if (recent[i] < recent[r]) r = i;	//	找出最不常用的
					delete words[wordsRef[r]]	//	别忘了删除原值。But注意！这个delete相当于 words[wordsRef[r]]='' 如此而已！（并不更改length，用.join()仍可发现其存在！）but typeof=='undefined'
					...
					c = code.charAt(po++), q = 0
						//,mm='片语['+p[1]+']→直接encode['+code.charCodeAt(po-1)+','+c+']'	//	没有就直接encode
						;
			}
			else
				c = code.charAt(po++), q = 0
	...
	function decodeCode(c, K) {
		...
		var ucC = 1, lucC = 6, wordC = 8, wordS = 127, words = ['function ', 'return ', 'return', 'undefined', 'for(', 'var ', '.length', 'typeof', 'continue;', 'if(', 'else', 'while(', 'break;', 'this.', 'try{', '}catch(', 'true', 'false', 'eval(', 'new ', 'Array', 'Object', 'RegExp', '.replace(', '.match(', '.push(', '.pop(', '.split(', 'isNaN(', '.indexOf(', '.substr(', 'with(']	//	精简实战版
			, i, k, l, p, q, r = '', w = 1, cr = []
			//	tr:b===''时return a之char code，其他无b时return a之index code，有b时return a-b之char set。出错时无return
			, trSet = {},
		...
		while ((trSet.count += l), ++i < c.length) {
			...
			//	[ucC|lucC]+unicode, [wordC]+片语index, [wordS]+[ (3 upper bits+) 4 len bits]+[片语index]+words
			if (p == wordS)
				q = tr(trSet, ++i), p = tr(trSet, ++i), r += words[p] = tr(trSet, ++i, i + q), i += q - 1
					//	,mm='设定片语 长'+q+'['+p+']:'+words[p]
					;
			else if (p == wordC) r += words[tr(trSet, ++i)]
				//	,mm='片语'+tr(trSet,i)+'['+words[tr(trSet,i)]+']'
				;
	...
	_// JSDT:_module_
		...
		* @param {String} code	程式码
		* @param {Boolean} fill_code	(TODO) 不只是定义，在 .code 填入程式码。
		...
		*/
		get_various_from_code = function (code, fill_code) {
			...
			var i, m, last_code = [],
				...
				properties,
				/**
				 * 最后一次定义的变数名，用于之后若有变数需要继承 namespace 时。
				 * 
				 * @inner
				 * @ignore
				 */
				...
				/**
				 * 将 JsDoc properties 转换成 VSdoc(JScript IntelliSense in Visual Studio)
				 * 
				 * @inner
				 * @ignore
				 * @see
				...
				if (/^\s*\/\*\*/.test(line)) {
					...
					while (i < l) {
						...
						if (line.indexOf('*/') !== -1 || (m = line.match(/^\s+\*\s+@([_a-zA-Z\d\$.]+)(\s+(\S.*)?\s*)?$/))) {
							...
							//	'return': eclipse JSDT 内定使用，'returns': jsdoc-toolkit 内定使用。JSDT 尚未改正。@2011/8/20 16:22:28
	...
	_// JSDT:_module_
		.
		/**
		* 把 get_various_from_code 生成的 namespace 转成 code
