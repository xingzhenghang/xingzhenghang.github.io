 ...
 * @fileoverview 本档案包含了 new ECMAScript standard 标准已规定，但先前版本未具备的内建物件功能；以及相容性 test
 *               专用的 functions。<br />
 ...
 * 
 * 注意: 本档案可能会被省略执行，因此不应有标准之外的设定，应将之放置于 data.native。
 * 
...
function module_code(library_namespace) {
	 ...
	 * 
	 * @class 标准已规定，但先前版本未具备的功能；以及相容性 test 专用的 functions。
	 */
	var _// JSDT:_module_
	= function() {
		// null module constructor
	 ...
	 * 未发现之index。未找到时的 index。基本上与程式码设计合一，仅表示名义，不可更改。(=== -1)
	 * 
	 * @type {Number}
	 * @constant
	 */
	...
	// java test: 加了下面这段在 FF3 会召唤出 java! IE中没有java object.
	...
	function check_version(version) {
		...
		if (typeof WScript !== 'undefined' && WScript.Version < version) {
			...
			var Locale = library_namespace.env.locale, promptTitle = Locale == 0x411 ? 'アップグレードしませんか？'
					: '请升级', promptC = Locale == 0x411 ? "今使ってる "
					+ WScript.Name
					+ " のバージョンは古过ぎるから、\nMicrosoft Windows スクリプト テクノロジ Web サイトより\nバージョン "
					+ WScript.Version + " から " + version + " 以上にアップグレードしましょう。"
					: "正使用的 " + WScript.Name
							+ " 版本过旧，\n请至 Microsoft Windows 网站将版本由 "
							+ WScript.Version + " 升级到 " + version + " 以上。", url = /* Locale==0x411? */"http://www.microsoft.com/japan/developer/scripting/default.htm";
	...
	set_method(globalThis, {
		...
		isNaN : function(value) {
			...
			/**
			 * 变数可以与其本身比较。如果比较结果不相等，则它会是 NaN。原因是 NaN 是唯一与其本身不相等的值。
			 * 
	...
	// IE 8, JScript 5.8.23141 中，DOM 可能没有 .hasOwnProperty()。
	...
	// Object.keys(): get Object keys, 列出对象中所有可以枚举的属性 (Enumerable Only)
	// https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Object/keys
	// 可用来防止 .prototype 带来之 properties。e.g., @ IE
	// cf. Object.getOwnPropertyNames() 会列出对象中所有可枚举以及不可枚举的属性 (enumerable or
	// non-enumerable)
	...
	set_method(Object, {
		...
		isSealed : function isSealed(object) {
			// 若欲更严谨些，可依照经验法则，避开无法测试的 write-only 物件、或一改变就会产生后续影响的 object，
			// 并对 object 作变更测试，确保 object 真的具有不可变更之性质。
			return false;
		},
		...
		// 会列出对象中所有可枚举以及不可枚举的属性 (enumerable or non-enumerable)
		// 列出的比 Object.keys() 多
		getOwnPropertyNames : Object_keys
	});

	// 会造成几乎每个使用 for(.. in Object)，而不是使用 Object.keys() 的，都出现问题。
	if (false)
	...
	// 稀疏矩阵 (sparse matrix) 用的 Array.prototype.some()
	// 要到 index > 1e7 比较感觉得出来。
	// e.g.,
	...
	// 稀疏矩阵 (sparse matrix) 用的 Array.prototype.every()
	// 要到 index > 1e7 比较感觉得出来。
	...
	// 测试 for ( ... in array ) 时，会依顺序提供 index。
	...
	function copyWithin(target, start, end) {
		...
		if (target < start || end <= target)
			...
			if (length < target + start) {
				...
				target += start;
			// 反向（后→前） copy，确保 copy from 不曾被本操作污染过。
			while (start < end)
	...
	set_method(Array.prototype, {
		...
		/**
		 * 对于旧版没有 Array.push() 等函数时之判别及处置。<br />
		 * 不能用t=this.valueOf(); ... this.push(t);
		 */
		// Array.prototype.push()
		push : function push() {
			var i = 0, l = arguments.length, w = this.length;
			// 在 FF3 仅用 this[this.length]=o; 效率略好于 Array.push()，
			// 但 Chrome 6 相反。
			for (; i < l; i++)
				this[w++] = arguments[i];
			return w;
	...
	// calculatable
	/**
	 * 取得最小最低可做除法计算数值。回传值为边界，已不可再做操作。但可做乘法操作。
	 * 
	 * @param [base_integral]
	 * @returns {Number}
	 */
	...
	// calculatable
	/**
	 * 取得最大可做加法计算数值。回传值为边界，已不可再做加法操作。但可做减法操作。
	 * 
	 * @param [base_integral]
	 * @returns {Integer}
	 */
	...
	function hypot(value_1, value_2, value_3) {
		...
		if (MAX < Math_hypot_up_boundary
				...
				&& Math_hypot_down_boundary < Math.min(value_1, value_2,
						value_3)
				// avoid overflow, minimise rounding errors (预防本该为整数的出现小数).
	...
	set_method(Math, {
		...
		log10 : function log10(value) {
			// 采用 /Math.LN10 会造成 Math.log10(1e-12) !== -12
	...
	// 注意: 因为要设定额外功能，RegExp.prototype.flags 放在 data.native
	 ...
	 * 
	 * @since 2010/1/1 19:03:40 2015/1/28 17:43:0 refactoring 重构
	 * 
	 * @example <code>
	...
	// [ "", "" ] @ IE8, [ "", "_", "" ] @ firefox 38+
	'_'.split(/(_)/)[1] !== '_'
	// 理论上 '.'.split(/\./).length 应该是 2，但 IE 5–8 中却为 0!
	// 用 .split('.') 倒是 OK.
	// || '.'.split(/\./).length === 0
	)
		(function() {
			var native_String_split =
			// 增加可以管控与回复的手段，预防有时需要回到原有行为。
			...
			(String.prototype.split = function(separator, limit) {
				...
				// 不改变 separator 本身，加上 .global flag。
				...
				while (result.length < limit && last_index < this.length) {
					...
					if (last_index < matched.index || matched[0]) {
						...
						// IE 中，匹配到 null string 时，lastIndex 会自动 +1。
	...
	function repeat(count) {
		...
		// 此法较 (new Array( count + 1 ).join(this)) 稍快。
		while (true) {
	...
	/**
	 * 去除首尾空白。去除前后空白。去头去尾。去掉 string 前后 space.
	 * 
	 * @param {String}
	 *            string input string
	 ...
	 * 
	 * parse:<code>
	// 字串的前后记得要加上刮号 ()，这是用来告知 Javascript Interpreter 这是个物件描述，不是要执行的 statement。
	data=eval('('+data+')');
	eval('data='+data);
	</code>
	 * 
	 ...
	 * 
	 * BUG:<br />
	 * function 之名称被清除掉了，这可能会产生问题！<code>
	(function(){
		var f=function(){...};
		f.a=...;
		f.b=...;
	...
	function json(val, name, type) {
		...
		case 'function':
			// 加入function
			// object成员，.prototype可用with()。加入函数相依性(dependency)
			var toS, f;
			// 这在多执行绪有机会出问题！
		...
		case 'object':
			try {
				...
				if (c == Date || vType == 'date') {
					// 与 now 相隔过短(<1e7, 约3h)视为 now。
					// 但若是 new Date()+3 之类的会出现误差！
					addE('new Date'
					// date 被当作 object
	...
	function JSON_stringify(value, replacer, space) {
		...
		function stringify(object, /* 累积的空格 */_spaces, key, obj) {
	...
	console.log([s!==q,s,q])
	// s: resolved, q: rejected. then方法执行完会另外产生一个新的 Promise 物件。
	...
	// setImmediate() 会调度宏微任务而不是微任务，可能导致不一样的调度结果。
	...
	// KEY_DEPEND_ON = KEY_VALUE; 因为没有冲突，可重复利用 KEY_VALUE。
	// 应对: @see TriggerPromiseReactions(promise)
	...
	function Promise(executor) {
		...
		// 因为本promise搁置而搁置的{Promise} or 函数。
	...
	// .then()会另外产生一个新的promise物件。 NewPromiseCapability()
	...
	Promise.resolve = function resolve(result) {
		...
		if (IsPromise(result) && result[KEY_STATE] !== PromiseState_pending) {
			// Promise.resolve() 基本上尽可能早点得到一个确定的 Promise 物件，并且不会改变传入的引数。
			// copy result → promise
			promise[KEY_STATE] = result[KEY_STATE];
			promise[KEY_VALUE] = result[KEY_VALUE];
	...
	// Promises/A+并没有关于Promise.reject或Promise.resolve的定义，它们是ES6 Promise标准中的实作。
	...
	// 如果参数中某个promise解决或拒绝，返回的promise就会解决或拒绝。
