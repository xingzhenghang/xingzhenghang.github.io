...
本计划目标是建立一个简单上手的 JavaScript 程式库架构。<!-- toolkit -->

## Web page usage / browser 运行方式
<!--
...
## Node.js usage / node.js 运行方式

### Node.js lazy installation. Node.js 环境最新版本懒人配置法
<code>[npm](https://www.npmjs.com/package/cejs)</code> 安装的可能不是最新版本，这里示范的是最新版本的安装方法。

不啰嗦，已经做过的步骤可以跳过：
1. 请先安装 [node.js](https://nodejs.org/) 与 [7-Zip](https://en.wikipedia.org/wiki/7-Zip)
2. 下载 GitHub repository 安装档 [GitHub.updater.node.js](https://raw.githubusercontent.com/kanasimi/gh-updater/master/GitHub.updater.node.js)
3. 在[命令行界面](https://zh.wikipedia.org/wiki/%E5%91%BD%E4%BB%A4%E8%A1%8C%E7%95%8C%E9%9D%A2)下，进到 GitHub repository 安装档(`GitHub.updater.node.js`)所在的目录，执行命令以下载 CeJS 程式库：

   ```bash
   node GitHub.updater.node.js
   ```

4. CeJS 程式库应该已经解压缩，并且放在安装档所在目录的 CeJS-master 目录下，可以开始试用：
...
5. 每次要更新到最新 CeJS 程式库时，只要重新执行一次 GitHub repository 安装档即可。

### Installation via npm 安装
<!-- NodeICO badges -->
[![NPM](https://nodei.co/npm/cejs.png)](https://nodei.co/npm/cejs/)
<!-- [![NPM](https://nodei.co/npm-dl/cejs.png)](https://nodei.co/npm/cejs/) -->

1. First, go to [nodejs.org](https://nodejs.org/), download the runtime environment and [install the node.js package](https://nodejs.org/en/download/package-manager/). 请先安装 [node.js](https://nodejs.org/)。
2. Then, install the CeJS library: 接著安装 CeJS library:
   ```bash
   $ npm install cejs
   ```
* 请注意：采用 `npm` 安装的可能不是最新版，尚未加入最新功能。建议下载最新版本压缩档，解开后配置；而不是直接执行 `npm install` 安装旧版的程式库。

### Installation via GitHub latest version 一般正常安装方法
鉴于更新频繁，有些功能可能最新版本才具备；若是执行的程式采用了新功能，将会发生严重错误。此时您可直接到 GitHub 下载最新版本压缩档，解开后配置。
...
* 由于频宽不足，加上载入时须做初始化，本功能示范页面在载入时得稍微等一下。
...
* [GitHub Pages](https://kanasimi.github.io/CeJS/) 在线演示。

## Requirements and dependencies
* 本 library 须使用新一点的浏览器/执行环境。
* 本 library 将使用到 global 变数 ```CeL```。

## Concepts 模组概念
详细请参阅[概念介绍页](http://lyrics.meicho.com.tw/reviews/review.pl?seg=CeJS)。
