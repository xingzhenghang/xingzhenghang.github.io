/**
 * @name CeL function for 繁简中文字词汇转换。
 * 
 * TODO:<br />
 * 在量大的时候，此方法速度颇慢。 Using Map()<br />
 * words conversion
 * 
 * @fileoverview 本档案包含了繁体/简体中文转换的 functions。
 * @example <code>

// 在非 Windows 平台上避免 fatal 错误。
...
function module_code(library_namespace) {
	...
	// using BYVoid / OpenCC 开放中文转换 (Open Chinese Convert) table.
	...
	function Converter_initialization(options) {
		...
		this.files.map(function(file_list) {
			var _options = {
				...
				// 在开始转换之后就不会再修改辞典档，因此可移除 .pair_Map。
		...
		if (this.corrections) {
			...
			// this.conversions: 手动修正表。提供自行更改的功能。
	...
	function convert_text(text, options) {
		...
		if (!options) {
			...
			// 输入单一词汇时使用，以期加快速度...可惜没有。
			// node.js: 直接开 `conversion.convert(text)` 速度相同，且还包含
			// .special_keys_Map 的转换，较完整。
			for_each_conversion = function(_text, conversion) {
				var convert_to = conversion.get_value(_text);
	...
	Converter.options = {
		CN_to_TW : {
			...
			// 以 generate_additional_table.js 合并新同文堂和 ConvertZZ 的辞典档。
			...
			// ** 下面的是上面词汇与单字转换后的再转换。
			[ 'TWPhrasesIT',
			// ↑ TWPhrasesIT.txt 有许多常用词汇，在 corrections_to_TW.txt 取消。
			'TWPhrasesName', 'TWPhrasesOther',
			// 若要筛选或增减 conversion files，可参考范例：
