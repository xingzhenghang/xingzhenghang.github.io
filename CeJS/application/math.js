 ...
 * @fileoverview
 * 本档案包含了生成 math application 的 functions。
 * @since	2014/10/3
 * @example
 * <code>
...
// 中式短除法(Chinese short division)并非 short division.
...
/**
 * 以 MathML 表现数学运算式。<br />
 * 将 HTML 中 &lt;math>expression&lt;/math> 皆转为 MathML。<br />
...
<math>资本收入=资本收益率×资本</math><hr />
<math>资本收入/国民年收入=资本收益率×(资本存量/国民年收入)</math><hr />
<math>
1/2+(1+2)/(2+3%)+((2+3))/((3+4))
</math><hr />
<math>
	...
	for (; i < length; i++) {
		...
		if (!_.support_MathML) {
			if (!node.title && node.getAttribute(convert_MathML.default_attribute)) {
				// useless... Chrome 不会像 <span> 一般显示 .title。
		...
		if (structure) {
			// 当可以使用 <math> 时，把原先展示的内容物 .childNodes 转而摆到 .title 去。
			// 使用例： <math alt="e^(-)" title="e-">电子</math>, <math alt="H_2 O" title="水 H2O">水分子</math>

			// 避免覆盖原先的 .title。.title 通常摆更详细的资讯。
...
convert_MathML.parse = function(text, queue) {
	...
	// TODO: &InvisibleTimes; 用于表示乘法运算中被省略的乘号。
...
convert_MathML.parse_scalar = function(text, no_MathML) {
	...
	// a-zA-Z: normal variable. 变量
	// \u0370-\u03ff: mathematical constant. 数学常数/希腊字母变量. e.g., π
	// \u2E80-\u30000: Unihan variable
...
// 将 convert_MathML.parse() 之结果，reduce 成所须的格式。
convert_MathML.reduce = function(structure, node, handler) {
	...
	&& structure.length === 2 && Array.isArray(structure[1])
		...
		if (structure[1].every(function(operand, index) {
			...
			if (library_namespace.is_debug()) {
				library_namespace.debug('array 型态' + (operand[0] === structure[0] ? '相同' : '不同'));
				library_namespace.debug(Array.isArray(operand[1]) && operand[1][0] === ',' ? 'operand 为 array' : 'operand 为 ' + (Array.isArray(operand[1]) ? operand[1][0] : operand[1]));
				if (matrix[0])
