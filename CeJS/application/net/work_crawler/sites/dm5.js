 ...
 * 
 * @fileoverview 本档案包含了解析并处理、批量下载 动漫屋网/漫画人 平台 的工具。
 * 
 * 由于 动漫屋网/漫画人 系列网站下载机制较复杂，下载图片功能为独立撰写出来，不支援 achive_images 功能。
 * 
 * <code>
...
function module_code(library_namespace) {
	...
	var default_configuration = {
		...
		// 有些漫画作品分区分单行本、章节与外传，当章节数量改变、添加新章节时就需要重新检查。
		// 当有多个分部的时候才重新检查。
		...
		parse_search_result : function(html, get_label) {
			...
			if (html.includes('<div class="mh-item">')) {
				...
				throw new Error(this.id + ': 网站改版？无法解析搜寻结果！');
			}
		...
		parse_work_data : function(html, get_label, extract_work_data) {
			...
			if (matched) {
				...
				throw new Error(this.id + ': 网站改版？无法解析作品资讯！');
			}
			...
			<span class="s">最新<span>&nbsp;<a href="/m853381/" title="山海逆战 第405回" style="color:#ff3f60;" target="_blank">第405回 神封不动（下）</a>&nbsp;前天 09:05</span></span>
			 </code>
			 */
			/<a [^<>]*?title="([^<>"]+)"[^<>]*>.+?<\/a>(?:(.+?)<\/span>)?/);
			if (matched) {
			...
			// 原先给tohomh.js用的。但是2019/7/7发现，无论哪个作品都没有这个标示了。
			if (false) {
				matched = text.between('最新').match(
						/<a [^<>]*?title="([^<>"]+)"/);
				if (matched) {
		...
		get_chapter_list : function(work_data, html, get_label) {
			...
			if (!is_dm5 && html.includes(' id="detail-list-select-2')) {
				// tohomh.js, ikmhw.js: 新→旧 ' id="detail-list-select-2' 旧→新
		...
		: function(XMLHttp, work_data, callback, chapter_NO) {
			/**
			 * 之前已经取得过图片资讯。但需要处理下载目录改变的情况。
			 * 
			 * TODO: fix https://github.com/eight04/ComicCrawler/issues/208
			 * <code>
			https://www.tohomh123.com/wocaibushiedunvpei/182.html

			解析到的第一张图片网址: https://mh3.zhengdongwuye.cn/upload/wocaibushiedunvpei/2606792/0000.jpg
			正确的的第一张图片网址: https://m-tohomh123-com.mipcdn.com/i/mh3.zhengdongwuye.cn/upload/wocaibushiedunvpei/2606792/0000.jpg
			</code>
			 */
			function fix_image_path(image_data, index) {
				...
				// 假如不是以 `chapter_data.directory` 开头，可能是因为连章节名称都改变了。
				// 这时就需要重新抓取图片网址。
			...
			// library_namespace.run_serial 在 ikmhw.js 太慢了。但一次呼叫太多行程，会造成无回应。
			...
			iterator(function(run_next, image_NO) {
				...
				function add_image_data(XMLHttp) {
					...
					if (iterator === library_namespace.run_serial) {
						...
						// 警告: 采用非同步的方法，获得网址的顺序不一定。因此不能采用 .push()，而应采用 [index]
						// 的方法来记录。
		...
		: function(XMLHttp, work_data, callback, chapter_NO) {
			...
			if (matched) {
				...
				// 有时var123会以数字开头，属于网站bug。 e.g., 风云全集
			...
			// 这个网站似乎每个章节在呼叫 chapterfun.ashx 之后才能下载图片。
			// 并且当呼叫另一次 chapterfun.ashx 之后就不能下载上一次的图片了。
			// 由于图片不能并行下载，下载速度较慢。
			// 但是可以多开几支程式，每一支下载一个作品。
			...
			library_namespace.run_serial(function(run_next, image_NO) {
				...
				library_namespace.log_temporary('图 ' + image_NO + '/'
						+ DM5.IMAGE_COUNT);
		...
		// 要用下面的函数必须指定 pre_parse_chapter_data : null
