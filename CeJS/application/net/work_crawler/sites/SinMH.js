 ...
 * 
 * @fileoverview 本档案包含了解析并处理、批量下载中国大陆常见漫画管理系统: 圣樱漫画管理系统 (圣樱CMS) MHD模板 PC端 的工具。
 * 
 * <code>
 ...
 * 
 * TODO: ONE漫画 https://www.onemanhua.com/ 可能是比 930mh.js 更旧的版本?
 * 
 * @see https://cms.shenl.com/sinmh/
 * @see https://www.manhuadui.com/js/common.js "Created by Shen.L on 2016/1/28."
 * 
 * @since 2018/7/26 11:9:53 模组化 MHD模板。<br />
 *        2019/2/4 add 930mh.js 使用 CryptoJS，采用 DMZJ模板。<br />
 *        2019/7/2 50mh.js 使用 CryptoJS，采用 DMZJ模板。
 */
...
function module_code(library_namespace) {
	...
	var default_configuration = {
		...
		// 1. 使用 PC端 网页取得搜寻所得的作品资料。 (default)
		search_URL : 'search/?keywords=',
		// 2. 使用API取得搜寻所得的作品资料。 (set search_URL:'API')
		...
		parse_work_data : function(html, get_label, extract_work_data) {
			...
			// ...
			// </ul>
			// <a class="intro-act" id="intro-act" href="javascript:;">展开详情</a>
			...
			Object.assign(work_data, {
				author : work_data.漫画作者 || work_data.漫画作者 || work_data.作者
						|| work_data.原著作者,
				status : work_data.漫画状态 || work_data.漫画状态 || work_data.状态,
			...
			if (!work_data.last_update) {
				// for 999comics.js
				var matched = html.match(/最近[于于]([\s\S]+?)<\//);
				// console.log(matched);
				if (matched) {
		...
		check_filtered : function(work_data, html, get_label,
			...
			if (this.try_to_get_blocked_work && chapter_id) {
				...
				&& (!this.recheck || this.recheck in {
					...
					// 这可以保留 work_data.chapter_list 先前的属性。
			...
			if (chapter_data.image_list.length === 0
				...
				if (work_data.removed) {
					library_namespace.info({
						...
						T : [ '§%1 已被屏蔽，不再尝试解析其他章节。', chapter_NO ]
					});
				} else {
	...
	function new_SinMH_comics_crawler(configuration) {
		...
		if (configuration.search_URL === 'API') {
			...
			// 因为不见得会执行到 parse_search_result()，不可放在 parse_search_result() 里面。
