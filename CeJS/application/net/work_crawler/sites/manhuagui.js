 ...
 * 
 * @fileoverview 本档案包含了解析并处理、批量下载中国大陆漫画网站 漫画柜 的工具。
 * 
 * 2017/10: 爱看漫/看漫画改名(DNS被导引到)漫画柜
 * 
 * 57mh 介面程式码类似于 999comics。manhuagui 似乎是在这基础上经过修改？
...
function module_code(library_namespace) {
	...
	var default_configuration = {
		...
		// 模仿实际人工请求。
		// 2018/4: manhuagui 不允许过于频繁的 access，会直接 ban IP。
		// 2018/7/12 22:29:18: 9s: NG, ban 2 hr.
		// 10s, 15s 在下载过100章(1 hr)之后一样会 ban 5hr。
		// 20s, 30s 在下载过200章(~2 hr)之后一样会 ban。
		// 60s 大致OK
		// 2019/2/6: 40s: NG, ban 1 day. 50s 在下载过50章后一样会 ban。.5 day?
		// 2019/3/1: 181s: NG. 3~4min 时，似乎会不固定时间检查、平均每天被封锁一次，每次封锁一日？
		chapter_time_interval : '4min',

		// 2018/3/3 已经不再有常常出现错误的情况。
		...
		get_chapter_list : function(work_data, html, get_label) {
			...
			while (matched = PATTERN_chapter.exec(data)) {
				...
				chapter_data = {
					...
					// .check_downloaded_chapters() 必须先确保已获得最终之
					// chapter_data.title。
		...
		parse_chapter_data : function(html, work_data, get_label, chapter_NO) {
			...
			// 漫画柜的webp图像档案可能是即时生成的? 大小常常不一样。
			chapter_data.image_list = chapter_data.files.map(function(url) {
				return {
					url : path + url + token
				}
			});
			// 当一次下载上百张相片的时候，就会被封锁IP。因此改成一个个下载图像。
