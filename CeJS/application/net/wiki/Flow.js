/**
 * @name CeL function for MediaWiki (Wikipedia / 维基百科): Flow, Structured
 *       Discussions
 * 
...
function module_code(library_namespace) {
	 ...
	 *            page title 页面标题。可为话题id/页面标题+话题标题。<br />
	 *            {String}title or [ {String}API_URL, {String}title or
	 *            {Object}page_data ]
	 * @param {Function}callback
	 *            回调函数。 callback({Object}page_data)
	...
	function Flow_info(title, callback, options) {
		...
		&& function(data) {
			...
			// options.multi: 即使只取得单页面，依旧回传 Array。
			if (!options || !options.multi)
				if (pages.length <= 1) {
					if (pages = pages[0])
						pages.is_Flow = is_Flow(pages);
					library_namespace.debug('只取得单页面 [[' + pages.title
					//
					+ ']]，将回传此页面资料，而非 Array。', 2, 'Flow_info');
				} else {
			...
			/**
			 * page 之 structure 将按照 wiki API 本身之 return！<br />
			 * <code>
	...
	function is_Flow(page_data) {
		...
		// e.g., 从 wiki_API.page 得到的 page_data
	 ...
	 * 
	 * @param {String|Array}title
	 *            page title 页面标题。可为话题id/页面标题+话题标题。 {String}title or [
	 *            {String}API_URL, {String}title or {Object}page_data ]
	 * @param {Function}callback
	 *            回调函数。 callback({Object}topiclist)
	 * @param {Object}[options]
	 *            附加参数/设定选择性/特殊功能与选项
	 */
	 ...
	 *            附加参数/设定选择性/特殊功能与选项
	 * @param {Function}[callback]
	 *            回调函数。 callback(title, error, result)
	 * 
	...
	function edit_topic(title, topic, text, token, options, callback) {
		...
		if (title.length > 260) {
			// [nttopic] 话题标题已限制在 260 位元组内。
			// 自动评论与摘要的长度限制是260个字符。需要小心任何超出上述限定的东西将被裁剪掉。
			// 260 characters
