 ...
 *       常用模板特设功能。本工具档放置的是几乎所有wiki计划通用的模板，或者少数wiki计划特有、且大量使用的著名模板。对各wiki有不同用途的模板，应放置于个别namespace下。
 * 
...
合并 `special page configuration.js`
...
function module_code(library_namespace) {
	...
	/**
	 * Test if `page_data` is biography of a person. 页面为人物传记。
	 * 
	...
	// ------------------------------------------------------------------------
	// template names: The first one is the main template name. 首个名称为正式名称。
	...
	var Old_vfd_multi__names = 'Old vfd multi|Oldafdfull|Vfd-kept|存废讨论被保留|页面存废讨论被保留'
			.split('|');
	...
	// 汲取 page_data 中所有全局转换（全页面语言转换），并交给 processor 处理。
	...
	function parse_conversions(page_data, options) {
		...
		if (page_data.ns === NS_Module) {
			...
			// [[w:zh:Wikipedia:字词转换处理/公共转换组#Lua版建立]]
			...
			|| /local\s+Item\s*=/.test(object)) {
				...
				matched = (matched[1] === 'name' ? 'Module:CGroup/' : '')
						+ matched[3];
				library_namespace.info('parse_conversions: 公共转换组模块 '
						+ wiki_API.title_link_of(page_data)
						// 重定向
						+ ' 重新导向至: ' + wiki_API.title_link_of(matched));
		...
		if (redirect_to) {
			library_namespace.info('parse_conversions: 公共转换组模块 '
					+ wiki_API.title_link_of(page_data)
					// 重定向
					+ ' 重新导向至: ' + wiki_API.title_link_of(redirect_to));
			conversion_list.skipped = 'redirected';
		...
		if (page_data.title
			...
			/-{([\s\S]+?)}-/g, function(all, inner) {
				...
				.replace(/\/\/[^\n]+/g, '')
				// 每条转换须用如下格式书写：* abc => xyz //blah blah ;
				.replace(/["'*#\n]/g, '').replace(/=>/g, '=>'
				// language code
		...
		function for_each_template(token) {
			...
			if (Array.isArray(item.rule)) {
				item.rule.forEach(function(token, index) {
					...
					parsed = CeL.wiki.parse("{{CItem|宿命之子诞生{{=}}>zh-cn:新生|desc=|original=The Hatchling}}");
		...
		if (conversion_list.transclusions) {
			...
			//
			|| page_data.ns === NS_Template ? '公共转换组模块' : '页面 ')
					+ wiki_API.title_link_of(page_data) + ' 内嵌转换组: '
	...
	var result_flags__Hat = {
		...
		rep : '重复提出，无效',
		commons : '应在维基共享资源提请',
		ne : 'notexist|目标页面或档案不存在',
		nq : 'notqualified|提删者未取得提删资格',
		// 保留
		dan : '删后重建',
		// 删除
		ic : '图像因侵权被删|ifd',
		...
		lssd : '无来源或版权资讯，快速删除',
		svg : '已改用SVG图形，快速删除',
		nowcommons : '维基共享资源已提供，快速删除',
		drep : '多次被删除，条目被白纸保护，禁止创建',
		// 转移至其他维基计划
		twc : '转移至维基共享资源',
		tws : '转移至维基文库',
		twb : '转移至维基教科书',
	...
	var result_flags__Old_vfd_multi = {
		k : 'keep|kept|保留',
		nc : 'no consensus|nc|无共识|无共识',
		m : 'moved|move|renamed|移动|移动',
		r : 'redirect|redirected|重定向',
		// incubated : 'incubated',
		// 包含提删者撤回
		sk : 'speedy keep|speedily kept|快速保留|速留',
		ir : 'invalid request|无效|无效|请求无效',
		// '''请求理由已消失'''，页面'''保留''' reason disappeared
		rr : '请求理由消失',
		merge : 'merge|merged|并入|合并',
		d : 'delete|deleted|snowd|删除',
		sd : 'speedy delete|speedily deleted|快速删除',
		tk : 'temporarily keep|暂时保留|暂时保留'
	};
	...
	function parse_Hat(token, options) {
		...
		if (token.type === 'section') {
			...
			token.each('template', function(_token) {
				...
				if (_flags) {
					flags = _flags;
					// 仅以第一个有结论的为主。 e.g., [[Wikipedia:页面存废讨论/记录/2010/09/26#158]]
		...
		// {{Talkendh|result 处理结果|target}}
		// 早期{{delh}}没有 result 处理结果。
	...
	// CeL.wiki.template_functions.Hat.result_includes('快速删除', 'd') === true
	// '快速删除' → 'speedy delete'
	// 'd' → 'delete'
	...
	function parse_Old_vfd_multi_page(page_data, options) {
		...
		parsed.each('template', function(token) {
			...
			// {{Old vfd multi|提删日期|处理结果|page=页面名称}}
	...
	/**
	 * 将 page_data 中的 {{Old vfd multi}} 替换成 replace_to。
	 * 
	...
	function replace_Old_vfd_multi(page_data, replace_to, options) {
		...
		if (!replaced) {
			...
			if (!replace_to.endsWith(line_separator)) {
				// 前面的 replaced 是替代，无需加入换行。
				// 这边将会把 `replace_to` 直接添加在页面顶端，因此需要格式化一下。
	...
	function adapt_function(template_token, index, parent, options) {
		...
		if (function_config.properties) {
			// 为 template_token 加上 template_functions 的属性。
			// e.g., token.expand()
	...
	// loaded_sites = { 已经处理过的 site_name }
	var loaded_sites = Object.create(null);

	// 检查所有依赖于 site_name_loaded 的。
	function initialize_functions_of_site(site_name_loaded) {
		...
		dependency_list.forEach(function(_site_name) {
			// _site_name 依赖于 site_name_loaded。
			...
			if (!dependent_on || dependent_on.some(function(__site_name) {
				if (!(__site_name in loaded_sites)) {
					// __site_name 所依赖的 __site_name 尚未 loaded。
			...
			dependent_on.forEach(function(__site_name) {
				...
				function_name_list.forEach(function(function_name) {
					if (!(function_name in functions_of_site)) {
						library_namespace.debug('设定 ' + _site_name + '.'
	...
	// 纠正 functions_of_site 之模板名称至重定向标的。
	...
	function initialize_session_template_functions(site_name, callback) {
		...
		// 登记好 site_name 以供 initialize_functions_of_site() 使用
		...
		if (library_namespace.is_debug()) {
			library_namespace.info('initialize_session_template_functions: 设定 '
					+ site_name + ' 采用 ' + dependent_on + ' 的模板特设功能。');
		}
	...
	function load_template_functions(site_name, callback) {
		...
		// 注意: 若已设定 `CeL.application.net.wiki.template_functions.zhwiki`，
		// 则不能采用汇添加 prefix `library_namespace.Class` 的
		// library_namespace.to_module_name()，否则会被忽略，不载入！
	...
	// wiki_API.parser.parser_prototype 会被复制到 instance 上。
	// 采用 library_namespace.set_method() 会无法列举与复制，会出问题。
	...
	Object.assign(template_functions, {
		...
		// e.g., zhmoegirl 设定 dependent on [ 'zhwiki' ]
		// 必须是模板名称不可能使用到的 key 值。
