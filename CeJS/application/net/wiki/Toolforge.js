/**
 * @name CeL function for MediaWiki (Wikipedia / 维基百科): Toolforge only functions
 * 
 * @fileoverview 本档案包含了 MediaWiki 自动化作业用程式库的子程式库。
 * 
 * 条件合适时，应该会由 CeL.application.net.wiki 载入。
 * 
 * TODO:<code>
...
function module_code(library_namespace) {
	...
	function set_SQL_config_language(language) {
		...
		// TODO: 'zh.news'
		// 警告: this.language 可能包含 'zhwikinews' 之类。
	...
	if (wmflabs) {
		try {
			...
			if (node_mysql) {
				...
				// Wikimedia Toolforge
				// 上之资料库仅为正式上线版之删节副本。资料并非最新版本(但误差多于数分内)，也不完全，
				// <del>甚至可能为其他 users 窜改过</del>。
				+ 'replica.my.cnf');
			}
		} catch (e) {
	 ...
	 *            回调函数。 callback({Object}error, {Array}rows, {Array}fields)
	 * @param {Object}[config]
	 *            configuration.
	 * 
	 * @see https://wikitech.wikimedia.org/wiki/Help:Toolforge/Database
	...
	// 进入 default host (TOOLSDB)。
	var SQL_session = new CeL.wiki.SQL(()=>{});
	// 进入 default host (TOOLSDB)，并预先创建 user's database 'dbname' (e.g., 's00000__dbname')
	var SQL_session = new CeL.wiki.SQL('dbname', ()=>{});
	// 进入 zhwiki.zhwiki_p。
	var SQL_session = new CeL.wiki.SQL(()=>{}, 'zh');
	// 进入 zhwiki.zhwiki_p，并预先创建 user's database 'dbname' (e.g., 's00000__dbname')
	...
	function SQL_session(dbname, callback, language) {
		...
		if (dbname) {
			...
			// this.config.database 已经在 set_SQL_config_language() 设定。
		}
	...
	SQL_session.prototype.databases = function(callback, all) {
		...
		this.connect(function(error) {
			// reset connection,
			// 预防 PROTOCOL_ENQUEUE_AFTER_FATAL_ERROR
	...
	if (SQL_config) {
		...
		// export 导出: CeL.wiki.SQL() 仅可在 Wikimedia Toolforge 上使用。
	...
	/**
	 * Get page title 页面标题 list of [[Special:RecentChanges]] 最近更改.
	 * 
	 * @examples<code>
	 ...
	 * 
	 * @param {Function}callback
	 *            回调函数。 callback({Array}page title 页面标题 list)
	...
	function get_recent_via_databases(callback, options) {
		...
		if (!SQL) {
			...
			// 不指定namespace，或者指定namespace为((undefined)): 取得所有的namespace。
			...
			Object.assign(SQL,
			// {String|Array|Object}options.where: 自订筛选条件。
			options.where);
			SQL = generate_SQL_WHERE(SQL, 'rc_');
			// console.log(SQL);

			// https://phabricator.wikimedia.org/T223406
			// TODO: 旧版上 `actor`, `comment` 这两个资料表不存在会出错，需要先侦测。
		...
		run_SQL(SQL, function(error, rows, fields) {
			...
			rows.forEach(function(row) {
				...
				? wiki_API.namespace.name_of(row.rc_namespace, options) + ':'
						: '';
				// 基本上 API 尽可能模拟 recentchanges，与之一致。
				result.push({
					...
					// 0: 可能为flow. 此时title为主页面名，非topic。由.rc_params可获得相关资讯。
