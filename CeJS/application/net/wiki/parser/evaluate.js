/**
 * @name CeL function for MediaWiki (Wikipedia / 维基百科): parse wikitext 解析维基语法
 * 
...
function module_code(library_namespace) {
	...
	/**
	 * 演算/简化要 transclusion 的模板 wikitext。
	 * 
	 ...
	 *            迭代呼叫层数。
	 * 
	 * @returns {Promise|Array} 简化过且解析过的 wiki syntax。
	 */
	function simplify_transclusion(wikitext, template_token_called, options,
		...
		parse 嵌入section内文 [[mw:Extension:Labeled_Section_Transclusion]]:
		{{#lsth:page_title|section begin in wikitext|section end in wikitext}}, {{#section-h:page_title}} 语意上相当于 {{page_title#section}}。如果有多个相同名称的section，仅转换第一个。The matching is case insensitive
		...
		var promise = parsed.each('magic_word_function', function(token) {
			...
			if (level > 3 || !parsed || parsed.type !== 'transclusion') {
				// `page_data ?`: 为了维持cache与第一次执行的输出相同。
	...
	// 循环展开模板节点。
	// 可考虑是否采用 CeL.wiki.wikitext_to_plain_text()
		...
		while (token && token.type === 'transclusion') {
			if (typeof token.expand !== 'function') {
				if (wiki_API.template_functions) {
					...
					library_namespace
							.debug(
									'建议 include application.net.wiki.template_functions',
	...
	// 类似 wiki_API_expandtemplates()
	// ** 仅能提供简单的演算功能，但提供 cache，不必每次传输资料回伺服器。
	// [[Special:ExpandTemplates]]
	// 使用上注意: 应设定 options[KEY_on_page_title_option]
	// 可考虑是否采用 CeL.wiki.wikitext_to_plain_text()
	function expand_transclusion(wikitext, options, level) {
		...
		if (typeof options === 'string') {
			...
			// .new_options(): 会设定 options.something_not_evaluated，避免污染。
			options = Object.assign({
				allow_promise : true,
				set_not_evaluated : true
			}, options);
	...
	var PATTERN_expr_乘除 = generate_PATTERN_expr_operations('[*/]|div|mod|fmod',
			2);
	var PATTERN_expr_加减 = generate_PATTERN_expr_operations('[+\-]', 2);
	...
	function eval_expr(expression) {
		...
		function _handle_binary_operations(all, _1, op, _2) {
			...
			throw new Error('二元运算符未定义，这不该发生，请联络函式库作者: '
		...
		function handle_unary_operations(all, op, number) {
			...
			throw new Error('一元运算符未定义，这不该发生，请联络函式库作者: '
		...
		while (true) {
			...
			new_expression = new_expression.replace(PATTERN_expr_加减,
					handle_binary_operations);
	...
	function evaluate_parser_function_token(options) {
		...
		function NYI() {
			...
			// 标记并直接回传，避免 evaluate_parsed() 重复呼叫。
		...
		case '#formatdate':
			...
			default:
				// TODO: 未指定日期格式时，会自动判别，并且输出格式化过的完整日期。
