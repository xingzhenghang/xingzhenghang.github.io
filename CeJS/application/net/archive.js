 ...
 * 
 * @fileoverview 本档案包含了 checking archive sites 用的程式库。
 * 
 * TODO:<code>
...
function module_code(library_namespace) {
	...
	function archive_org_operator() {
		...
		get_URL(archive_org.API_URL + URL, function(data, error) {
			// 若正执行者，必须负责执行完注销掉 archive_org_running。
			...
			function do_callback(data, error) {
				library_namespace.debug(URL + ': 登记 result: ' + [ data, error ]
	 ...
	 * snapshot.
	 * 
	 * archive.org 此 API 只能检查是否有 snapshot，不能制造 snapshot。
	 * 
	 * @param {String}URL
	 *            欲请求之目的 URL
	 * @param {Function}[callback]
	 *            回调函数。 callback({Object|Undefined}closest_snapshot_data,
	 *            error);
	...
	function archive_org(URL, callback, options) {
		...
		if (cached_data && !cached_data.checking) {
			library_namespace.debug('已登记 [' + URL + ']。直接处理掉。', 3,
					'archive_org');
		...
		if (cached_data) {
			...
			library_namespace.debug('登记 URL [' + URL + ']，表示正处理中。', 3,
					'archive_org');
	...
	if (false) {
		var dns = require('dns');
		// 短时间内 request 过多 host names 会造成 Tool Labs 常常 DNS error，
		// getaddrinfo ENOTFOUND。
		dns.setServers(dns.getServers().append([ '8.8.8.8', '8.8.4.4' ]));
	}

	/**
	 * 检查 URL 之 access 状态。若不可得，则预先测试 archive sites 是否有 cached data。
	 * 
	 * @param {String}URL
	 *            欲请求之目的 URL
	 * @param {Function}[callback]
	 *            回调函数。 callback(link_status, cached_data);
	 * @param {Object}[options]
	 *            附加参数/设定选择性/特殊功能与选项
	 */
		...
		function do_callback(status, OK) {
			...
			if (OK) {
				...
				archive_org(URL, function(closest_snapshot_data, error) {
					// 会先 check archive site 再注销此 URL，
					// 确保之后处理时已经有 archived data。
	...
	Object.assign(archive_sites, {
		// 为了模拟一般情况下的 access，因此设定 user agent，避免特殊待遇。
