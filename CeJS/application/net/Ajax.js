 ...
 * @fileoverview 本档案包含了模拟WWW客户端发送HTTP/HTTPS请求用的 functions。
 * @since 2015/1/1
 */
...
function module_code(library_namespace) {
	...
	function normalize_URL_to_fetch(URL_to_fetch, charset, options) {
		...
		if (Array.isArray(URL_to_fetch)) {
			...
			// 当输入 {URL} 时，node_https.request() 会将 {URL} 转成
			// {Object}options，不会考虑额外选项 (headers, ...)。
			// 且必须处理 charset，干脆直接将 {URL} 转成寻常 plain object / {URI}。
	...
	// 1 读取中 Sending Data
	// 2 已读取 Data Sent
	// 3 资讯交换中 interactive: getting data
	// 4 一切完成 Completed
	...
	/**
	 * 读取 URL via XMLHttpRequest。
	 * 
	 * @param {String|Object}URL_to_fetch
	 *            欲请求之目的 URL or options
	...
	function get_URL(URL_to_fetch, onload, charset, post_data, options) {
		...
		try {
			// IE:404 会 throw error, timeout 除了 throw error,
			// 还会 readystatechange;
			// Gecko 亦会 throw error
			...
			// TODO: 处理有 onload 下之 timeout 逾时ms数
			...
			if (!onload) {
				...
				// XMLHttp.responseText 会把传回值当字串用
				// XMLHttp.responseXML 会把传回值视为 XMLDocument 物件，而后可用 JavaScript
				// DOM 相关函式处理
				// IE only(?):
	...
	function to_form_data(parameters, callback, options) {
		function get_file_object(value, callback, key, slice) {
			...
			_.get_URL(value, function(XMLHttp, error) {
				...
				library_namespace.debug({
					...
					T : [ '自网路取得 URL：%1，%2{{PLURAL:%2|位元组}}。', value,
		...
		function process_next() {
			...
			// 预防有 null, undefined, {Number}, true 之类。
	...
	/**
	 * <code>
		读取URL by XMLHttpRequest
		http://jck11.pixnet.net/blog/post/11630232
	
	 * 若有多行程或为各URL设定个别XMLHttp之必要，请在一开始便设定deprecated_get_URL.multi_request，并且别再更改。
	 ** 在此情况下，单一URL仍只能有单一个request!
	 ** 设定 handle_function 须注意程式在等待回应时若无执行其他程式码将自动中止！
		可设定：
	...
	arguments f:{
		...
		async:ture/false,	//	true if want to asynchronous(非同期), false if synchronous(同期的,会直到readyState==4才return)	http://jpspan.sourceforge.net/wiki/doku.php?id=javascript:xmlhttprequest:behaviour
		user:'userName',
		passwd:'****',	//	password
	...
	handle_function:
	自行处理	typeof handle_function=='function':
	function handle_function(error){..}
	代为处理	handle_function=[d_func,0: responseText,1: responseXML]:
	responseXML:	http://msdn2.microsoft.com/en-us/library/ms757878.aspx
	function d_func(content,headers[,XMLHttp,URL]){
		if(headers){
			//	content,headers各为XMLHttp.responseText内容及XMLHttp.getAllResponseHeaders()，其他皆可由XMLHttp取得。
		}else{
			//	content为error
		}
	}
	...
	function deprecated_get_URL(f) {
			...
			if (_f.asked > 2) {
				...
				try {
					if (typeof _f.asked === 'undefined') {
						_f.asked = 0;
						alert('我们需要一点权限来使用 XMLHttpRequest.open。\n* 请勾选记住这项设定的方格。');
					}
		...
		try {
			// IE:404会throw error, timeout除了throw error, 还会readystatechange;
			// Gecko亦会throw error
			try {
			...
			// &&_f.XMLHttp.setRequestHeader
				...
				// use .getAttribute('method') to get 长度不一定如此
				if (false)
					_f.XMLHttp.setRequestHeader("Content-Length",
							f.sendDoc.length);
				// 有些CGI会用Content-Type测试是XMLHttp或是regular form
			 ...
			 *      根据 W3C的 XMLHttpRequest 规格书上说，①在呼叫 open
			 *      时，如果readyState是4(Loaded) ②呼叫abort之后
			 *      ③发生其他错误，如网路问题，无穷回圈等等，则会重设所有的值。使用全域的情况就只有第一次可以执行，因为之后的readyState是4，所以onreadystatechange
			 *      放在open之前会被清空，因此，onreadystatechange 必须放在open之后就可以避免这个问题。
			 * 
			 * 每使用一次XMLHttpRequest，不管成功或失败，都要重设onreadystatechange一次。onreadystatechange
			 * 的初始值是 null
			 * 
			...
			if (!f.fn) {
				/**
				 * 非async(异步的)能在此就得到 response。Safari and Konqueror cannot
		...
		if (!e)
			if (typeof _oXMLH === 'object' && _oXMLH) {
				if (_oXMLH.parseError
					...
					// 用 .responseXML.documentElement 可调用
					deprecated_get_URL.doing--;
					deprecated_get_URL.clean(URL);
					return;
				}
	 ...
	 * 有headers时content包含回应，否则content表error
	 */
	deprecated_get_URL.HandleContent = function(content, headers, _oXMLHttp,
		...
		if (headers) {
			...
			// error
			// test时，可用deprecated_get_URL.XMLHttp.open("HEAD","_URL_",true);，deprecated_get_URL(url,handle_function,'HEAD',true)。
	...
	/**
	 * 快速 merge cookie: 只检查若没有重复的 key，则直接加入。不检查 path 也不处理 expires, domain,
	 * secure。<br />
	 * 为增加效率，不检查 agent.last_cookie 本身之重复的 cookie。
	 * 
	...
	function merge_cookie(agent, cookie) {
		// 初始化 initialization + 正规化 normalization
	...
	/**
	 * 读取 URL via node http/https。<br />
	 * assert: arguments 必须与 get_URL() 相容！
	 * 
	 * @param {String|Object}URL_to_fetch
	 *            欲请求之目的 URL or options
	 * @param {Function}[onload]
	...
	function get_URL_node(URL_to_fetch, onload, charset, post_data, options) {
		...
		// assert: 自此开始不会改变 URL，也不会中途 exit 本函数。
		 ...
		 * mh160.js 必须使用 request，https-proxy-agent 才能正常工作 TODO:
		 * socks-proxy-agent
		 * 
		 * <code>
		...
		if (!proxy_server
			...
			if (proxy_original_agent && proxy_original_agent.last_cookie) {
				// 复制原agent的cookie设定。 @see merge_cookie()
		...
		if (agent) {
			...
			&& agent.protocol !== URL_options_to_fetch.protocol) {
				var message = {
					...
					T : [ '自定义 agent 与 URL 之协定不同，将尝试采用符合的协定：%1',
					//
					agent.protocol + ' !== ' + URL_options_to_fetch.protocol ]
				};
			...
			library_namespace.debug('采用泛用的 agent。', 6, 'get_URL_node');
		...
		if (options.cookie && !agent.last_cookie) {
			library_namespace.debug({
				...
				T : [ '重新设定 cookie 成：%1', options.cookie ]
			}, 3, 'get_URL_node');
			agent.last_cookie = options.cookie;
		}

		var request, finished,
		// result_Object 模拟 XMLHttp response。
		result_Object = {
			// node_agent : agent,
			...
			// 因为可能 redirecting 过，这边列出的才是最终 URL。
			// https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/responseURL
			responseURL : URL_to_fetch
		},
		// assert: 必定从 _onfail 或 _onload 作结，以确保会注销登记。
		// 本函数unregister()应该放在所有本执行绪会执行到onload的程式码中。
		unregister = function() {
			/**
		...
		_onfail = function(error) {
			if (unregister()) {
				// console.log('exit: 预防 timeout 时重复执行。');
				return;
			}

			// console.trace([ options.error_count, options.error_retry ]);
			// 连线逾期/失败时再重新取得页面之重试次数。
			if (options.error_retry >= 1
			// 例如当遇到 404 或 502 时，再多尝试一下。
			...
			&& error.code !== 'TIMEOUT') {
				if (error.code === 'ENOTFOUND') {
					...
					// 这里用太多并列处理，会造成 error.code "EMFILE"。
					// console.error(error);
					// console.error(options);
				}
			}
			// 在出现错误时，将 onload 当作 callback。并要确保 {Object}response
			// 因此应该要先检查 error 再处理 response
		...
		_onload = function(response) {
			...
			// 在这边不过刚开始从伺服器得到资料，因此还不可执行unregister()，否则依然可能遇到timeout。
			if (finished) {
				return;
			}
			...
			// 若原先有agent，应该合并到原先的agent，而非可能为暂时性/泛用的agent。
			merge_cookie(options.agent || agent, response.headers['set-cookie']);
			// 先合并完cookie之后才能转址，否则会漏失掉须设定的cookie。
			...
			if ((response.statusCode / 100 | 0) === 3
				...
				// 重新导向的时候亦传送 post data。For wiki API
				// e.g., https://github.com/kanasimi/CeJS/issues/12
				// 但piaotian.js搜索作品名称直接302导向时，加入post_data会产生 405 Not Allowed
			...
			// 在有 options.onfail 时仅 .debug()。但这并没啥条理...
			...
			// node.js会自动把headers转成小写。
			// https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition

			// 在503之类的情况下。可能没"Content-Type:"。这时 response 将无.type。
			...
			&& !options.write_to && !options.write_to_directory) {
				// 照理unregister()应该放这边，但如此速度过慢。因此改放在 _onload 一开始。
				unregister();
				library_namespace.warn([ 'get_URL_node: ', {
			...
			library_namespace.debug({
				...
				T : [ '等待接收从网址 [%1] 传输回的资料……', URL_to_fetch ]
			}, 3, 'get_URL_node');
			...
			response.on('end', function() {
				...
				// 照理应该放这边，但如此速度过慢。因此改放在 _onload 一开始。
				// unregister();
				...
				if (!data) {
					...
					// 若是容许空内容，应该特别指定 options.allow_empty。
				...
				if (flow_encoding && data) {
					...
					case 'gzip':
						...
						/**
						 * <code>
						可能因为呼叫到旧版library，于此有时会出现 "TypeError: Object #<Object> has no method 'gunzipSync'"
						有时会有 Error: unexpected end of file
						</code>
						 */
						try {
				...
				// non-standard 非标准: 设定 charset = 'buffer' 的话，将回传 {Buffer}。
				if (data && charset !== 'buffer') {
					// 未设定 charset 的话，default charset: UTF-8.
				...
				// result_Object模拟 XMLHttp。
				result_Object.responseText = data;

				// ------------------------------

				if ((response.statusCode / 100 | 0) !== 2) {
					// ssert: options.error_retry >= 1 ? 最后一次 error
				...
				// assert: 执行至此表示成功取得资料、没有错误，
				// 开始正常运作至结尾，不会再有中途 return。
		...
		// 必要的 headers: User-Agent, Accept-Language。其他是为了模拟得更真实点。
		URL_options_to_fetch.headers = Object.assign({
			...
			// 少了 '*/*' CrossRef API 会回应 406 "No acceptable resource available."
			+ ',image/avif' + ',image/webp,image/apng,*/*;q=0.8'
					+ ',application/signed-exchange;v=b3;q=0.9',
			// Accept : 'application/json, text/plain, */*',

			// 为了防止 Cloudflare bot protection(?) 阻挡，必须加上 Accept-Language。
			...
			// 可以将其非正式称为 Accept-Transfer-Encoding，这个名称显得更直观一些。
			// 当 TE 设置为 trailers 时，如果服务端支持并且返回了
			// Transfer-Encoding: trailers，那么同时也必须返回另一个响应标头 Trailer，
			// TE : 'Trailers',
			...
			// 为了顺畅使用 Cloudflare，必须加上 Sec-Fetch-headers？ e.g., mymhh.js
		...
		// && node_zlib.deflateSync
			...
			// 早期 node v0.10.25 无 zlib.gunzipSync。Added in: v0.11.12
			// 'gzip, deflate, *'
		...
		if (post_data) {
			...
			Object.assign(URL_options_to_fetch.headers, {
				...
				// prevent HTTP 411 错误 – 需要内容长度头 (411 Length Required)
		...
		/** {Natural}timeout in ms for get URL. 逾时ms数 */
		...
		_ontimeout = function(e) {
			...
			try {
				...
				// timeoutイベントは発生しているものの、イベント発生后も引き続きレスポンスを待ち続けている
				// request.end();
				// request.abort();
				request.destroy();
			} catch (err) {
		...
		if (timeout > 0) {
			// setTimeout method 1
			// 此方法似乎不能确实于时间到时截断。或许因为正在 handshaking?
		...
		// 遇到 "Unhandled 'error' event"，或许是 print 到 stdout 时出错了，不一定是本函数的问题。

		// debug error: socket parse error
		// CloudFlare 遇到 HPE_INVALID_CONSTANT，可能是因为需要 encodeURI(url)。
	...
	/**
	 * get https:// through proxy 用于取得https网站。
	 * 
	...
	function HttpsProxyAgent(proxy_server, options) {
		...
		this.createConnection = function connect_proxy_server(options, callback) {
			...
			// console.log(request);
			// 此时若有 agent，会是 https。但是待会要用 http connect。
	...
	try {
		...
		library_namespace.debug([ this.id + ': ', {
			...
			T : '无 node.js 之 `fs` 套件，因此不具备 node 之档案操作功能。'
		} ]);
		if (false) {
	 ...
	 * cache 作业操作之辅助套装函数。
	 * 
	 * 注意: 若执行 onload() 时没提供 XMLHttp，表示采用 cache。
	 * 
	 * TODO: 以 HEAD 检测，若有新版本则不采用 cache。
	 * 
	...
	function get_URL_cache_node(URL, onload, options) {
		...
		function(error, data) {
			...
			// 警告: 对于从 HTTP 标头获得文件名的情况，就算不设定 options.reget 也没用，还是会重新获取档案。
			...
			_.get_URL(URL, function(XMLHttp, error) {
				if (error) {
					...
					// WARNING: XMLHttp 仅在重新取得 URL 时提供。
	...
	function node_fetch(input, init) {
		...
		function executor(resolve, reject) {
			...
			// CloudFlare 必须设定好headers才能才会才允许回传资料。
	 ...
	 * 必须额外设定 credentials。
	 * 
	 * TODO: fetch 预设上不传送或接收任何 cookies，如果网站依赖 session 会导致请求回传未经认证，需要使用 cookies
	 * 
	 * @examples <code>
	...
	function fetch__get_URL(input, init) {
		function executor(resolve, reject) {
			...
			// CloudFlare 必须设定好 headers 才能才会才允许回传资料。
			// get_URL() 可自动设定 headers。
