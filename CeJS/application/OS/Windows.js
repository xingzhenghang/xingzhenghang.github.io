 ...
 * @fileoverview
 * 本档案包含了 Windows 系统管理专用的 functions。
 * @since	
 */
...
// 用 IE 跑 ActiveXObject 可能会出现 ActiveX 指令码的警告，须更改「允许主动式内容在我电脑上的档案中执行」或改用 <a href="http://msdn.microsoft.com/en-us/library/ms537628%28v=vs.85%29.aspx" accessdate="2011/12/11 20:4" title="The Mark of the Web (MOTW)">Mark of the Web</a>。
// IE 工具→「网际网路选项」→「进阶」→「安全性」→「允许主动式内容在我电脑上的档案中执行」
...
// WScript.Echo((_.no_COM? '没' : '') + '有 Windows Component Object Model。');
 ...
 * ** HTA 中应该在 DOM ready 后呼叫，否则 document.getElementsByTagName 不会有东西！
...
function parse_command_line(command_line, no_command_name){
	...
	// commandLine 第一引数为 full script name
...
/*	2007/11/17 23:3:53
	使用 ADSI (Active Directory Service Interface) 存取资料
...
function addUser(name,pw,group,computer){
 ...
 //	连上伺服器
 var oIADs,o;
 //	利用Create指令，指定产生一个新的使用者类别，以及使用者帐号的名称。使用SetInfo的指令将目录服务中的资料更新。
...
function getEnvironment(){
 ...
 // Windows 95/98/Me の场合、使用できる strType は Process に限定されます。
 var i,j,k,o=new Enumerator(WshShell.Environment("Process"));/*	Win9x、NT（Administratorもしくはほかのユーザー）の区别なく、すべての场合でエラーが発生しないようにするには、strTypeに"PROCESS"を指定するとよいでしょう。
	机器上所有已定义的环境变数Windows environment variables	http://msdn2.microsoft.com/en-us/library/fd7hxfdd(VS.85).aspx	http://www.roy.hi-ho.ne.jp/mutaguchi/wsh/refer/lesson11.htm	http://nacelle.info/wsh/03001.php	http://www.cs.odu.edu/~wild/windowsNT/Spring00/wsh.htm
 ...
 */
 while(!o.atEnd()){
  ...
  WinEnvironment[i.slice(0,j)]=i.substr(j+1);	//	value以';'作为分隔，若有必要可使用.split(';')
  o.moveNext();
 }
 ...
 //	VB中用For Each .. In可列举出全部，但JS则不行-_-所以得自己列举
 // fso.GetSpecialFolder( 0: Windows 文件夹 1: System 文件夹 2: Temp 文件夹 )
...
.
/**
 * 转换 VB 的 Safe Array 成为 JS Array.
...
.
/**
 * 转换JS Array成为VB的Safe Array.
 * Safe Array To JS Array: plaese use new VBArray().
 * JScriptの配列は実际にはCSV文字列だったりする。VBScriptのvartypeに食わせると8(VT_STRING)が返ってくることからもわかる。
 * @param array
 * @returns
 * @see
...
function SendKeys(keys,appTitle,timeOut,timeInterval){
 ...
 if(isNaN(timeInterval)||timeInterval<1)timeInterval=100;	//	时间间隔
 ...
 if(!SendKeysU)SendKeysU=100;	//	时间间隔单位
 while(keys.match(/\{([.\d]+)\}/)){
