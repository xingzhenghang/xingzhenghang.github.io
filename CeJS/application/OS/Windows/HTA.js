 ...
 * @fileoverview
 * 本档案包含了 Microsoft Windows HTML Applications (HTAs) 的 functions。
 * @since	
 */
...
IE临时文件的位置可以从注册表键值 HKLM\Software\Microsoft\Windows\CurrentVersion\Internet Settings\Cache\paths\Directory 中读取。
*/
...
*/
	...
	//	要先浏览了网页，才能实行 IEApp.Document其他功能。
	this.go(URL || '');
...
_.IEA.prototype = {
	 ...
	 * 以有无视窗，否则以有无内容判别是否 OK.<br />
	 * 关掉视窗时， typeof this.app.Visible=='unknown'
	...
	/**
	 * go to URL. 注意: 若是 .go('URL#hash') 则可能无反应，须改成 .go('URL')。
		...
		try {
			if (URL === '' || isNaN(URL)) {
				...
				if (URL) {
					...
					//	IEApp.Document.frames.open(URL);	**	请注意：这里偶尔会造成script停滞，并跳出警告视窗！
...
*/
	...
	quit : function(quit_IEA) {
		...
		if (typeof CollectGarbage === 'function')
			//	CollectGarbage(): undocumented IE javascript method: 先置为 null 再 CollectGarbage(); 设置为null,它会断开对象的引用，但是IE为了节省资源（经常释放内存也会占系统资源），因此采用的是延迟释放策略，你调用CollectGarbage函数，就会强制立即释放。
	...
	// 用IE.doc().title or IE.app.LocationName 可反映状况
	doc : function() {
		try {
	...
	eval : function(code, scope) {
		// IE 11 中无法使用 this.win().eval:
		// Error 70 [Error] (facility code 10): 没有使用权限
...
*/
	...
	g : function get_node(selector, base_space) {
		...
		if (node && (('' + node.tagName).toLowerCase() in {
			...
			// IE11: typeof node.contentWindow === 'object', but node.contentWindow.document: 没有使用权限
	...
	// form name array
	// formNA : 0,
	//	return name&id object. 设置这个还可以强制 do submit 使用 name 为主，不用 id。
	fillForm_rtE : 0,
	/**
	 * 自动填写表单
	 * 
	 * @param {Object}pair
	 *            设定 pair: {id/name: value}
	 * @param submit_id
...
function get_HTML(url) {
	if (url && !get_HTML.no_directly) {
		...
		try {
			// 当大量 access 时，可能被网站挡下。e.g., Google.
	...
	if (IEA_instance.href().includes('.google.com/sorry/IndexRedirect')) {
		...
		// 如果您使用了自动程式常用的进阶字词，或是快速传出多项要求，系统可能会要求您进行人机验证 (Captcha)。
		throw new Error('如要继续，请输入人机验证 (Captcha) 字元。');
	}
...
//	WSH环境中设定剪贴簿的资料：多此一举	http://yuriken.hp.infoseek.co.jp/index3.html	http://code.google.com/p/zeroclipboard/
