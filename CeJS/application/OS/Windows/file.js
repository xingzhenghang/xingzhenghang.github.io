 ...
 * @fileoverview
 * 本档案包含了 Windows 的档案/文件管理 functions。
 * 
 ...
 * @class	Windows 下，档案操作相关之 function。
 */
var _// JSDT:_module_
= function() {
	//	null module constructor
 ...
 * 可用 move_file_filter() 来排除不要的<br />
 * 本函数可能暂时改变目前工作目录！
 * @param {String} from	from file
 * @param {String} to	to file
 * @param {String} base_path	base path
 * @param flag
 * @param {Function} filter	可用 filter() 来排除不要的
 * @return	{Object} report
...
move_file = function move_file(from, to, base_path, flag, filter) {
	...
	*/
	...
	// TODO: 对from不存在与为folder之处里: fuzzy
	...
	& _f.copy ? 'copy' : to === _f.remove || flag & _f.remove
			&& !to ? 'delete' : from !== default_from || to ? 'move'
					: 'list';
	// if(func=='delete')to=_f.remove; // 反正不是用这个判别的
	//library_namespace.debug('use_exact: ' + use_exact + '\nbase_path: ' + base_path + '\nfrom: ' + from);
	// BUG: 这样顺序会乱掉，使得 traverse (遍历)不完全
				...
				try {
					...
					if (func === 'delete') {
						...
						if (not_test) {
							...
							if (func === 'copy')
								//	Copy() 用的是 FileSystemObject.CopyFile or FileSystemObject.CopyFolder, 亦可用万用字元(wildcard characters)
								i.Copy(t, overwrite);
							else
								i.Move(t);
						}
...
move_file.f = {
		...
		copy : 64,
		/**
		 * 当 target 指定此 flag，或包含此 flag 而未指定 target 时，remove the source。
		 * @_memberOf _module_
		 */
		remove : 128
};
...
function mv(from,to,dir,only_filename,reverse){
 ...
 return e||new Error(to,(to==1?'移动するファイルは存在しない':'移动先が既存した')+':\n'+_f+'\n→\n'+_t);
}
...
/*
TODO
	未来：change_attributes(path,'-ReadOnly&-Hidden&-System')
	...
	while (true)
		try {
			...
			if ((e.number & 0xFFFF) !== 70) {
				return pErr(e, 0,
						'开启资料档 [<green>' + FN + '<\/>] 时发生错误！'),
						6 === alert(
								'开启资料档 [' + FN + ']时发生不明错误，\n	是否中止执行？',
								0, ScriptName + ' 测试是否已开启资料档：发生不明错误！',
								4 + 48) ? 1 : 0;
			}
			if (2 === WshShell.Popup(
					'★资料档：\n\	' + FN + '\n	无法写入！\n\n可能原因与解决方法：\n	①资料档已被开启。执行程式前请勿以其他程式开启资料档！\n	②资料档被设成唯读，请取消唯读属性。',
					0, ScriptName + '：资料档开启发生错误！', 5 + 48))
				return 2;
		}
	return 0;
};
...
/*
	转换捷径, 传回shortcut的Object. true path
...
function parse_shortcut(path,rtPath){
 if(typeof path!=='string')path='';
  ...
  //	档名有可能是不被容许的字元（不一定总是有'?'），这时只有.url以文字储存还读得到。
...
ShowCommand=	//	规定Internet Explorer启动后窗口的初始状态：7表示最小化，3表示最大化；如果没有ShowCommand这一项的话则表示正常大小。
IconIndex=1	//	IconFile和IconIndex用来为Internet快捷方式指定图标
IconFile=C:\WINDOWS\SYSTEM\url.dll
Hotkey=1601

Hotkey:
下面加起来: Fn 单独 || (Fn || base) 择一 + additional 择2~3
base:
0=0x30(ASCII)
9=0x39(ASCII)
A=0x41(ASCII)
...
//filepath=OpenFileDialog();	基于安全，IE无法指定初始值或型态
...
/**
 * 取得下一个序号的档名，如输入pp\aa.txt将尝试pp\aa.txt→pp\aa[pattern].txt
 * @param file_path	file path
 * @param begin_serial	begin serial
 * @param pattern	增添主档名的模式，包含Ser的部分将被转换为序号
 * @returns
 */
...
.
/**
 * 将以 adTypeBinary 读到的 2 bytes 一组之 ADO Stream binary data 资料转换成可使用之 data。
 * 
 * @example
 * <code>
 * // 较安全的读档：
 ...
 * 
 * @param binary_data
 *            ADO Stream 来的 binary data
 * @param {Integer}[length]
 *            处理 length
 * @param {Integer}[type]
 *            是否为数字
 * @returns {String|Number}转换过之资料
 * @see <a
 *      href="http://www.hawk.34sp.com/stdpls/dwsh/charset_adodb.html">Hawk&apos;s
 *      W3 Laboratory : Disposable WSH : 番外编：文字エンコーディングとADODB.Stream</a>
 * @_memberOf _module_
 */
translate_ADO_Stream_binary_data = function translate_ADO_Stream_binary_data(binary_data, length, type) {
	...
	// Error 5029 [RangeError] (facility code 10): 阵列长度必须是一有限的正整数
	...
	for (; i < length; i++)
		try {
			...
			e.description = 'translate_ADO_Stream_binary_data: 输入了错误的资料:\n'
					+ e.description;
			throw e;
		}
...
.
/**
 * 转换以 adTypeBinary 读到的资料
 * @param	data	以 adTypeBinary 读到的资料
 * @param	pos	position
...
retErr : function(e) {
	e.description = 'Ado_binary: 输入了错误的资料:\n' + e.description;
	throw e;
},
/**
 * @private
...
/**
 * 实际上没多大效用。实用解决法：少用 ADO_Stream.Write()
 * @return
 * @_memberOf	_module_
 */
renew : function() {
...
/**
 * 简易开档函数。<br />
 * 会先尝试以 ADO Stream 开档，不成时则尝试 FSO。<br />
 * 主要提供给 <a href="#read_file">read_file</a>, <a href="#write_file">write_file</a>
 * 使用。
 * 
 * @param {String}file_path
 *            file path
 ...
 * 
 * @since
 * 2012/3/29 22:45:19	重写 open_file, read_file, write_file. 行为有所改变，与之前版本不相容!
 * 
 * @see <a
...
function open_file(file_path, format, IO_mode) {
	...
	if (isNaN(format)) {
		try {
			// ASPの场合, Err.Number=-2147221005表不支援.
		...
		// 使用某些防毒软体(如诺顿 Norton)时，.OpenTextFile() 可能会被拦截，因而延宕。
		try {
			if (IO_mode === _.IO_mode.ForAppending && !FSO.FileExists(file_path))
				// 无此档时改 writing.
				IO_mode = _.IO_mode.ForWriting;
...
//	TODO: 与 guess_encoding.OS_alias 统合。
open_file.OS_alias = {
	'GB 2312' : 'GB2312'
};
	...
	try {
		...
		if (is_ADO_Stream(file_stream)) {
			...
			if (file_stream.Type === AdoEnums.adTypeBinary) {
				...
				//	此时 IO_mode === true 表示不将 binary data 转换为 String!
				if (!IO_mode)
...
function write_file(file_path, contents, format, IO_mode, no_overwrite) {
	...
	try {
		if (is_ADO_Stream(file_stream)) {
			...
			// 设定 end of the stream 后，将 truncate 之后的内容。
			// http://msdn.microsoft.com/en-us/library/windows/desktop/ms676745.aspx
			library_namespace.debug('设定 end of the stream from: ' + file_stream.Position + '/' + file_stream.Size + ', EOS:' + file_stream.EOS, 2);
			//file_stream.Position = file_stream.Size;
			// .SetEOS() 不能传入 arguments，只能直接将当前 .Position 作为 EOS。
			file_stream.SetEOS();
			// assert: file_stream.Position === file_stream.Size && file_stream.EOS === true
			library_namespace.debug('设定 end of the stream to: ' + file_stream.Position + '/' + file_stream.Size + ', EOS:' + file_stream.EOS, 3);
		...
		if (e.number & 0xFFFF === 5)
			// for FSO
			library_namespace.debug('contents 中有此 locale 无法相容的字元，例如在中文 Big5 中写入日文假名。');
		return e;
	}
}
...
.
/**
 * 将编码为fromCode之档案fileName中所有不合编码toCode之char以encodeFunction转换
 * @param fileName
 * @param toCode
 * @param fromCode
...
_// JSDT:_module_
.
/*	将string text中所有不合编码toCode之char以encodeFunction转换
convert string encoding<br />

CeL.iconv('测试每个字元 abc あaいiうuえeおo','Big5')
...
set_Object_value('folderProperty','Attributes,DateCreated,DateLastAccessed,DateLastModified,Drive,Name,ParentFolder,Path,ShortName,ShortPath,Size,Type,Files,IsRootFolder,SubFolders',1,set_Object_valueFlag.array);//Files起为Folder property
...
_.traverse_file_system(FS_function_array)	省略path会当作所有Drives
_.traverse_file_system(FS_function_array,'c:')	c:→c:\→sub dir of c:\
_.traverse_file_system(FS_function_array,'c:\\')	c:\→sub dir of c:\
_.traverse_file_system(FS_function_array,'c:\\cc')	c:\cc,cc为dir→sub dir of c:\cc\
...
function folderFunction(folderItem){
 ...
 //if(t==folderItem.Name)	//	仅单层subdir次目录
 //if(t.indexOf(path_separator)==t.lastIndexOf(path_separator))	//	仅单层及次层subdir次目录
 if(t.replace(new RegExp('[^'+path_separator_pattern+']','g'),'').length<3)	//	仅3层subdir以下
  ;
}
...
filter:
	file_filter	仅单一 filter 时预设当作 file_filter, should has NO global flag.
	[file_filter,folder_filter]
file_filter	筛选档案, should has NO global flag.
folder_filter	筛选资料夹, should has NO global flag.



TODO:
限定traverse深度几层,sort=8,preOrder=0,widthFirst=0,postOrder=16,depthFirst=16
*/
//_.traverse_file_system.stop=false;

//_.traverse_file_system[generateCode.dLK]='initialization_WScript_Objects';
/**
 * 巡览 file system 的公用函数
...
function traverse_file_system(FS_function_array, path, filter, flag) {
	...
	if (FS_function_array === _f.get_object)
		// or FS_function_array=[,,]:	可以使用 Array 常值中的空白元素来建立零星稀疏的阵列。
	...
	if (!filter)
		...
		// filter=[filter,filter]; 通常我们输入的只会指定 file filter
		filter = [ filter, ];
	...
	testFilter = function(f) {
		try {
			...
			throw new Error(e.number,'traverse_file_system: 错误的 filter:\n'+f+'\n'+e.description);

			e.description = 'traverse_file_system: 错误的 filter:\n' + f + '\n'
			+ e.description;
			throw e;
		}
	};
	...
	while (!_s.stop && objBuf.length)
		if (item = objBuf.pop()) // FSOFunction执行途中可能将此项目删除
			...
			case _f.folder:
				...
				// if(!traverse)traverse=0; // 加上次一层: 会连这次一层之档案都加上去。
				...
				if (fc) {
					...
					// 因为档案可能因改名等改变顺序，因此用.moveNext()的方法可能有些重复，有些漏掉未处理。
...
//	send in file manager, list/open by file manager. 档案管理器/文件管理器.
//	Windows 档案总管检视. Views in Windows Explorer.
 ...
 *        2012/11/12 0:35:50 refactoring 重构 for sub function.<br />
 *        2012/11/12 17:48:34 置入 CeL.application.OS.Windows.file.
 */
function show_path(path) {
	if (!show_path.panel) {
		library_namespace.warn('请先设定好 panel: using show_path.set_panel()。', 2,
				'show_path');
		return;
	}
...
/**
 * 执行 directory, file 与 navigation 之 sub function。
 */
 ...
 *            FSO 路径。
 * 
 * @returns {Boolean} 因为通常放在 onclick 中，因此 return false。
 */
...
/**
 * 当 show_path.handler 遇到 show_path.default_name 时，会作为此 FSO 之 default handler
 * name。
 */
