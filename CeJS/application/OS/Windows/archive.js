 ...
 * @fileoverview 本档案包含了压缩 compress / archive file 的 functions。
 * 
...
function module_code(library_namespace) {
	...
	function parse_7z_data(status, log, error) {
		...
		if (!message) {
			if (library_namespace.is_debug())
				library_namespace.warn('无法 parse header!');
			this.callback(this.path, new Error('无法 parse header!'));
			return;
		}

		// TODO: 系统化读入固定栏位长度之 data。
		...
		for (i = 0, length = file_list.length; i < length; i++) {
			...
			library_namespace.debug('一个个猜测边界: ' + i + '/' + length
				...
				if (end[j - 1] + 1 < start[j]) {
					...
					if (match = fragment.match(/^\S+/))
						if (match[0].length < fragment.length) {
							...
							library_namespace.warn('无法判别 ' + header[j - 1]
									+ ' 与 ' + header[j] + ' 之边界。fragment: ['
									+ fragment + ']');
							continue;
						}
					if (match = fragment.match(/\S+$/)) {
			...
			if (calculate_done || no_calculate_count > 20) {
				library_namespace.debug('于 ' + i + '/' + length + ' 跳出边界判别作业: '
		...
		for (i = 0, length = file_list.length; i < length; i++) {
			if (match = file_list[i].match(pattern)) {
				...
				library_namespace.warn('无法 parse [' + file_list[i] + ']');
			}
		}
