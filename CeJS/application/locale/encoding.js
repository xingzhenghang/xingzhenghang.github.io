 ...
 * @fileoverview 本档案包含了编码，例如自动侦测地区语系/文化设定编码的 functions。言语判定ライブラリ。
 * @since
 */
...
function module_code(library_namespace) {
	 ...
	 */
	/**
	 * <code>	自动判别档案（或字串）之编码	文字エンコーディング判定を行う
	autodetect encoding
	...
	/**
	 * guess character encoding / character set of file. 侦测档案编码。现可侦测中日韩东亚语言。
	 * 
	 ...
	 *      accessdate="2012/2/25 9:13" title="一种语言/编码检测的复合方法（转） - 少华的日志 -
	...
	function guess_encoding(file_path, is_HTML, options) {
		...
		// 读 binary data 用 'ISO-8859-1' 会 error encoding.
		...
		if (typeof t != 'string') {
			// 此时type通常是unknown，不能用+=
			// t=''+t;
			return guess_encoding.unknown_encoding;
		}
		/**
		 * Unicode的Byte Order Mark(BOM)在UTF-16LE(little
		 * endian)里，它是以FF-FE这两个bytes表达，在BE(big
		 * endian)里，是FEFF。而在UTF-8里，它是以EF-BB-BF这三个bytes表达。
		 */
		if (t.slice(0, 2) === '\xFF\xFE') {
			...
			// 即使是用OpenTextFile(_.open_format.TristateFalse)，UTF-8还是会被转换而判别不出来。
			...
			if (!is_HTML
				...
				if (reading_length
					...
					library_namespace.debug('原先取样文本长度: ' + reading_length
							+ ' 过短，重新读取长度: '
				...
				t = code;
			code = undefined;
			library_namespace.debug('取样文本 (' + t.length
		...
		// 假如是HTML档，判断是否有 charset 设定。这个判别放在unicode之后，其他自动判别之前。
		if (is_HTML) {
		...
		if (!code) {
			...
			for (enc in guess_encoding.mapping) {
				...
				if (enc === 'EUC-JP') {
					// 半角・全角形[ｦ-ﾝ] 可能在以 'EUC-JP' 读取 'EUC-KR' 或 'GB 2312'
					// 时大量出现而造成误判。加以日本语文件本身也少仅用这些字母，因此割爱。
					t = t.replace(/[\uFF66-\uFF9D]+/g, '');

					i = t.replace(/[\t\x20-\x7f]+/g, '')
					// Windows 下， EUC-JP 对无法编码者会改成 '・' 而非 '�'。
					if (library_namespace.is_debug(2))
						library_namespace.debug('[・] count: ' + i.count_of('・')
								...
								1, 'guess_encoding');
					// 计算 '・' 占非 ASCII 之比例。
					// .02: 依据经验而得之 magic number 阀值。
					if (i.count_of('・') / i.length > .08)
						continue;
				}

				// http://en.wikipedia.org/wiki/Specials_%28Unicode_block%29#Replacement_character
				// 可惜 ADO_Stream.ReadText 仅会把无法编码者改成 '?'，而不会改成 replacement
				// character '�'。
				// TODO: '�' 的比例过大时(e.g., ratio>.0001 && count>1)则 pass.
				if ((i = t.indexOf('�')) === -1) {
				...
				// .001: 依据经验而得之 magic number 阀值。
				if (unknown_character_count / t.length > .001) {
					library_namespace.debug('看来似乎不是 encoding [' + enc
							+ ']! Unknown characters: '
				...
				// most_probable_code_changed 或 test_group 刚结束时皆需要作测试。
					...
					if (highest_ratio >= guess_encoding.default_select_boundary) {
						if (!(enc in test_group)) {
							...
							for (i in test_group[enc_tmp]) {
								library_namespace.debug('由于 ' + enc + ' @ ['
										+ test_group[enc_tmp][i]
										+ ']　编码类似，有时会产生误判，因此持续作测试以找出最可能之编码。',
										2, 'guess_encoding');
								break;
							}
						}
	...
	// TODO: 提供一种通用模式来处理单字节编码 - 俄语编码(KOI8-R, ISO8859-5, window1251,
	...
	guess_encoding.mapping = {
		...
		// 将 EUC-KR 排在 EUC-JP 前面是因为 EUC-KR 字码的字在 EUC-JP 中看来常常只是些罕用字，而非无法辨识的情况。
		'EUC-KR' : {
			// EUC-KR 的标准需要再降低一点。
			'ko-KR' : .8
		},
		...
		// 将 GB 2312 排在 Big5 前面是因为 GB 2312 常用字在 Big5 中常常是\u8000之后的常用字，Big5
		// 常用字却常常是 GB 2312 中奇怪字码与罕用字。
		'GB 2312' : {
			// 0: use guess_text_language.default_select_boundary
			'cmn-Hans-CN' : 0,
			// 偶尔会有以 'GB 2312' 编码的 'cmn-Hant-TW'. .8: 依据经验而得之 magic number 阀值。
			'cmn-Hant-TW' : 0
		},
		'Big5' : {
			// 0: use guess_text_language.default_select_boundary
			'cmn-Hant-TW' : 0,
			// 偶尔会有以 'Big5' 编码的 'cmn-Hans-CN'. .8: 依据经验而得之 magic number 阀值。
			'cmn-Hans-CN' : 0
		},
		'Shift_JIS' : 'ja-JP',
		...
		// accessdate="2012/3/17 20:27">俄语字母</a>,
		// <a
	 ...
	 * 
	 * @returns {String}code (default) 判别出之编码。
	 * @returns {Object}ratio object {code: 比例/ratio/可能性/possibility}
	 * 
	 * @since 2011/12/11 00:18:07 重构<br />
	 *        2011/12/28 22:28:35 refactoring: 重构以判别 EUC-KR。 2012/3/17 17:16:32
	 *        两段式判别：降低不常用字之比重。 2012/3/17 23:20:20 Adding Arabic, Russian.
	...
	function guess_text_language(text, language_to_test, options) {
		...
		unrecognized, character_count = {
			...
			if (old_length && signature_RegExp[filter]) {
				...
				// add_tag 本 lang 为计算所必须，不能 miss。但一一判断速度过慢。
				if (false)
		...
		// 按照特征码一个个将之去除，计算符合的长度，猜测最有可能者。
		remove_lang('en-US', true);
		remove_lang('en-US', seldom_character_count);
		// 因为 'en-US' ⊂ 'x-general'，必须将 'en-US' 排在 'x-general' 之前。
		...
		// 依各种常用字母之经验法则侦测/判别。.95, .5: 依据经验而得之 magic number 阀值。
		...
		if (
		// 判断已知的同时，未知字码需要极少…不过这在遇上符号时可能会出现错误。因此最好将符号排除。
		unrecognized / featured > guess_text_language.unrecognized_boundary) {
			...
			// 若 ratio === 1 但指定 language_to_test{}，可能造成回传与原先结构不同之
			// language_to_test{}!
		...
		if (i = character_count['x-CJK']) {
			// 由于 CJK 于各语言各有不同比例，因此加点比重至此。各比例为依据经验而得之 magic number。
			character_count['cmn-Hant-TW'] = (character_count['cmn-Hant-TW'] || 0)
					+ i * .2;
			character_count['cmn-Hans-CN'] = (character_count['cmn-Hans-CN'] || 0)
					+ i * .15;
		...
		for (i in language_to_test) {
			...
			// 加上 unrecognized 的影响。3: 实为依据经验而得之 magic number。
			+ (i in {
				'ru-RU' : 9
			} ? 9 : 3) * unrecognized;

			// 'ko-KR' 几乎只用한글(朝鲜字母)，为作平衡加回来。.5: 实为依据经验而得之 magic number。
			if (i === 'ko-KR') {
		...
		// 经过厮杀战的才当作有其价值。
		if (!test_all)
			library_namespace.debug('没有所占比例超过门槛，且可以准确判断的 encoding。 ', 2,
					'guess_text_language');
	...
	// 某种语言之字元数大于此界线阀值，即视为此种语言。依据经验而得之 magic number。应 > Math.max(.5,
	// guess_text_language.default_boundary)。若低于此，则进入厮杀战。
	guess_text_language.default_select_boundary = .9;

	// 要作为候选者之最低限度阀值。依据经验而得之 magic number。
	guess_text_language.default_boundary = .4;

	// 若无法判别之字元比例大于此界线阀值，则当作 miss 过多，无法判别。依据经验而得之 magic number。
	guess_text_language.unrecognized_boundary = .3;

	// 依据经验而得之 magic number。
	...
	character_signature = {
		...
		'cmn-Hant-TW' : "\u3100-\u312F国这来会们对与说学实经么样关应两体却将内党台总处观湾变声气权传觉儿写满万产数让乐医独带静战单济读轻绝区岁众边条争里举转灯画断广历价联显钱续双怀欢图随乡据亚称办严验参黄药证仅尽楼继专够妈欧脚杂恶槟宽县扩虚帮轿辞届阅挤奖巿汇吴盐摄译叠砖荡洒赚吁虫剂侠秘驶",
		'cmn-Hans-CN' : "这个为们发说时对业经过进开现产动长实种还关从报场两问电资员总样济无间军记华设题务书门应头东见队农义强际话尔达术战给结统儿论认决计组别领议处车气联办运导马调规广难变亚张传价观风权团项许质举专该级边选带标爱识视谈历确单赛营证转亲费让罗约极则听类织创众厂连职势县协备况师构严觉兴节远满续优责亿语纪艺护线乐仅维积热环复较乡红钱获讲响图离轻显兰请验劳销紧飞评负须坚贸读击继终访养银镇态苏陈阳刘财户预铁卫层围欢习刚虽诗货岁检吗讯闻纳绝险龙剧够试监汉伤买胜挥诉药适执树础编奖细额谁稳馆脸讨临阶陆审荣帮妇词顾宁简减冲泽脑卖杨训录网竞笔园贵换顿鲁错妈杀绍压灵购粮扩怀补惊测饭块丽杂谢顺汇庆练叶损贫钟孙异萨岛烟扬库针纸伦鲜吴矿韩遗驻敌灾败签戏伟输赶恶阵寻归轮诺纷齐币坏奋拥钢莱渐赞释吨综桥误弹疗梦枪绩虑乌绿邓厅译毕贝择赵订圣丝摄鱼违盖谓彻启夺顶课载仪贷贯盘晓诚宾谋摆贡辆摇纯艰挂乔爷宽宫锦债筹亩灭诸阴抢罚凤卢软圆墙袭倾缓弃邮烧辉颗趋辑郑闭隐鹏鸡岗剑鉴缩跃键册赏偿穷惯坛润络览劲贴忆储亏蓝洁饮龄闪涨净忧犹毁闹纽肃玛废贺镜缘页叹绕骑鸟舰迟暂宪阔辈烦沟轨档码迈铺赖饰绪颇汤辩询扫辽伪荡浓纺闲递帐剂迁邻锁锋腾纵凯赢罢腊厉丧冯铜狱侧谊涌频阅炼贾骗侨篮纠绘驶兹挤胁诞滚详阁俩骂钻拨畅轰尝赔鸣丛颁劝渔愤颜滨扑扰驾脉盐陕烂脏尘诊牵摊闯飘驱踪锡缴丢赠聪纲吕拟柜饱韦栏胀吓苍唤贤疯涂怜抚逻厌弯贩胶锅窝锐牺莲轿贪挣滩娱夹逊赋恼诱悬缝辅鸿帅庙雾仓岭咨舆侦闷赚钦惩辖喷猎萧绳浑艳卧冻巩庞笼谨铃兽谱绑肠饿阐祸顽纹肤纤颤裤挡婴驰赌撑鲍韵岂砖饲齿缅颖纱绵滥兑壳谦颂硕骤绸谅睁贿缠邹谐饼苹骄沪隶谭皱铭肿泼衔铸冈鹰勋琼聂拦狮驳锻凑鸭绒坝灿衬颠鹅讼恳钩耻骚蕴叠顷链贼赐饥斋坟钉岚谴栋渗妆垒渊耸仑轴娅诈讽饶颈厢酿绣鹤枣焕窑捞帜谎缔钓讶厕懒瞩坠哑胳桩壶荆晕轩账垄娇辐侠伞搁钞啸溃谜揽舱稣垦贞挚浆蜡沧贬俭烛瞒铝帘宠诵蚀垫铅崭哗赁镑鸦竖龚畴缆荫么缚潇阎枫钥谣肾秃拢炕驴斩嘿澜浇腻粪汕萝邢泻谬窜驼涩驹瑶蚁纬瘫杠琐濒烁讳虾赣闸庐闽呐碱搅葱旷虏锣哟咏捡沦呕鄂钧绅挠掷缉矶凿缪轧毙颊龟贱搂鸽呜诫觅烫莹秽酱尴酋捣匀锤缕聋侣涝侄镶阀歼删窥尧馈冉芜浊赂袜睐缀娄盏骆胧拣绎屿贮坞讥汹眯筛绞韧瑛诀赃涤瘾氢闺诡矫蝇痒婶窦洼谍祁跻赎晖酝盔挝骏窍悯邯掺翘蚂氨恪铮绰锈鑫彝滕铲橱骥骇墩鳞筝绷缎惫绥惭苇锯驯颓凛诧泵憋铀涡檐铎茧辫嚣溅樱笋皖钮沥雏黔疮攒捂痪祯棱荧彤炽榄诬鲸绽篱咙娣拧淄敛颐搀毗娴喔栅绊笃桨橄颅谛浒镖绚琅骡辙锭诅茬绢帧笺噜韬蔼锥钊渎鳌屉羁倩饺辗槛忏砚唠擒钝慑驭桦蓦贻酶恺荪袄嬷蹬禀馒荔浏匮叽莺峪锄轶悍纶坯殡怅葆滤陇肮奚钙鳖辍厮捅荟芊缭栈椭诃镀踌潍铉钾谤钳毡诲禺郸癫诏宓嗨绮秸谕锌诠鲨蝎狈巅缤纫镐辄衅棣锚浚攥萦哝侥鹃亵跺诣涟俨驮谟焘袅芍槟崽佟秆鬓匕鳄粞馅喽姗哧骋陨炜鸠赘讷呛囱妩脓缮鸥啧茯峦桢绛驿雳疟舔傣撵镯铐帼沅赈馍诶脐涧跄鸳钗泾铧媲诂渍讹诽纭呗祛懋隽箫敖镰钠饷谚珑泞鳗觑谆锹讴蹒窿掖饵",
		// 在香港增补字型集2016里面，有收录了22个香港常用字形与大五码所用字形不同
		// 原字符为：兑悦捝敚棁涚税脱蜕说锐阅媪愠氲煴缊腽蕴辒酝告。转换成的目标字符为：兑悦捝捝敚棁涚税脱蜕说锐阅媪愠氲煴缊腽蕴辒酝吿。
		// 'cmn-Hant-HK' : "兑悦捝捝敚棁涚税脱蜕说锐阅媪愠氲煴缊腽蕴辒酝吿",

		// 加上 日本语の仮名: 平仮名+片仮名, 片仮名拡张,
		// <a
		// href="http://ja.wikipedia.org/wiki/CJK%E4%BA%92%E6%8F%9B%E6%BC%A2%E5%AD%97"
		// accessdate="2012/3/17 14:6">CJK互换汉字</a>
		// 日本の文字コード规格JIS X 0213において人名许容・康煕别掲と呼ばれる汉字の一群と互换性を确保するために収录されたもの。
		'ja-JP' : "\u3041-\u30FF\u31F0-\u31FF\uFA30-\uFA6A発権対戦経产関実済気働労変図増応伝続価众団総収说験様営観売両齢挙拡満仏単塩戸値児従鉄効検廃歴処県栄楽転囲円歩険込悪豊撃択読覚歳壊帰払辺绝専継汚雑絵薬录乗繊拠頼焼剤圏颜縄蔵厳扱摂弾鉱脳軽隷沢仮抜庁恵郷暦亜闘訳巻镰臓绿縁稲銭譲釈浄掲勅悩氷遅窓寛覧勧拝启枠揺聴駅騒戻賛陥缲犠駆粋黙奨霊顕歯壳雕隠猟覇瀬縦惣舎斎漑竜斉诏懐阅舗嬢剣獣娯姉鶏廻挿穏渋涙鋳畳捜渇歓菓锐铳駄桜呉畠咲峠滝戯钵搾鸠曽髄錬亀呪鸾酔雰蛍俵槻谣姫倹頬掟砕竖脇袄徕讃竝壱煕埼醤塁绶醸呑塀勲炀萩崋栃讐倶晔渓渕罫锭喩匂诹鞄鴎泛",
		// CJK general
		// CJK 各语种通用之标点符号与常用字
		'x-CJK' : "\u3000-\u303F的一是不人在有我了中为以大之生个他上而也时到可年自地要能那就于出子家然成著如过得心下多作和所后十天无都情民小现政因此当事前去种其行意方文日好但想发性者分看三法美起只用最她外本长同你开理主已没面代动间及力些二重由定道还新里高化女又共己进从或爱使公感正世相知更等明全常问立社老很教活何书物工并再四表身果点才真把度头回向至走受风像什合见水业次界许少统题位五月台手几被名平山必选光命九言安加目提结它员部花每太路治则任机先望便亲入义话流解白各论认场难今交视建未百利做原期第车色诗放形深远市电孩育影北八往反口神曾念金别比特空失师华通夜清求近制接给七即眼直信且品非报识门记象量落展思改该打院海六西运千谈候完式母取呢整际字程考张务持步父切造南病资议强质决容听死虽喜指东音计语基甚存若男故示管设极保似调始离告类具友系林仍令境府除需快精根片件苦值红吃住态须早英笑连另引易血否导星婚树首热段般委局史究乎梦夫马土术军较愿格希确亦推半研香越达响足依朋角纪致消终竟永节校标刻构火势包待怎服痛系素商层环韩卫歌青士低伤免充集项领吗德例约闻群食收族渐策陆木细规底雨园试房草古增密找景创黑兴功晚福阳久剧倒斯悲况照突坐追息善科居费元庭叫春拉笔破级排案注亮效习维限责官富准演初港组属施王执室康复留简陈客班益街异假怕查协野灵投承球职旧跟微艺阿司幸尔份杨石背送飞丽率紧李站吧零备播革句绿请适支讲尤抗超啊城预供奇获负浪曲穿篇云助派省阶只团害算克河温谓讨惊冷良财借寻急答评顾龙止配忘愈味某衣护停判休呼谁均散买馆困积临诉课检秋退固压农布短村版妇营醒健脸银印唯宣律换担缘号述宁忽技朝油皮线叶守料源装透既逐养修烈眠睡纸贵靠戏拿归状企列忍潮私讯遇型坚沙乱差采脑漫窗菜掉脂唱抱汉雄察拥童端里恋概余肯速酒虑互威忆洲礼访默刚恐户模激蛋迷露典疑游劳妳岸忙武防升患掌织趣努授握江沈险午块遍错烟移范危症缺避阵铁鞋额屋左脱舞途乃座纯补遗哥店探榔玩冲章吸围席罗予周暗爸票卖寂波狂禁豪择田纷莫闲顿征括析泪略触减肉钟顺隐亡央尊弟批击弃淡湖藉降页副彩挥熟束疗练翻虎享含延慢忧折制顶倾划尚税萧坏志怪普洋诚谢鲜偶优弹荣录干冰词雪孤您智索酸繁苏释针厚圆惜救沉祖跳发序毒浮训君器妻架编偏楚泥饭柔牵毕贸夏厂敢皆哀寒欣置销队刀挂杀遭旁船鼓胜卡堆弱植兰右彼癌登竞飘堂妹岛床染渡警贪鱼刘桌沟浓巴宪欲毛牛丝复凡吉喝域材股仰尼招麻刺挑旅暴盛藏诸刊套媒摇败朵附昨耳胸货辉隔介吹抽映秘纳丰轮帝彷愁泰衡跑乏奔幻怨懂昏牌筑荒闹孔恨晨睛误赶齐垂宝惯摆替茶融赏骨尘宜州厅佛摊涉绪缓纵聚镜鸟姻扬测牠硬缩谷趋迁射干桥距郭占尺敬枝混灰狗竹罪鸡餐伸嘴巧彭献磨籍壮控杜毫牙载雷颗佛兵培审旦淋盖奋穷罢赛陷颇饮俗婆幅暂曹杯滋墙监糖胆庄阴伦尝宗扰敏款斗龄俄抬柯残凉涌番符累蓝闪雅孙板森碎松丈京丧拜珍甘迹返促哲喊巨拍稳贫付估尽凝启妙旗升矿耶胁辛兄哭废暖横犹圣菊貌赖雾仔勇寞封洗琴诺辩周麦伴幕叙涵裂壁弄慧焦痕薄裁回邱障坡季净疾皇肃描核爬莲订仁坦奈滴灾瑞盘输逃遥魂姿彻搬炉盼祝稍胞踏迎博姓姊寄干幽末犯督稿肩脉贡阔饰塑泛溪础米售孕川币怒肪裕冬匆宇忌忠旋洛烦粗芙岂跃颜励圈奏悦慎拒擦晓烧秀伯佳哈戴按珠志豆邦频鲁咖啡埋堪措枪洞灭滥碧糊署赞贤郁鬼布娘崇损操殊疏羽软逝迟呈抛振璃盆瞭签网茫违闭伞兼哪奉径惧拐抵滚煤爆脏航袋详辈迫醉阁震庞刑塞尾屈拨拟游粉艰覆谋购迈邀隆伟偷亿夕姐婴尿幼徒恩欲扮浅疲绕荡诊贴串余倍剩卷呀峰巷循抑敌杖松汗滑湿炸烂玻系蝶猫贩逵邻醇镇陌陶妥弯攻污润玉碰躺陪丹乘仙唤叹坛悟扎托拔柴洁肺肠苗葛苍辆锁冒宫悄悔搞泄甲跨召怜插摸昔冲沿添淇涛猛甜申瘦绵耕诞譬乳并割勤惨挣液燕粒胡舒袖贬贷赴跌踪辑辜逢饱饼魔占廿惑愚庆戒枯瓦瓶翠胖荷铃阻鸣仇坑尖挖拾叹泽兽祇窄耐询宾赠丁契奶宅宋峡徐舍斥昆梯柜乌狱盟稀笼蓉衰赤逼驰啦悠凭扫斜朗歧氏漂潜碍绘腐菲谨迅递腾准匹吐嚼悉椅棉朴歉汽渴痴祸穆缠骂耀腿芳蒙袭贯锐钢钟雕侵杰仪偿奥宿惠捕撞株玲盾肥胎腹莎蔬谐邓酬骑慈扭援摩撑晶柳洪涯燃赋赢面伏册妨宴愤拖晴淑托辱驻倡傲唐夹愉扣抓桃棒污盏眉纠纲脆烟董衷逊采剑叔吟呵吓庙憾挺搭殖渔牲狭碌贝铺键丢厌孟慨披挫卷晃棵沦漠猜盈矛绍羊翼肚蔡鼠佣丛娱寸岩岭帆廉闷悬扇拓掩朱氓灿牺筹纹罩聪腔荻赵迹躲辅丑鉴凤仿俱儒冠刷娃宏恒慕扯捐掘抚梅汪泣泊炎络聊胃芽赌辨郎伊劲吞墨宙寺库弊恰恒恕慰暮栋涨牢祥稻纽绩缝臂舍荫薪券卑婉敲斤昂畅柏栏欺毁泡泉浸溶溜烛狠矣码翎遂井俊债储勾圾垃弦恢恼捉捧抢晕枚泳渎熬矮致肝芦觅郝炼黎齿伍俯凌剪劣嘛夺宰挽曼梵殷氛浑澳瑰疯磅肾膀膜芒蓄虔躁逆遵雀颠馨骗鼻傅催削剥勉勃勒厉孝扶攀敦柱梭澄泼灌瓜畏租秩羞翁胀肤蝴裔谬郊霜丸倦劝吾吻喃嫁嫩屏抒桂汤漆盲碗稚紫综腰舟芬蒂誓谊谅誉轨遮铭驾侧呆喧喷姑崩幢截扁抹扑撒桑沾溢漏焕睁碑筋卧蛇蜜谦郑钻骄鹤鹿黯亭叮咬喻嗔向塔姆嫌寓帽戚扛栅玛甫卢硕箱虐贿逸锅云伙伪傻吊堡填媳宛愧恳拆撤挚暑欠毅津溃煮畔眸祈秤糟缔罚膨菌衍讶迪酷铜锋陋脏侣咳喉啸冈岗怖揭搁携晰朽沛浩澈玫琐痴皱秒窝粹扎罹耸莽裸夸谜辰迄逻邮辟雇颤伐冻凯凸刹吵圃堤涂寿娇庸廊厨恍抉拳旨枕榜浴滔疫疼碟绽肌臭舌芭葡薯亏注赫轰迢鞭骚乖倚哨哗尝囊坠壤巾帐弥怯惟斗旷桩歇池淹淫潭蒙涩滩烤焚牧矫稽糙缴罐翅翩萎蔽萨蠢裙谱赞舆阀驱鹅鼎僵劫卜叛念乔墓垦寡耻拚掷斩栽楞殿渺漪浆炒畴瘤瞧肆膝艾莉萄蕾襟诠贺蹲郁骤亟仗俩允凋吼喘址姨媚媜履役徊憬抖捷摘撰搅昭汁熙爷眷砍筒耗肖脖臣藩蚀诱踩衔锦隙魁乍伫佩函哦哑啼嗜嘻噪坎埃塌垒夷奢妄尹屠帕厢彰徘慌挡攘斑曦橱渊溺浊煞爽狼瞎祭禽秃秉稼笨笺绣翔聆胶茁兹萌蓬虹衫襄讳跋辄逛遣邪霍霸靖顽颁魅僻卓哺唇伙峻惹戈拂撼昌棘棋歪泌淘凑烘熄烁玄莹痞癖盗睹瞻砲秦获竿签粪耍聘膛茂蕴蚊蟹袱褪证诵咨谣趁躯酌锡霞韵鹰幸仓兢冥卒咯唉嘉嘲坷坟奠娜寮巡掠摔摧框梳棚梁溯煌苏眺缚绳肢肿腊舖蕉𫉄蜂裳裹诡豫贞贱逗辽钉饿魄鸭麟丘乙兔冤卵卸卿吏吱啪嗽囚坊宠屑岳帜厮弓征忡惚挟挨擅昊曰毋澎焉炼珊瑜瞇瞬硝祷颖簿耽脊菩葬蚁竖艳跪踢顷颈饥喂骇鸿侯倬仆侨兮剔勿卅呐咎嗓噩妆屡弗怡惭悯拘拼拭掀捣昧札梁梢械棕泛沫淌渥渣炭狮琳疆砂碳穴窃笋糕绊绮罕肇脾芸茅茄苞荆菁苇藤虞蛮裴猪蹈辣闯阮蓦胡仞佐刮匈匮厄咄啃嘶嚷壶妒姜婿嬉帅忱悴凄愕憔揉搜撕敷斧旺枉椰枫榕毯渠灼炫犬瑟瓷町矩砌祉笛箭簇羡耘耿艘芝芹萍蒸姜贾赔钓钞锣隘颊饶驳鸽侈侮僚剖咒喟嘟呕咙圳坤寨屁尸汇恭惋惘惕憧扔捏摒捞攸曳枷桶沸沮洽淆潘溅烹煎爪犁瑕甩疤窒笙笃粮捆缅茧缤腺臼苟葫蔓蕃蛛趟踽迭逮酣铎阖陡陵雯韦鲸黏仲局倘偕家兀凶兆凄匪咕咽哩喳嗅垢奕奴婪寰峭崖巍巢徨惶栗惫惩扒抨拱拯揣擎拢斌晦暇柄棍椎榴桧汲涓湄漾潺淀潇弥烽烫狄瓣愈皂盯眩磋谷竭笠帘粥纺紊缀苛茸蔺蛙豁赐蹄逾遁钗钝霎餽黝亥仕佣傍凹刃匙后哼冢妮姥嫂孰孽尬尴庚廓徬怠憎懊忏扉拙掏捻揽敞昙栖杠樟氧淙渍濡炊炳焰熊炽犀疚盒瞪矗矜磁窜笈纱芋芯苓蔚虾褐蹊跷辖迥迸遏厘镂阱陀雁霓须鬓丑于伺侃偎剌克匠叉叩哎哇咪哄啜喂嘈垮堵垫姪奸嫉宵寥尉屎屐屉屯峙岚庇庐廷恃恬恣悼惆懦揆敝杞杉杆樊橙歹氯汐沃湃漓渗浇炮烙煽猴猎珮畸疮御棱窘筷粽绑缄缸羡翱腕膺艋苔蜿蜒螂衲咏詹谛谎豚贮轩遐遑遽鄙酵铅凿驼黛丰乞佼俐侦僧兜冀凶厝叭吆咀咱咻喇嗯噬埸妃妓姬嬗岱峋嵌帖吊彬怔恻慷憩懵戍戮扳押挪搥摹晤暧曝杏汝沐汰沼涡溉涤澜炙炯熔燥烬狐珂畦疵痰痺皓盅盹眨睫瞄瞳磷礁秧粧绒翰肋膏范菠芜蕻蜘蟑裨诈谭踉踱迤逄邃锄阪隧霉肮骸髓鸦仆佑俺偌倏凰凳划劈卉厥吮呻咚哉咫啤唳啻啾嗤嘘噎当噜咽垠墟夭妾姚娶娴孺寝峨嶙巳庶庵弛怵恫惦悸惰慑戳拈拎挨揹搏撮拦攫旭旱枒棺桠榷榻榖橘钦氟汶淤涸淮湘漩濯浏瀚沥灶爵琉琦疡瘴盎瞒瞥蒙砸硫祗稠洼笆籐籽粱糯絮绰纬绷才坛罔臻芷茱莞蓊蕙薰荐蛾猬蛰蠕袍赎趴跚跛逾蹦躬辐逅邂邸酥锢镕镶隅霾靴饴驯魏鲤鳞莺鹭丫么乒伕佃伶俎刁勘募勋勺匿吭呎吠咐咸咦咧啥唾嗣呜嗡噢嚅埔尧壑奄奸娓娥娼婷妪寇屹崭屿弘弭彗徉徙徜忿怦悚恺懈憨戊抄捺据搓搧折敛毙斟旬晖朴柚桔栗梨棠楔槽槛殉汀汾泯涕涎淅淬渲渝淼滤瀑炬熠熏爹牟牡琢璞琼瓢甬症皎矢砰砚磊矶窑篷箫簸绚绻羁胰腋腑腮臆舜舰刍莘蒲蔑蚋蜷蜗螺蜡诘谚谙贼赘踞蹂踵辟逍迳腌衅釉锻铿闲合阎阡霖靡秋缰千韶饲馥鲍鳄麝丐乓仄伽俑俟倥倔倪冕冶冽几卦卯叱呷咆咋哮唬啁喏嘎嚏囿坪垓埂墅壕夸奎妖嫦嫣嬝孜孵寅岐崎崛崔帚帛帙并弧弩别御徽忖忪惺愫慵戎戛戢戾拌抿拮掬捩掣捶搪撇掳擘昀晌暨暱曙朔朦胧杭杰柢桐梗枣椒榆椿桦橡檀殃殆毡氤氰汔沽浦涅淳湛渭湍滇漱澡泞濠濒灸燎燠狎狈狸猖呆獗狞琛瑯瑚瑶璧珑瓮亩畜痔痹瘠瘪皈盱眶睦睿瞠砭磕砾祠禧禹稔穗秽穹窦竺筐箸个箧粘弦绅辫羚羲翡翳聋腥膳脐舐舛舶芥茉苯茵萃菱席蕊薇藜蔼虏蜻螃蟀蟋蔑衙袁褂褶诀诧诣诫谕誊谩谴豹貂赂趾趿跎跫踮蹉蹴躅辙迂迦逞邵醺钳锥锵阙升霏靥鞍鞠馒骼鲑鹑咸曲鼾丕亨伎佰俏俘倌倨俾傀儡兑冗凛刈刎刨剃匍匐匡叨咤呱咿唆唧哽唏啄啖喀喋喑嗝啧嘹恶噙苏圜坍埕埠墀壅夙娑娟娩娉嫖孀宕宦寐岔嵯岳巫帷幔庖廖彀彊恤恂恙悖愣惮懑扼拄拇捎捆扪挲掂掐揩揍揪揶揄撩撚擂挛旄旌晏朕杳柙柒桓桉栖桎梓梏楠楫楝楣椽榨槁榭槭槿樽栉檬柠櫺殄毽氮汜沌沂沏泄浣淦淖湮溲滓漉潢澹沈瀛烯煦熏燄牝牴狡猝猾玖琵琶璀璇璜璨畚痊痉瘀瘟痈杯眈困睬睽瞌碘碉禀窈窕窖窟窣窸箕篆簌粕粟糜纫绢绸缎缜绎缱钵羔羹翌翦肘肴胥胚胝胭胼胯胺脯腴腧臊舂舅舵艇荐茴荏莓萋菡菇蓁蕨薛藐藻蚓蚵蚱蚯蛔蛤蜀蜉蜢蜥蜴蝗蝣袓袤褚褓褒褥亵觑讼讹䜣诛诬诲谏谑诌谟贰赁赭蹋蹙躇蹭踌轧轴逡遨遴迩邹酢醋醛钊闵暗陛隄隳雌雏霄腼鞘韪颔颚饨馑飨驹髻鳟鸢卤麒麓麸麾黜黠霉亢亳仟伉仳佗佞佚侍佬仑俚俞俸俪剉剜剿勋勰匾叟叼吁吋吝呃吕吩呤哂哆唔呗吃喙嗟嗦嗥嗒嗲嘀噗噘噫噤哝嚎嚓囝囤囫圭圯坌垣垛堙塘堑壹妁妞姗姶婢媛媾嬴孑孚孳宥屣仑崧嵋嵬嶂巇岩幄幌幡庄庾弈徂徇忑忐忮忤怍恿悱愎愆愿恸戌戟戡扈拗拽拷拴捍捌挼掮捸揖揠搽搔搐摁搂掴搴摭撂挠擢撵擞晒晾暄暌暸曜曩杌杵杼柬枸桀桅梧梆棹楷椹榛槐梿樵梼橹栾欷欸歆歃欤毓汛沁汨泓泅泠泫洫洒洙浬浠湎湲渫湫溥漳潦炷焊烷焙煨煖燧爝爰牒牖犄犊狙狷猥猩猷猿玦玷珀珺琪琥瑙瑾璁璐瓠甄甭宁甸畀疋疙疹痂痍痣酸瘁瘩瘝瘸癞癫盥眇睇睨瞅瞟瞰瞿矽矸砥碓碇碣碾磐磺祀祺祯禾稷窠窳竣笳筑筵箝箔箍箴篁篡篾簧簪箪粲糗纮绀绞绫纶绯缙纤縻纂罂罣翟耆聒聂聿肄肓肛胱胴胫腆膊臃臀胪臾芮苣苜苑茹茗茨茜莠荼堇菰葵萼葩葺蒿蒜蒺莳蓿蔻蔷薮藕蘑藓蘸蚪蚤蚌蚣蛀蛭蛩蜓蜈蜃蜚蝌蟆螫螯蝇蛎蠹衢袂衿袒裎裘裱裾褊褛褴覃觚讫讷诃诽谍谒谤豇豉豕豳赧赦趺跤踝踟蹇蹚蹩轼迓迨逋逭遄郢酊酐酗酖酡酩酮醐醍釜钩钿铣锌铗锈镀锚锤锹镐锤闰隋陲隼雍雉雩霆霹靶靳鞬韭颐饵肴糊餮饕驭骏骛骢骷髦髫髭鬃鲭鳃鳗黥黧啮国来会学于着与当么体没机将区点内万并几条党数声争参巴却查写装划断采双称医游随尽黄站欧号奥跟份按股范愿卡画税独拿抓吧担某属哈届状牌俄售靠温藏哪假宝怕楼托献哥静搞篇勒佛乱您缺旧湾沉套丰穿脚澳跑啊礼脱姐尤掉姆伙默残迹麦灯励誉虚践泪壮径潜辞触横胆凭涛浙峡猪虫蒋叙厦碍恋凉盗寿湿晋惨炉猫遥浅尸抛悦弥惧携滞剥厨窃禅狭屡嘱厘彪蛮芦粤彦刹祷枢躯斐淀禄蚕阜昼堕丙渤郡茎寝栓硅挟嘘踊殴邑蔗匣柿蝉璋栩沓嵩堰斡狩柑荀祟甥岑泗酪丞萱坂劾舷熹雹弼痢槌亘菖皿塾冢羌筏虱糠柩晁汞妍昕砺酉胄嫡盂奘倭浜虻鞅赳咤髯橇厩妊怏娠隍痘胤宸漕黍疸槃瞑桧蟠祚癸渚畿檄杷蛆偈蛹稗壬疱襁僭裟柘袈俳岬笞蓑悌靼菟珈腱砒杓弁瞋菅噌砦斫笏衾疥槎桁徭肱沆苫毘霰羯甑藁掴鬲蛉椋楮堀瞽俣愍樗箪檗隈恚缶蛸薨禊笄楸笥搦祓旻磔拶尻祐襞魃梃袴跏襦笊埒袢埴悛邨璟掾甍貊𫓴舳毬俊嚢酎檎羂糸冱𩽾麿𩾌修丶珪霭歔巣鬘髪抔跖憺昆薙媪絜谿帯昉㭴燐膣荘𫍥捗疎麹鞜靺叡黒胄鞨溯堺隣徳笹侘潟徴缮広渉穽穂靭瑠倣毎辔粛椁楯楢碁梶查壌晩圧襕畑苎剰辻痩农组诸宪広护岛协输财阁备制职贸织圧帯积线鲜异课准侧适叶复减银负徴违",

		// Hangul characters frequency table
		// 加上 한글(朝鲜字母): Hangul Syllables ハングル, Hangul Jamo ハングル字母, Hangul
		// Compatibility Jamo ハングル互换字母
		'ko-KR' : '\uAC00-\uD7A3\u1100-\u11FF\u3131-\u318E',
	...
	// 次常用字, 罕用字
	seldom_signature = {
		'cmn-Hant-TW' : "厦践纤艳裤懒表祂怅档讽俭沧瘾缕酿板凄萤巨台汹辗闽堕玮颂冯嚣晒览酝酱颓鹃晋杆唇蒋捡壳禅簷萝衬馈瘫腻苹飒台篱陨韧痪萦搜卜蝉雇咛瞩窥累莱郤阑颅龟叽嘱峦怆拣昼滞涟滨泻葱蜕蚂啰筛纾舺茎袅袜阂闺鹏雕出滢篮缪铝钥饥碱斋亘俨吨垄妩嬷掺旳桨樱荦缆讥跄蹒迺阐骋删呛娄彦撷斲殡涧痒纭缉聇蚕诅诤谲踬逦锈镌铸刮饪馋驮骆骥魇铲匀啣嗫坜壸峥嵘巅帧厕历枢榉荧毁玑皑筝蓑缭蛊衮诩赀躏辍扣钙铲飙驴鸶傚偬侥唠婶孪恹扞抡栈榈氢氲泾狰犷痳睐礡禄稣篓籁舱荖荪裥訧谧赃踊踯轶钧钾铨鉴阕隶颢馊鸪鹌鹧龃龉啯哒啭呓囵塭坝嫄荫懔怼懹搆摃搟搀攒敻斓朼楬椭橥歛毵泹狲疪疴祼篑粤纨绐缈缥脍虩蝼蝈袪讵诙诟谔謢谪賸踄踼跻轫轭辕邺钮锟铄闸阉雳韬颦飓馁馄駴骓骡骜骊鲨鲥鸵鹄冬龊龌伧侬傧傥兀匦呇哖哟唢噉哙啮囓囱埆垩堭坞墙奂娲娆嬏娴屘宧屧屦崁岖𪩘帼庛廌廇庑悭戚怂扡扙拫掅掼挞拧摈朸枻榞檥棂殒瞉沬溯浃涣渧湿沪卤濔潋灏滟烃焄玆瑧瓯痌痫眬砺禋禢秏穈筚箩糢团绌紽绛缁𫄧缛繖缨翘芵芧芔莴葰药莅蔪参蓟蚬蛱摆觞讦讪诋谆諓谥哗贻赊账蹰跹蹑轾輀挽辘迆迼遯邬酦鈙钴铬铤锰镁锾镑镍鎗链镖镰铛镯镳𬬻銮阊阒闿阨隽霁巩韾顸颉饯馅馔餍驷髅哄阋鱿鲫鸲鹊鹗鹜鹞鹂黩",
		'cmn-Hans-CN' : "鸯迩掳飓沱辘痉缰谙伫唁砾萤闵跷馀抡玺铿鹊挎烬锵狞撷潼蜕鲤镁诙阙蓓犊抠挛炖涣臧摞钵咔谒蹑侏惴钰饪嗳惬哒酯楂苒谏噶佯犷鸵篓撬缨讪筱瘪荃玮铨斓镌癞卞轼锢滦赡阑岖锲泸疡铢硌惮蜗悻鳍鹭猬辕卤侗冼栾腌腓馋瓮掰牍缄馁罂皑蔷撅芩谧懿镍珅躏锰镣礴熨忒圩啬仃囔嗷鄱侬嗬赓腼籁昵濮诘珥铵痫烨崴忻麋夯蝠鹦汩擞骠溟怆榈咂赊栉谑怂煜燮骅镛诛孪莆涮蛟恻粳捋奂呸霭镭铠皋芪蛰徕鹉淞蹿蝙脍蛐胫镢峥缜鼐嗖阖晗飒岌荤箩刽疽绯闰蓟唷诿褴涪泱恸漯鹳钨阂篝飙澧啕菏挞洱舀褛硒偃枳筠耷珏绀噼掇狰碴鸾跆耙裆猕昙铛皙馏暧柠龛颍觐矾啷舫嵘闩钛诩湟琏粼绫扪堑懑郴岷闾栀遛咛蛎惇匝剽弋潞蟾驷豌铬阉坳濂嵇幷婕牦髅掸诋萸苎涿姣铤飕杈舸獭嗫剁枭骁绉颚鏖踹煊绶搡酰飨邬颉砧楹耄饯滂鱿鲟琮镂缥蹶鲻妲绺珲谩芎蠡颌嗑飚郅晔爿摈癣燔霁赝孢幺噔骊颧殚郧唢啐蔺荚蟒辊虬仨纾稞粑唰潸蛊缫俪蔫靓塬谀汴嘁睑钚颏谄铡遒珩昱崮豺罄颔酚莅纨兖佤铣殓鹜啮锃饨骰蛳缈旮鳝荞纣邋旯膘魇鲫翊敕婊犟馄氦墒咝珞茏昶饽膈铂斛孬烃傩瓯哐旖灏銮骞镳鞑垩镕鹗偻遢鹫旎鹄枇睢鄯嘤驸锨滁琨蚜伢呓痼锷衮砷睾糅沭锺恽荥钅嘭趄腈耋孱圪龌龊垅钺赅婀谲垸畲岫烩缙娲铆鲲尕倜聩荠嫔硼刍熵圻谶骈佘鳅钴猗绌衩蜇闫僮啵痈鹘邳蹼涔龇蝈夔藓肽岜愠焱浃嵊铄橹滹痨邈贲诌莒訇轱囡洵坨咣藿玥诤棂苡爻佻怩鬟浔颦篙锂砣苔誊砝叁轲鹂佝陂贽邕犍暹趔囹柞忸芾戕婺栎骧锉艮葳炀揿溴薮鬈邝妪蓖圄馗讣饬姘鄢桠勐觎侩煲锑矍噱垭侔郦剐殒鹑秭瓴囵蒌蒯畹叵觊钎歙膑谥谌胛贰镉犒苕滢诰痿谪脘坻钤龉蜍迳觞龃崂傥擀醴澍橐诨伧峁闳珉戬骝溧逶磬碜儆鳏薏鳟龋謇溘晟杲殁靛祜姹焯喁鳃篑玑羸抻瓤嗄诮籼茔欤辎陟嗵乜豢螟傈蒡鄞鸪谵沤氲鲑谗苷蕤颀哔玳耦烊铰忾钼秣旃绦嘣狒馊枞苋龈琰讫螳牯翕悒呲纰恹鹌涞醚婵讦饴舢恁炔钿鸨骛麸苻娆阄鎏郜苁钡榫珐蕲琬璎劭踯锏锴秫擤濑褡獾啰挈幛呶轭钒氅弑邺唿鹞痧钹钏癜罡磙鸢韫甙僳耒邡铳焖椁缛瑁陉阕峄瓒媪腚屺骜汊绔蒴滟唑儋辔谡阗蓼洄轫螨晷讧铱镫髋侉缦锶玟嗐樾趵岿洮庥癯煅蘅砻鹧轳缢罅溏殇腭馑掼巽垧榉狍邰茆玎绾畈辇鞣哞僖鲈钜屙鹈呦妤綦咭诟锗疝囗渑荸鬣捭獐砀酽诒缃鹕谔芨娌馔泔靥峒锒踅撺凫鼬鼹艄镗蹓芗羟矩颢獠妯枰沣糌咩骐叻篦橛芡臬蓥啭趸瘘疴馕阇疣廪灞阚朐莪锛碚筮姝膻鲇瑗荨闱郗箧墉怄荦竽笤疖麂逖疃鳕菽檩雎枋觥蘖扦茭莼蚩锟薤埝绂偬枥韪匏悭辚亻仉樯薷皴曷稹鸩纥鹬镊痱貉沔峤郓鲅撸隰毂桡仡馐邛珙喱鬻咴绻泮讱岙郯黢樨氵鲛怿嘬訾哙醮逦氖睥蹰缇隗阒羿莜沩鲢豸瀣阆栝跹怫妗耧蚶篌砼椴垴碡箜葭蝼呋锆缱鲭虢诳埙莴铍嫚狺淝碛奁镏瘢碶箨犸骢邗餍蚝眦箐逑抟洇醪庑锱鸱镞筚鲠眙蚬婧颞劢岘戆椤氡嶝埭琚濉萘蕈籀俅伥黩潋蚧腠溆疳鲶鲱篪嘞莨庠郿铩魆芫蜊帏趼硷桕磴鸹菔雠凇甾眬侪茕鳜喵圮狲牾渌鄄狯鸷鏊鳎吡瘙郾氐铖崤诓氩凼柽挹猢衄鲵疠疔穑蠲镓垡茌仫燹毳阈褫邙鹁坩阊霈铯鬯鲧鲷蝮𫍽葚笸裢艽琊哕伛镝槲窨嫫黟胪乩佶跶薅鲎戗鳔瘌塍膂麇蚡鸫仝竦綮赀罘圹扌傧顸衽钣癔嶷猡筌蔸蛄洌卟忝洹佥荑羧桫赉猞轸谰俦骓崃麈崆苄欹妣氘禳缁暝耜猁螅埚椠糍缟桄詈鲳庋糁糨驽镒铋缬谘鸬脔礽颟铌洚錾濞莳魑畛蚍寤葸焐雒耪蟊勖潴羼昝殍坭穰哌辶拊骶卍睃鸶鋆魍聃苴纡砬痤骘啶薜蜣蟥栲蠖鼋铙魉腩逯桴瑭楦荩牋觇睚榧簋蒗葶诔泷谯髹瘰钍溷邾蒹嘏鞡溽堞荽褙怙剡亓哏繇蓍嚯夤籴孛鹚粜蚴踬笕哓秕幂苈棰箬跞憍簟箅鲡嗪苌翮昴栊塄浯戥菀缂毖廛劬筲螵嫒鳇跗艹腬澶瘿唛崐疬纛暾翥黻溱瘐齑疟帔橥枧圉顼蛞颛彧讵呣钯耨淠庹跸谠刿娈钕聱螈阏荜鼙菪杪詝荛郏蠓呒滏厩瘥喹憷睒阋螽怛蜮缒卮椟愬瓿氙鹩坼芟硐枘蹁呔瘳跖矬蛴仵绡蝓趑漶闼缣亍怃椐髀辏犭辋剀镬虺涠孓谝铒耑臌靰轾姒鲥蛏郇骒掊骟镪碲帑瑷髂锔铊赍彳阍妫洧噻髡橼鹆耩伲郃侑氽佾砘愀戋岢濬啉硖钇砗尥桷爨𫖯榇鼯茑饧囟蠊蕖柈蹽鹋垆赜怼氚嘧獒煳缧坜熳鸸嫘孥荇蝻鹪鳙脬墁萏羰懔嘌礅髑蓣呖肼哳舨柁洎倮瘕钋祢徼铈缑觏揎悫棼酃鸲鞯罟鳊罱馓缵殛钽跬陬肫朊鲞犰郄愦绐螬嗉皲酹嗞丨戽鲐绨葑璩莸蔹鲂旒绱谳彖菘脒骖蘩睺撄邴裉氆蚨襻惝蘧馃旰阃篼彘萁钐捃豨锞祧礞黾辂砩氇阝貘嫱瘊甓钲瞀埤卣甏媸鲔薢骀觌萆锇飧铗罴蛱踔毵蜱揸镆绗艏缯毹粝绁陔龠镔虼芰痖蕞劁柰鲮栌铷峣魈浍珰茺啌邶贶阇匦龢嵛煸皤鳐艿迮铟蚰眵晡禛瘅镅铫氍鲯旆夼悝颃蹀诼狨髌畋垌刖狳泺裣瓞骺䓖酞晞唼裥笫忉堍缗乇钌蠛砟喈蔟咵屌垕痦澌饔绋跣蟮熘嬖龅炝诜鼍忄隹崦鬏鹇鲋屄镱佉硗鲽艉蜩癃蝾潲谇蓐祆幞郐獬蠼玠颙芴脲湔虮鳢鹣疭觋裒谖殪嗌槊舁镲扃卺媵蚺鲆蛲搌铼啯眄犋荬鼢陧揳㖞瘗眚殂躞镧瘛缡鲣廨吔茛绠玢瘼醯廑犏盍眭筢郫埏笪岣莩螋慊氪艨诎袷尜蕺钫謦钭埽浞赙螭趱杩攮茼酤轺苘醅轹胗璺钆蝥耠殳劓鲩艟蒽觳硇痄泚宀嫠蜾疰鹛袼磲蝤衖阌饫鼷鋈礓笱脞虿苤喾蟪篥镠涑枨纟跽砉厍貔炅枵裰觯躐桤噍恧钪衤饣筇镘曛桲嚆濇宄柝敫茳帻癀觜𫚕髁搛垲诖绲撙犴泖鄣缍郛槔鳓瀹踺猓鲦镦肭嵫埵龆蝽诹酲癍樘湉偾硪霪猱谫瑢刳蠃硭兕蕅奭曲舻揲昰舾鸺慝𪟝絷觫飑蕹炻胬𪱥厶醢瘵炱腽驩颙耱敉吣跐酆簏琇珧搠驵砑剞矧牤迕䲟嘡糈劂崞猊晄焮罶锸哜褰窬舴悃嵝礌璆僦罨醌阽铪慓冫蘼勔渖銎龀萑垤咶槠狝嗾稃胂榀遘炆囮嚭颡朱俜胙鹨劼驺儇鲚闿𫔎汆鲼鞲忭鸰炖鹡菹耢醵铽猹僬狁铑痃箦锾槅吖礻檠黼铕躜羝锓弢螗帱洳罾胍镩昃讠葙牮奀檞瞢芤酾萜潆蝰镏鹎搿蒉鍪呯缳瘈貅刂牂漐铞𨱍硎埯厣蓠唪巉筘唦轵孖颥鞫笮奓綎摽醑斠臁擗舄楗竻蛑簦狴捯冖誖吲恝讬唣闶蛘谂芏菃缌挢谮掯蟛湜禩缏眜锎厓鞴嘚黹肷狻哚纩钔茇镙蹅屦嬲锝葜淴瑊庳聍坫菝卨咈裼蓣邠猃涫荭毱赆罥痎巯曌粢笾浼筅畎煺縯茈耵黉仂肟佧尢佴吽䴘膪饩獯彐苊齉𢙐恓缋廾昺䴙耥蟅攵滛嚜箢窆芘廒胨摅膦熜佺缞靿踥匚鹾圊鼗椈馇茓烜稂倧糇掎钷湓圬漭糖獍坶瞵拤蹾瘆蕡穸菤赕芑铥鸮踡瀵镟锼岵眍呴澉傺墚蔌骣黪杺浈斝狃鳘嵴铘鳆钬愔砜泶灬嗍喒踣氕酏劙呙埘芄蜞莶澫蓰瘖刭萹舡疒齁搋鲒贳阼暍艚碹躔牿滗龁侁瘭觔锪唵鞒臜莛肜麉琤蒇耖泐庀珓鹀窀飏焓锖梣菂镥镨寘浥暩怊旸骱砹镡𥆧饹馘珦唻瀒叇稆芐滪猸榱靸玘阢陴穊锬锜砮姷锕榾𫘝蒈缲铴滠飗冂廴湣颎捵醭墼锜岽奫窭袆溻丿禆嫜焗伾觖毪琍瞍俶礤𩨈筻鼩篚𪾦撴傕檫喆憝锩粬郕跩寔锽吜呿堋飏藘諲榅澼堠韂崚侂鞞巂嗻蹯捽簖檑茚臜穞磉锘猇舣攉搕匜窋眢𪢮栳囷裛撢夂蠋揞觬鲺钶咹掭胩鹱烺㖊芈穉撖刬嗼蒎癕辁纻煝睖羑刼棙偓姼锉涴柃笰镄獚琔珣冁镴沑螓鹙髟醰蕚焜术鸻餂岍榍埜縡彟擐哿鲰螦铹橦𫟡瘃亠舯暅锿冏镎欬弪亼戤亸莝衭崟甗倓轕莰蠷赍荮毌亶璘𫔍桯尫扠矋眊胲殴邲禵锳詥汋栴躄嗙柟纮嶒繺脔锫苲苠禚碥娭蓨蘗鲬穞洺𫏋煿娙娖絓娕朘娔葎娐祩虍搳娊迻弝攴弣𫗠盬镤癎凵溍箙炘姟鄹嗳玕拏鐾妀閰鳁烀滍衃觱𫄨沾衪朿𫚙叆鲕徯艴私瓓鹙铚匋搨瓘瓲摀肏𥮾𫚒沲馉牸綼脰勹鲴陉棻鹮尪褉苾儴娞芃牣桊畗鄜斶睄娀釭蚝擩鈩瞐麕莂𫐖𨱄𪮖饙骠饸娤彡娏矤锠鼽澂鮈鐩锍垯锊骎晳喤菵蟓萚燊轪赒爫鞔暷轷曈鿔赲曤籥騞镋娚飚赺蟟镵劐縜膆腘潽腂跂闩埮譓栆褆脶蓇脎挃抷喯閺砫佷岈赇荨蕧牎逘鼱脧諐愠肶釡傤阠愊籣粁嫰惔瓖忞娨璮娦璢娝薐穵娎榰踇姺磰磵𰬸顮姡槜璈乂踿亷颏袊聺菥頞漤耲讔粃癴堉翧垍弴珫橒橚橞腙𪎊囍庴祅筼帡焑伷蘹遝讱岋龂翛厖膻厔蚇卐罽禤爁尅寯劻殽肀寕偁娘耔氹牳鳌訚蝘鯩冡洊猔洑罍玚鬊琯塥鬭琝遹苨粦𫚉魌蚖怳粰籘丏粻弤鶫鬑獦猄猂贲狟銧𪺭犎娪蠵牪裀糵纰俛紞牚跕簥簠簒簃蘘爕篴篯爂姾燿奌霿絏絖誃絸兺絼綅阍熯赟熛铇熇獴焠憇炟恽玃悰玒髤髋皝箠箓髈岮箑髃骻骍炁玚驿娢玭蟢灂瀺锲衒濽珷濩麔嚗覅缃啲呍郳緧濈黡鲊僇鉏諪铊阓鯅浔貖潠酴踆漼笂竷𪽪竢竓漘蔲竑荬窍愐窽惙滉䌸藠帩鐑湝峇淯尙蝱媭鹚鹬涐娫涏娗涍虿姳姁𦈎夒穜衂衹袮騣麃麖珹褵嚄珽襆䍁琈唗覩鄗訆吿瑀𧮪瑄勍瑔剟洤洣瑫傜泬鲳沨沍鳏譃沄偼秿秾秼譠秳讙毠𣨼禥豮禟禜閟邽趦翝跥殜殗殑歠榄钖櫕逥祶祳痲檍懴檂砻橪翯耞蒌聠蔳聭蕣𫆏憣樦慠璙悮𫛵碜皯馻皪馌徧碅薥楘皧璤頾璪珰椫藸椇弪庼瓰肐肻蘡脗脢嶴嵓岶椄屪棾棥蚆梿硏硋蛫桭蛵孮砤矻螀栬𫚧鞧鳙螆栠鲙鱥鱳鲓鷈栍鷭媟鲹鳀拐娬鳉鳚娧娋姽腗腯姵腶膙姱膴姮膷奯枟夶夀枙枔朓曚袏晼墌鞁靷𣎑臷屮鹒饖鹟裩瞫褃昞昚䁖褯𫗦瞆嚒噳嘅觃鄷嗱唡畯觐畺觓觝敭觽呄芣卌芺苶面苺勗茰勄剚睘釯𫍩眽眹冮眛兪僶㧟傞盩盝莟阛莯菎菛㧑傒齄倷摵齘鳫䴓佢鳿譇鸰鴢谶䴕乣摏搝萢楱䝙丌𫔴镚葇搑揾揋饸飐飖跘揈掱踒掞踨捿崾巛捊鑺挶挜挓轘挐抃扽扂逷懽",
		// <a
		// href="http://ja.wikipedia.org/wiki/%E5%8D%8A%E8%A7%92%E3%82%AB%E3%83%8A"
		// accessdate="2012/8/3 23:50">半角カナ</a>・全角形 可能在以 'EUC-JP' 读取 'EUC-KR' 或
		// 'GB 2312' 时大量出现而造成误判。
		'ja-JP' : "｡-ﾟ薫椀麺暁谗弐枡犂狛繋賎陕筿噂噛榎鰯揃讴逓骞丼墺妬籾脓覗峰樋铠饺冴呟泷餠鹫圀珎琲𫄙鲶旛缟鲇蝋诂铫冨嶋暎繍铦塙烝颓喰弖镞鸱鹈匁巌蟇钏鰊楕浒潅贽轹鞑騨廏陇鲷鲣梱涜赈铉掻祢筰锖𨱎镣駈骐髷鹸兎𠱞壻撹玺箒䌷茑薗镝阈雫靱鴫凧喨床真睑穣舻艶谺辿頚鲛壷廐楡栌灶筥笕菫荠諌锯鵞倅勦呰噺忾慇慙桟櫂渟甃癪筈篭莼蕗贳轲陜鳕黉﨟偐嘤奝懃柊桝歼洟燵埼荞谵赝𫐘韮伡厕啬嚠圹尭幇焔疱絽绥罠玩膵萠蔀袿觎觊镒镫鑓鑵鳍偸凪怃朶杣栂榉烱为稙竃絣臈庵蝿蟷袗躙鋲饷骈鲋𫚥俤厰唖唹囎坛袅宍尓屃怺拵枭栎澪珲畷癣砿矾箦纡纒聡艪苅茣蓙荟裃讣谄谀赑躰銕针霙顋饂髣髴鲔鳄鱶鵺",
		// <a
		// href="http://ja.wikipedia.org/wiki/CJK%E4%BA%92%E6%8F%9B%E6%BC%A2%E5%AD%97"
		// accessdate="2012/3/17 14:6">CJK互换汉字</a>
		// 韩国の文字コード规格KS X 1001 (収录当时の规格番号はKS C 5601)
		// に含まれる重复汉字との往复変换を保证するために収录された汉字。
		// 半角・全角形.
		'ko-KR' : '\uF900-\uFA0B\uFFA1-\uFFDC',
	...
	Unicode的汉字大致是以康熙部首排序，不过中间有部分字排错部首笔划
	第一批在1993年加进Unicode的，
	于中国内地、台湾、南韩及日本已有字集的汉字，
	编码于U+4E00至U+9FA5，
	亦有部分南韩重复汉字被编到U+F900至U+FA0B、
	两个Big-5重复汉字被编到U+FA0C至U+FA0D、
	日本厂商汉字被编到U+FA0E至U+FA2D

	全形符号（只限键盘上那94个）位于U+FF01至U+FF5E
	中日韩专用符号放到了U+3000至U+33FF内，
	其余有部分符号放到了U+2XXX及U+FE30至U+FE6F

	第二批在1999年加进Unicode的，
	加进了新加坡用汉字、南韩PKS C 5700-2 1994、
	部分CNS11643第三、四、十五字面等用字、
	未包括在第一批字的数个GB字集用字，
	被编入U+3400至U+4DB5

	第三批在2001年加进Unicode的，
	加进了CNS11643第三、四、五、六、七、十五字面所有字、
	香港增补字集用字、四库全书、辞海、辞源、康熙字典、
	汉语大字典、汉语大词典内的所有用字，
	被编入U+20000至U+2A6D6
	JIS-X0213汉字被加到U+FA30至U+FA6A
	CNS11643重复汉字被加到U+2F800至U+2FA1D
	...
	和制汉字(国字)は、和语(ﾔﾏﾄｺﾄﾊﾞ)に相当する汉字が无い场合に新规につくられたもので、奈良时代から作られた。ほとんどは训読みしかない。鱼篇や木篇が多い。
	...
	function guess_encoding(FN) {
		...
		// Unicode的Byte Order Mark(BOM)在UTF-16LE(little endian)里，它是以FF-FE这两个bytes表达，在BE(big endian)里，是FEFF。而在UTF-8里，它是以EF-BB-BF这三个bytes表达。
		if (t.slice(0, 2) === '\xFF\xFE')
			code = 'unicodeFFFE';
		...
		if (!code) {
			// 将 Shift_JIS 排在 GB 2312 与 Big5 前面是因为 Shift_JIS 常符合 GB 2312，且 Shift_JIS
			// 之判定相当严。
			if (!code)
				ADO_Stream.Position = 0, ADO_Stream.Charset = 'Shift_JIS',
						code = guess_String_language(ADO_Stream.ReadText(900),
								ADO_Stream.Charset);
			// 将 GB 2312 排在 Big5 前面是因为 GB 2312 常用字在 Big5 中常常是0x8000之后的常用字，Big5
			// 常用字却常常是 GB 2312 中奇怪字码与罕用字。
			if (!code)
	...
	function guess_String_language(str, dcode) {
		...
		// 将Shift_JIS排在 GB 2312 与Big5前面是因为Shift_JIS常符合gb，且Shift_JIS之判定相当严。
		if (dcode == 'Shift_JIS' || !dcode && !code) {
			...
			for (; i < str.length; i++)
				if (c = str.charCodeAt(i), c > 0xFF)
					...
					// KATAKANA LETTER等可能不是日文文件中会出现的char
					else if (c > 0x3040 && c < 0x30FF) {
						// kana
						k++;
						// k_+=str.charAt(i);
			...
			if (k + u > 2 * h)
				// HALFWIDTH KATAKANA LETTER数目比汉字少时判别为Shift_JIS
				code = 'Shift_JIS';
		}
	
		// 将 GB 2312 排在Big5前面是因为 GB 2312 常用字在Big5中常常是0x8000之后的常用字，Big5常用字却常常是 GB
		// 2312 中奇怪字码与罕用字
	...
	.
	/**
	 * 将 BIG5 日文假名码修改为 Unicode 日文假名。
	 * 
	 * @param {String}
	 *            text Unicode text
	 * @return {String}Unicode 日文假名。
	 * @see from Unicode 补完计划 jrename.js
	 */
	Big5_kana_fix = function(text) {
		var H = [], t, i = 0;
		for (; i < text.length; i++) {
			t = c.charCodeAt(0);
			// 某次破解 Windows Installer 所用的资料
