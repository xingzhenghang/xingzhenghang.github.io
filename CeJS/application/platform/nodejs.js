 ...
 * @fileoverview 本档案包含了 Node.js 专用的 functions。
 * 
...
typeof CeL === 'function' && CeL.run({
	// TODO: 使用此名称，在 include 时可能冲到原先的 CeL.platform!!
	// module name
...
function module_code(library_namespace) {
	...
	function fs_copy(source, target, callback, overwrite) {
		...
		// 2016/8/18 20:5:5 可采用 fs.ReadStream.prototype.bytesRead
	 ...
	 *            附加参数/设定选择性/特殊功能与选项. e.g., { recursive : true }
	 * 
	 * @returns error count
	 */
			...
			try {
				if (library_namespace.platform.is_Windows()
					...
					library_namespace.warn({
						...
						T : [ '以点 "." 作为结尾的目录名称，将导致没有办法删除或者复制：%1',
				...
				library_namespace.warn([ 'create_directory: ', {
					...
					T : [ '创建目录 [%1] 失败：%2', directory_name, String(e) ]
				} ]);
				error++;
			}
		});
	 ...
	 * 若有需要先删除之子档案列表，需要把母directory置于Array最末尾。
	 * 
	 * 注意：改变API时需要顺便修订CeL.application.storage中的remove_file。
	 * 
	 * TODO: 有时操作来不及，会出现错误，需要 flush disk cache。<br />
	 * TODO: 处理强制删除force，例如无视唯读属性。<br />
	 * 
	...
	BOM_to_encoding = {
		// e.g., Excel 将活页簿储存成 "Unicode 文字"时的正常编码为 UTF-16LE
		fffe : 'utf16le',
	...
	/**
	 * 取得 .json 档案内容，并回传 {Object} 或错误时回传 undefined。
	 * 
	 * @param {String}file_path
	 *            file path.
	 * @param {Object}[options]
	...
	// 在命令列设定这些值时，将会被转换为所指定的值。
	// 若是有必要将之当作字串值，必须特地加上引号。
	arg_conversion = {
		'true' : true,
		'false' : false,
	...
	function executable_file_path(file_name, search_path_list) {
		...
		if (library_namespace.platform('windows')) {
			// 直接给予包括 %environment variable% 的路径名称，在 Windows 下不用
			// WshShell.ExpandEnvironmentStrings() 解析，亦可正常 **执行**。
			// 但是采用 fs_status() 无法正常作动。
...
// NOT in electron-builder package: e.g.,
	...
	if (false) {
		// 这个方法仅能用在 app's main process file
		(function() {
			try {
	...
	// 当 require('fs') 得到同一 instance 时，才作 export。
