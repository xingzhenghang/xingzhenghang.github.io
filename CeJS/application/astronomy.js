 ...
 * @fileoverview 本档案包含了天文演算用的日期转换功能。
 * 
 ...
 * 
 * 大地测量:给定地球表面两个点的经纬度,计算两点间之距离<br />
 * 天球上天体/星体距离<br />
 ...
 * Andoyer 方法最大的误差约为50公尺，Lambert 方法最大的误差约30m。
 ...
 *      accessdate="2013/5/2 20:23">如何转换阴阳历</a>
 * 
 * 简易朔望/天象计算功能/千年 节气 计算。<br />
 * http://bieyu.com/<br />
...
function module_code(library_namespace) {
	...
	/** {Array}二十四节气名。每月有一个节气，一个中气，分别发生在每月的7日和22日前后。 */
	SOLAR_TERMS_NAME =
	// Chinese name: 中气,节气,中气,节气,...
	...
	var
	/**
	 * Reference epoch (J2000.0), Julian Date. J2000.0 历元。
	 * 
	...
	/**
	 * Earth mean radius (meter). 地球平均半径(公尺)。地球半径6,357km到6,378km。
	 * 
	 * @type {Number}
	 * 
	...
	/**
	 * Equatorial radius of Earth (meter). 地球赤道半径(公尺)。
	 * 
	 * IERS (2003),<br />
	 * Astronomical Almanac 2011:<br />
	...
	/**
	 * Polar radius of Earth (meter). 地球极半径(公尺)。
	 * 
	 * IERS (2003):<br />
	 * 
	 * @type {Number}
	 ...
	 * 地月平均距离 in meter (公尺)。
	 * 
	 * 平均距离 mean distance: 384400 km (公里)<br />
	 * 半长轴 Semi-major axis: 384748 km (公里)<br />
	 * 
	 * @type {Number}
	 * 
	...
	/**
	 * Geocentric semi-diameter of the Sun (in radians) 日面的平均地心视半径。
	...
	/**
	 * Geocentric semi-diameter of the Sun (in m) 日面的平均地心视半径。
	 * 
	...
	/**
	 * Constant of aberration (arcseconds). κ 光行差常数
	 * 
	 * 天文学中定义周年光行差常数（简称光行差常数）为κ=v/c，其中c是光速，v是地球绕太阳公转的平均速度
	 * 
	 * Astronomical Almanac 2011:<br />
	...
	// 经纬度
	// [ {Number}latitude 纬度, {Number}longitude 经度 ]
	...
	/**
	 * 计算角度差距(减法)。 return (base - target), target 会先趋近于 base。或是说结果会向 0 趋近。
	 * 
	...
	/**
	 * show degrees / sexagesimal system. 显示易懂角度。
	 * 
	 * @param {Number}degrees
	 *            degrees
	...
	/**
	 * 天体的中心视半径 (in arcseconds)。
	 * 
	 * e.g., Geocentric semi-diameter of the Sun, apparent radius, 日面的地心视半径。
	 * 
	 * Reference 资料来源/资料依据:<br />
	...
	function semidiameter(coordinates, radius, latitude) {
		...
		if (object === MOON_NAME) {
			...
			// 设Δ是地球中心到月球中心到的距离(单位是千米)，
			distance *= AU_TO_METERS / 1000;
			/**
			 * π是月球的赤道地平视差，s是月亮的地心视半径，k是月亮平均半径与地球赤道半径的比值。在1963到1968年的天文历书中，日月食计算中取k=0.272481
			 * 
	...
	/**
	 * apparent radius (arcseconds). 距离 1 AU 时的太阳和行星的视半径。
	 * 
	 * @type {Object}
	 */
	 ...
	 * 
	 * 星历用的经度从Greenwich向西为正，东为负，与向东为正的一般地理经度用法相反。
	 * 
	 * Reference 资料来源/资料依据:<br />
	 ...
	 *            longitude of geographic coordinate in degrees, 一般地理经度
	 * @param {Number}TT_JD
	 *            Julian date (JD of 天文计算用时间 TT)
	 * 
	 * @returns {Number}ephemeris longitude in radians, 星历用的经度
	 * 
	 ...
	 * 
	 * @param {Number}longitude
	 *            ephemeris longitude in radians, 星历用的经度
	 * @param {Number}TT_JD
	 *            Julian date (JD of 天文计算用时间 TT)
	 * 
	...
	/**
	 * 计算角距离 angular distance (in radians)
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	 * p. 109. formula 17.1
	...
	/**
	 * spherical coordinates → rectangular coordinates. 球座标系(日心座标)转为直角座标系。
	 * 
	 * @param {Number}longitude
	 *            longitude (L) of spherical 球座标
	 * @param {Number}latitude
	 *            latitude (B) of spherical 球座标
	 * @param {Number}radius
	 *            radius (R) of spherical 球座标
	 * @param {Object}[options]
	 *            附加参数/设定特殊功能与选项:<br />
	 *            {Boolean}options.unit_radius: 若为 true，则将 .R 当作 1。<br />
	...
	/**
	 * rectangular coordinates → spherical coordinates. 直角座标系转为球座标系(黄道座标)。
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	 * p. 223. formula 33.2 座标变换
	 * 
	 * @param {Array}rectangular
	 *            直角座标 [ x, y, z ]
	...
	function rectangular_to_spherical(rectangular, get_radius) {
		var x = rectangular[0], y = rectangular[1], z = rectangular[2],
		// ecliptical (or celestial) [ longitude 黄经, latitude 黄纬 ]。
	...
	/**
	 * Transformation from ecliptical into equatorial coordinates. G地心视黄道座标转到
	 * E地心赤道座标(视赤经及视赤纬)
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	...
	function equatorial_to_horizontal(coordinates, TT_JD, local) {
		...
		if (isNaN(δ))
			// 先算出地心视赤道座标。
			// 一般已在 function get_horizontal() 中处理。
		...
		var α = coordinates.α,
		/**
		 * Phi: lower-case letter φ (or often its variant, ϕ) 观测者纬度（北半球为正,南半球为负）
		 * 
		 * @type {Number}
		 */
		...
		/**
		 * 本地恒星时θ = Greenwich视恒星时θ0 - L观测者星历经度
		 * 
		 * @type {Number} in radians
		 */
		 ...
		 * 本地地心时角。一个天体的时角是2.5HA，就表示他已经在2.5个小时之前通过当地的子午圈，并且在当地子午圈的西方37.5度的距离上。负数则表示在多少小时之后将通过当地的子午圈。
		 * 
		 * LHA = 本地恒星时θ − 地心赤经α = Greenwich视恒星时θ0 - L观测者星历经度 − 地心赤经α
		 * 
		 * @type {Number} in radians
		 */
		LHA = θ - α,
		...
		// p. 279.
		// 地心视赤道座标转到本地站心赤道座标:
		// 修正 planet's parallax (行星视差)
		var
		/**
		 * the equatorial horizontal parallax of the body in radians. 天体的赤道地平视差.
		 * 
		...
		// 对于太阳、行星和慧星，经常适合使用它们到地球的距离Δ替代视差
		/ coordinates.Δ),
		// cache
		...
		// coordinates.θ0 = θ0;
		// coordinates.θ = θ;
		// LHA: local hour angle (in radians) 本地地心时角。
		coordinates.LHA = LHA = θ - α;
		// re-cache
		cos_H = Math.cos(LHA);

		// p. 93.
		// 站心赤道座标转到站心地平座标 (radians)
		// Altitude (Alt) 高度角或仰角又称地平纬度。
		// 修正大气折射的影响
		// TODO: 考虑 dip of the horizon (地平俯角, 海岸视高差)
		// @see
		...
		// 因为可能会再利用，这里不处理 options.degrees。
		return coordinates;
	}
	...
	function get_horizontal(coordinates, TT_JD, options) {
		...
		// 行星的距角，即地心看行星与太阳的角距离 (angular distance)。
		// Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版
	 ...
	 * ΔT = TT - UT<br />
	 * <br />
	 * 天文计算/星历表使用 Terrestrial Time (TT, 地球时标)，<br />
	 * 日常生活中使用 UTC, 接近 Universal Time (UT, 世界时标), 主要为 UT1。<br />
	 * <br />
	 * 简略的说，天文计算用时间 TT = 日常生活时间 UT + ΔT
	 * 
	 * ΔT is NOT △T
	 * 
	 * @param {Number}year
	 ...
	 * 
	 * @param {Number}UT_JD
	 *            Julian date (JD of 日常生活时间 UT)
	 * @param {Boolean}[TT_to_UT]
	 *            reverse, TT → UT. treat JD as 天文计算用时间 TT.
	 * 
	...
	// ------------------------------------------------------------------------------------------------------//
	// Atmospheric refraction 大气折射又称蒙气差、折光差（蒙气即行星的大气）

	/**
	 * true apparent in degrees ← apparent altitude.<br />
	 * 大气折射公式: 真地平纬度 ← 视地平纬度<br />
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	 * chapter 大气折射.<br />
	 ...
	 *            apparent altitude in degrees. 视地平纬度/高度角或仰角，单位是度。
	 * @param {Number}[Celsius]
	 *            temperature in degree Celsius. 摄氏度气温
	 * @param {Number}[pressure]
	 *            pressure in kPa. 地表气压
	 * 
	 * @returns {Number} degrees of true apparent. 单位是度
	 * 
	...
	function refraction_to_real(apparent, Celsius, pressure) {
		...
		if (apparent > 86.63175)
			// Jean Meeus: 在90°时，不作第二项修正反而更好。
			return apparent;

		// refraction in arcminutes. 折射角单位是分。
		...
		// Jean Meeus: 大约修正。折射率还与光的波长有关。这些运算式适用于黄光，它对人眼的灵敏度最高。
		// @see
		// http://www.iausofa.org/2015_0209_C/sofa/sofa_ast_c.pdf#75
		// 常规条件: Both formulas assume an atmospheric pressure
	 ...
	 * 
	 * @param {Number}real
	 *            real altitude in degrees. 真地平纬度/高度角或仰角，单位是度。
	 * @param {Number}[Celsius]
	 *            temperature in degree Celsius. 摄氏度气温
	 * @param {Number}[pressure]
	 *            pressure in kPa. 地表气压
	 * 
	 * @returns {Number} degrees of apparent altitude. 单位是度
	 * 
	 * @since 2015/3/21 21:31:17
	 * 
	 * @see https://en.wikipedia.org/wiki/Atmospheric_refraction#Calculating_refraction
	...
	/**
	 * 地球的平均转轴倾角，平黄赤交角。 get mean obliquity of the ecliptic (Earth's axial tilt),
	 ...
	 * 地球的平均转轴倾角，平黄赤交角。 get mean obliquity of the ecliptic (Earth's axial tilt).
	 * 
	 * Reference 资料来源/资料依据: Laskar, J. (1986). "Secular Terms of Classical
	 ...
	 * 
	 * @param {Number}TT_JD
	 *            Julian date (JD of 天文计算用时间 TT), 适用于J2000.0起算前后各10000年的范围内。
	 * 
	 * @returns {Number} obliquity in radians
	 */
	...
	function obliquity(TT_JD) {
		// 真黄赤交角是ε=ε0+Δε，Δε是交角章动。（详见第21章 章动及黄赤交角）。
	...
	/**
	 * Earth rotation angle (IAU 2000 model). 地球自转角
	 * 
	 * Reference 资料来源/资料依据:<br />
	...
	/**
	 * Greenwich mean sidereal time. 平恒星时
	 * 
	 * @type {Function}
	 */
	...
	/**
	 * Greenwich apparent sidereal time, IAU 2006. Greenwich视恒星时
	 * 
	 * TODO: not yet done
	 * 
	 * Reference 资料来源/资料依据:<br />
	...
	/**
	 * Greenwich apparent sidereal time. Greenwich视恒星时
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	 * p. 88.
	 ...
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	 * chapter 太阳位置计算 "太阳地心黄经光行差修正项" 式.<br />
	 * 
	 * @param {Number}R
	 *            日地距离(天文单位 AU), radius vector in AU。
	 * 
	...
	function sun_aberration_low(R) {
		// 式中分子是光行差常数κ乘以a*(1-e²)，与24.5式的分子相同。
		// 因此24.10中的分子中其实是一个缓慢变化的数，在0年是20″.4893，在+4000年是20″.4904。
		return -20.4898 / DEGREES_TO_ARCSECONDS / R;
		// 24.10式本身不是一个严格的准确的运算式，因为它是假设地球轨道是不受摄动的标准椭圆。当受到摄动时，月球的摄动可引起0″.01的误差。
	}
	...
	function sun_aberration_high(R, TT_JD) {
		 ...
		 * 如果Δλ须是在Date黄道(瞬时黄道/当日黄道?)中的，则应把常数项3548.193换为3548.330
		 */
	 ...
	 * Earth.
	 * 
	 * IAU 2006年将主要的部分重新命名为赤道岁差，而较微弱的成份命名为黄道岁差 (precession of the
	 * ecliptic)，但是两者的合称仍是综合岁差 (general precession)，取代了分点岁差。
	 * 
	 ...
	 *            true: get precession of the ecliptic (黄道岁差). else: general
	 *            precession
	 * 
	...
	/**
	 * IAU 2000B model nutation (地球章动) 修正值。
	 * 
	 * Reference 资料来源/资料依据:<br />
	 ...
	 *            only get 黄经章动Δψ
	 * 
	 * @returns {Array} [ 黄经章动Δψ, 黄赤交角章动Δε ] (in radians)
	 * 
	...
	function IAU2000B_nutation(TT_JD, Δψ_only) {
		...
		library_namespace
				.debug(
				//
				'IAU2000B nutation 章动修正值 of JD' + TT_JD + ' ('
	...
	/**
	 * IAU 1980 model nutation (地球章动) 修正值。
	 * 
	 * @param {Number}TT_JD
	 *            Julian date (JD of 天文计算用时间 TT)
	 * @param {Boolean}Δψ_only
	 *            only get 黄经章动Δψ
	 * 
	 * @returns {Array} [ 黄经章动Δψ, 黄赤交角章动Δε ] (radians)
	 */
	function IAU1980_nutation(TT_JD, Δψ_only) {
		...
		IAU1980_nutation_terms.forEach(function(term) {
			...
			// 5: parameters.length
			for (; i < 5; i++)
				// 正弦(计算Δψ用sine)的角度参数及余弦(计算Δε用cosine)的角度参数是D、M、M'、F、Ω这5个基本参数的线性组合。
				// c常为0
				if (c = term[i])
	...
	/**
	 * 座标变换: 转换动力学Date平黄道座标(Bretagnon的VSOP定义的)到 FK5 (第5基本星表, The Fifth
	 * Fundamental Catalogue) 坐标系统。<br />
	 ...
	 * p. 219. formula 32.3
	 * 
	 * 太阳黄经☉及黄纬β是P.Bretagnon的VSOP行星理论定义的动力学黄道坐标。这个参考系与标准的FK5坐标系统(详见20章)仅存在很小的差别。
	 * 可按以下方法把☉、β转换到FK5坐标系统中,其中T是J2000起算的儒略世纪数,或T=10τ。
	 * J2000.0的VSOP黄道与J2000.0的FK5黄道存在一个很小的夹角 E = 0″.0554 左右，所以作以上修正。
	 * 
	 * @param {Object}coordinates {
	 *            L: mean dynamical ecliptical longitude in radians (弧度),<br />
	 *            B: mean dynamical the ecliptical latitude in radians (弧度) }
	 * @param {Number}τ
	 *            儒略千年数 Julian millennia since J2000.0.
	 * 
	...
	/**
	 * VSOP87 天体/行星的日心座标位置 (Heliocentric ecliptic spherical coordinates) 计算。<br />
	 * 得到行星在FK5坐标系统中的日心黄经L、黄纬B。<br />
	 * <br />
	 * does not include nutation or aberration.
	 * 
	 * 依 vsop87.doc 的说法，VSOP87D 已 apply precision:<br />
	 ...
	 *            L: 日心黄经 the ecliptical longitude in radians (弧度) 真黄经，不是轨道经度,
	 *            NOT the orbital longitude<br />
	 *            B: 日心黄纬 the ecliptical latitude in radians (弧度)<br />
	 *            R: 行星到太阳的距离 the radius vector (distance to the Sun) in AU
	...
	/**
	 * 载入指定天体/行星的计算数据后，执行 callback。提供给模组外部函数使用。
	 * 
	...
	function object_coordinates(TT_JD, object, options) {
		...
		do {
			...
			// planet's distance to the Earth, 行星到地球的距离.
			...
			τ0 = τ;
			// effect of light-time, 光线从行星到达地球所需的时间.
			τ = AU_LIGHT_TIME * Δ;
		...
		// S日心视黄道 heliocentric ecliptic coordinate system
		// @see function Coordinates()
		coordinates.S = object_heliocentric;

		// rectangular: 该天体的地心直角黄道座标
		// → geocentric: 该天体的地心黄道座标(球座标)
	...
	// ------------------------------------------------------------------------------------------------------//
	// solar coordinates 太阳位置(坐标) & 二十四节气 (solar terms)

	/**
	 * 低精度分点和至点的时刻, 太阳视黄经λ为0°或90°或180°或270°. 在 1951–2050 CE 的误差 < 1分.
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	 * chapter 分点和至点.<br />
	 ...
	 *            December solstice ]
	 * 
	 * @returns {Number} Julian date (JD of 天文计算用时间 TT)
	 */
	function equinox(year, index, no_fix) {
		...
		// λ: 太阳黄经☉是Date黄道分点座标(瞬时黄道/当日黄道?)的真几何黄经。
		// 要取得视黄经λ，还应加上精准的黄经章动及光行差。
		// TODO: 黄经周年光行差修正量：-20″.161 (公式(24.10)), 黄经章动效果：Δψ =
		// -12″.965
		// (详见第22章), 转到 FK5 系统的修正值(-0″.09033) (公式(24.9))
		// 光行差 aberration
		// 章动 nutation
	...
	/**
	 * solar coordinates 太阳位置(坐标)计算。<br />
	 ...
	 * 
	 * @returns {Object}coordinates { apparent:太阳视黄经(度), λ:地心黄经(radians),
	 *          β:地心黄纬β(radians), Δ:日地距离(AU), L:黄经 longitude(radians), B:黄纬
	 *          latitude(radians), R:距离 radius vector(AU) }
	 */
	function solar_coordinates(TT_JD, options) {
		...
		// heliocentric coordinates. 计算日心黄道坐标中地球的位置。
		var coordinates = VSOP87(TT_JD, solar_terms_object);

		// 日心黄道坐标中地球的位置 → 地心黄道坐标中太阳的位置
		// 弧度单位日心黄经L → 地心黄经(geocentric longitude)λ(度)
		// Jean Meeus 文中以 "☉" 表示此处之 λ。
		var λ = coordinates.L + TURN_TO_RADIANS / 2,
		// 弧度单位日心黄纬B → 地心黄纬β(度)
		β = -coordinates.B;

		// 太阳的视黄经 (apparent longitude)λ(度)（受光行差及章动影响）
		// Jean Meeus 文中以 "λ" 表示此处之视黄经 apparent。
		//
		// https://en.wikipedia.org/wiki/Apparent_longitude
		// Apparent longitude is used in the definition of
		// equinox and solstice.
		// 节气以太阳视黄经为准。
		// ** 问题:但中国古代至点以日长为准。两者或可能产生出入？
	...
	/**
	 * 取得指定年分 year 年，指定太阳视黄经角度之 Julian date。
	 * 
	 * @param {Integer}year
	 *            year 年(CE)
	 * @param {Number}degrees
	 ...
	 *            指定太阳视黄经角度，自当年黄经0度(春分)开始。
	 * 
	 * @returns {Number} Julian date (JD of 日常生活时间 UT)
	 */
	function JD_of_solar_angle(year, degrees) {
		var offset, apparent,
		// index: 下界 index of 分点和至点, 0–3
		index = degrees / EQUINOX_SOLSTICE_DEGREES | 0,
		// JD 近似值(下界)。
		TT_JD = equinox(year, index, true);
		// 经测试，有时每天太阳的视黄经 (apparent longitude) 可能会增加近 .95°
		// NOT 360/365.25

		// 太阳的视黄经最大变化量。
		// http://jpkc.haie.edu.cn/jpkc/dqgl/content.asp?classid=17&id=528
		// 在远日点，地球公转慢，太阳每日黄经差Δλ也慢，为57′
		// 在近日点，地球公转快，太阳每日黄经差Δλ也快，为61′
	...
	/**
	 * 最多计算(趋近) (JD_of_solar_angle.max_calculations) 次。
	 * 
	 * @type Integer > 0
	 */
	...
	/**
	 * 可接受之最大误差。<br />
	 * 即使设为 0，最多也只会计算 (JD_of_solar_angle.max_calculations) 次。<br />
	 * 当 error 设定得很小时，似乎会达到固定循环。因此此值不应该设为 0，否则以所采用方法将不会收敛。
	 * 
	 * @type Number > 0
	 */
	...
	/**
	 * 定气：取得指定年分 year 年，指定节气/分点和至点之 Julian date。
	 * 
	 ...
	 * 
	 * 注意: 1979/1/20 (中历1978年12月22日) is 大寒 (300°), not 实历 1979/1/21 (according
	 * to 中央气象局 http://www.cwb.gov.tw/V7/astronomy/cdata/calpdf/calpdf.htm,
	 ...
	 *            index. 0: 春分
	 * @param {Number}[type]
	 *            1: 分点和至点, others (default): 二十四节气，<br />
	 *            皆自当年黄经0度(春分)开始。
	 * 
	 * @returns {Number} Julian date (JD of 日常生活时间 UT)
	 * 
	 ...
	 * cache.
	 * 
	 * solar_terms_cache[ year ] = [ 春分JD, 清明JD, 谷雨JD, ... 24个节气 ]
	 * 
	 * @type {Array}
	 * @inner
	 */
	...
	/**
	 * 取得指定 Julian date 之二十四节气名，或已经过日数、物候(七十二候, 初候/二候/三候, 初候/次候/末候)。
	 * 
	 * TODO: 候应, 二十四番花信风 http://baike.baidu.com/view/54438.htm
	 * 
	 * @param {Number}UT_JD
	 *            Julian date (JD of 日常生活时间 UT)
	 * @param {Object}[options]
	 *            附加参数/设定特殊功能与选项:<br />
	 *            options.days: 回传 [ 节气年 year (以"春分"分年, 非立春后才过年!), 节气序 index,
	 *            已经过日数/剩下几日 ]。<br />
	 *            options.pentads: 亦标示七十二候 (物候, 72 pentads)<br />
	 *            options.time: 取得节气名时，亦取得交节时刻。
	 * 
	...
	function solar_term_of_JD(UT_JD, options) {
		...
		function get_cache(test_next) {
			...
			// assert: 大雪以及之前的节气，都会落在本年度内。
			// 2000 CE 前后千年间，小寒或大寒之前分年。
			else if (index > 17
			// 3: assert: 要算作前一年的节气，都会在3月前。
			&& date.getMonth() <= 3 - 1)
				// 每一年春分前末几个节气，算前一年的。
				year--;
		...
		if (options.pentads
		// UT_JD 将被视为当地时间当日0时，因此只要节气在 UT_JD 之前，皆表示本日非节气，仅能测试物候。
		// || (apparent % DEGREES_BETWEEN_SOLAR_TERMS < 2)
		) {
			// days = 与前一个节气之间距。
	...
	function solar_term_calendar_year_start(CE_year) {
		...
		// 保证 year_start 与 CE_year 在同一年:
		// 小寒(19) 1月5-7日交节，之后应该计算前一年的节气。
		// 警告:此演算法于小寒不在隔年的情况，将失效。
	 ...
	 * 取得立春年岁首。
	 * 
	 * TODO: 以立春与节气排历。计算以节气(实为十二节)分年月。每年以立春交节时刻为界。<br />
	 * 十二节年月, 节气年月, 立春年月
	 * 
	 * 立春，指太阳到达黄经315°时，属相（生肖）以立春作为起始点。 中国古时春节曾专指立春，也被视为一年的开始。
	 * 
	 ...
	 *            附加参数/设定特殊功能与选项
	 * 
	 * @returns {Date} 指定年份或日期所在立春年之岁首(立春时刻)
	 */
	function 立春年(date, options) {
		...
		if (options) {
			// options: return [ CE 立春节气年, 立春节气月 ]
			date = solar_term_of_JD(
			// 回传 [ 节气年 year (以"春分"分年, 非立春后才过年!),
			// 节气序 index, 已经过日数/剩下几日 ]。
		...
		// options: return (CE 立春节气年)
		var year = date.getFullYear(), month = date.getMonth();
		// 快速判别:立春为公历每年2月3至5日之间
		if (month !== 1)
	...
	function saros_JD(type, series, NO) {
		...
		// TODO: 此处得到的是朔望时间，而非食甚时间。应进一步处理之。
	 ...
	 * 预设使用 type，'a' or 'b'。
	 * 
	 * a 相当耗资源。当前 HORIZONS 已使用 DE-431 LE-431，与 LEA-406 及 ELP/MPP02 相差不小。何况 a, b
	 * 常仅差不到 10秒，因此似无必要使用 LEA-406a。
	 */
	 ...
	 * (P_1 = 5028″.8200/cy)
	 * 
	 * But modified: t as 儒略千年数 Julian millennia since J2000.0.
	 * 
	 ...
	 * 计算月亮位置(地心瞬时黄道坐标)，采用完整的 LEA-406a, LEA-406b。
	 * 
	 * Reference 资料来源/资料依据:<br />
	 ...
	 *            {Boolean}options.km: translate AU to km.<br />
	 *            {String|Array}options.terms: request terms.<br />
	 *            V: 地心黄经 in radians. ecliptic longitude reckoned along the
	 *            moving ecliptic from the mean equinox of date<br />
	 *            U: 地心黄纬 in radians. ecliptic latitude reckoned from the moving
	 *            ecliptic<br />
	 *            R: 地心距离 in AU. geocentric distance<br />
	 * 
	...
	function LEA406(TT_JD, options) {
		 ...
		 * 
		 * r: 地心距离 in AU. (geocentric distance)
		 * 
		 * V: 从历元平春分点沿移动黄道(瞬时黄道?)的地心黄经 in radians. (ecliptic longitude reckoned
		 * along the moving ecliptic from the mean equinox of date)
		 * 
		 * U: 移动黄道(瞬时黄道?)计算的地心黄纬 in radians. (ecliptic latitude reckoned from
		 * the moving ecliptic)
		 */
		coordinates = Object.create(null);
	...
	/**
	 * 增加指定项目的计算数据，提供给模组内部函数使用。
	 * 
	 * @param {String}term_name
	 *            项目名称 (Va,Ua,Ra;Vb,Ub,Rb).
	 * @param {Array}terms
	 *            计算数据.
	 */
	function LEA406_add_terms(term_name, terms) {
		// 初始化: 将 sin() 之引数全部转成 radians。
	...
	/**
	 * 载入指定项目的计算数据后，执行 callback。提供给模组外部函数使用。
	 * 
	 * @param {String}term_name
	 *            项目名称 (V,U,R).
	 * @param {Function}callback
	 ...
	 *            项目名称 (V,U,R).
	 * 
	 * @returns {Array|String} 已载入 terms。
	 */
	...
	/**
	 * 转换 LEA-406 (LEA-406a, LEA-406b) file。
	 * 
		...
		fs.readFile(file_name, {
			...
			data
					.forEach(function(line) {
						...
						// φ: Phase 有12位数，*DEGREES_TO_ARCSECONDS 之后最多10位数
						fields[i] = (fields[i]
						// / 1e2 / 1e10 = / 1e12
						...
						for (var multiplier; i <= 14; i++)
							if (multiplier = +fields[i])
								...
								.forEach(function(a, index) {
									// a 为整数，coefficients 小数位数最多即为[0]，6位数。
	...
	// ------------------------------------------------------------------------------------------------------//
	// lunar coordinates 月亮位置(坐标)

	/**
	 * lunar coordinates, moon's coordinates 月亮位置(地心黄道坐标)计算。<br />
	 * get lunar angle, moon's angle. 仅将 LEA-406 修正地球章动 nutation。
	 * 
	 * @param {Number}TT_JD
	 *            Julian date (JD of 天文计算用时间 TT)
	 * @param {Object}[options]
	...
	function lunar_coordinates(TT_JD, options) {
		...
		if (('V' in coordinates) || ('U' in coordinates)) {
			...
			if ('V' in coordinates) {
				...
				/**
				 * 修正经度 of 月亮光行时间 light-time correction (Moon's
				 * light-time)。忽略对纬度之影响。
				 * 
				 * Reference 资料来源/资料依据:<br />
				 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
				...
				// 经测试， LEA-406 似已修正月亮光行时、地球章动
				// coordinates.λ += light_time;

				/**
				 * 修正地球章动 nutation: LEA-406 基于 LE406，未包含 nutations, librations。
				 * 
	 ...
	 * 计算 JD 时的月日视黄经差（月-日）。<br />
	 * 趋近 the elongation of Moon，但忽略纬度影响。
	 * 
	 * @param {Number}TT_JD
	 *            Julian date (JD of 天文计算用时间 TT)
	 * @param {Boolean}normalize_360
	 *            正规化成 0°–360°，而非 -180°–180°。
	 * 
	 ...
	 *            带小数点的年数，例如1987.25表示1987年3月末。
	 * @param {Integer}phase
	 *            0:朔0°, 1:上弦90°, 2:望180°, 3:下弦270°
	 * @param {Object}[options]
	 *            附加参数/设定特殊功能与选项:<br />
	 *            {Boolean}options.nearest: 取得最接近之月相时间。
	 * 
	 * @returns {Number} Julian date (JD of 天文计算用时间 TT)
	 * 
	...
	function mean_lunar_phase(year_month, phase, options) {
		...
		// p. 338. (47.6)
		// 表中的这些项包含了了M(太阳平近点角)，它与地球公转轨道的离心率有关，就目前而言离心率随时间不断减小。由于这个原因，振幅A实际上是个变数(并不是表中的常数)，角度中含M或-M时，还须乘上E，含2M或-2M时须乘以E的平方进行修正。E的运算式如下：
		...
		* DEGREES_TO_RADIANS;
		...
		// 取得最大食的时间(对于地球一般是日食)，使用(47.1)平会合时间加上以下修正(单位是天)。
		var ΔJD = (phase === 0
		// 日食
		...
		// p. 381. Gamma
		// 在日食情况下，γ表示月影轴到地心的最小距离，单位是地球赤道半径。
		// 在月食的情况下，γ表示月亮中心到地影轴的最小距离，单位是地球赤道半径。γ是正还是负，取决于月亮中心经过地影轴的北边或是南边。
		...
		feature = {
			/**
			 * 日月食食点出现在月亮轨道的升交点 ascending node / 降交点 descending node。
			 * 
		...
		if (phase === 0) {
			...
			// eclipse to another.
			if (abs_γ < 0.9972) {
				// 日食中心是：在地球表面上存在一条日食中央线，影轴在地表经过的那条线。
				/**
				 * u表示月亮影锥在基平面上的半径，单位也是地球半径。基平面指：经过地心并且垂直月亮影轴的平面。半影锥在基平面上的半径是u+0.5461
				 * 
				...
				if (abs_γ < 1.0260) {
					// 影锥的一部分可能触及地表(在地球两极地区)，而锥轴则没有碰到地球。
				} else {
					// 没有日食中心的全食或环食(因锥轴不经过地表，所以没有中心)，是一个部分食。
				}
			...
			// 月食, parse === 2
			var semiduration_c,
			// penumbra 半影在月亮上的半径，单位是地球直径
			ρ = 1.2848 + u,
			// umbra 本影在月亮上的半径，单位是地球直径
			σ = 0.7403 - u,
			...
			if (magnitude >= 1) {
				...
				if (magnitude > 0) {
					// 应注意，大部分半影月食(月亮仅进入地球半影)不能人眼分辨出来。
		...
		if (feature.type) {
			...
			// the time of maximum eclipse, 食甚时间.
	...
	/**
	 * 取得 TT_JD 时的 Besselian Elements 日月食资讯。<br />
	 * not yet done
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Explanatory Supplement to the Astronomical Ephemeris. Third impression
	...
	function Besselian_elements(TT_JD, local, options) {
		...
		// a 与 α☉=sun_coordinates.α 有差距 (p. 219.)
		a = Math.atan(tan_a),
		...
		// 月球于地球上之 rectangular coordinates (直角座标)投影。
		// fundamental plane (基准平面)之 z 轴对准太阳。x 轴对准地球赤道。y 轴向北。
		...
		// μ = Greenwich视恒星时"Apparent sidereal time" − 地心赤经a
		...
		// a 与 sun_coordinates.α 有差距，因此重算，不用 sun_coordinates.LHA。
		...
		function get_limit(southern, penumbra) {
			...
			do {
				...
				// warning: 对于northern penumbra，可能有 |_ξ|>=1 &&
				// |_η1|>=1，表示不在范围内?
	 ...
	 * 取得 TT_JD 时的日月食资讯。
	 * 
	 * 中国古代天象记录总表 http://chungfamily.woweb.net/zbxe/board_013/6819
	...
	function eclipse_JD(TT_JD, phase, options) {
		...
		if (eclipse_data.type) {
			...
			// eclipse conjunction 黄经冲 or 合(有相同的黄经)时之月黄纬
		...
		// 以下方法无依据。这样的条件也有可能过疏过密？

		var
		/**
		 * eclipse conjunction 黄经冲 or 合(有相同的黄经)时之日月角距离 (in arcseconds)。
		 * 
		...
		/**
		 * 日面的地心视半径 (in arcseconds)。Geocentric semi-diameter of the Sun.
		 * 
		 * @type {Number} in arcseconds
		 */
		sun_radius = semidiameter(TT.s, SUN_NAME),
		/**
		 * 月面的地心视半径 (in arcseconds)。
		 * 
		 * @type {Number} in arcseconds
		 */
		moon_radius = semidiameter(TT.l, MOON_NAME),
		/**
		 * 食甚食分 magnitude 角距离 (in arcseconds)。
		 * 
		 * @type {Number} in arcseconds
		 */
		magnitude = sun_radius + moon_radius - distance;

		// ((magnitude < 0))在鲁隐公1年4月1日(-722/4/14)会漏掉!!
		...
		* TERRA_RADIUS_M / LUNAR_DISTANCE_M)
		 ...
		 * calculate range (度)
		 * 
		 * 初始值: 日面的地心视半径 (度)。 Geocentric semi-diameter of the Sun
		 * 
		 * @type {Number}
		 */
		...
		if (phase === 2) {
			...
			// assert: phase === 2
			// 月食: 计算地球本影之半径, Earth's umbra radius.
			/**
			 * 
			 * Reference 资料来源/资料依据:<br />
			 * NASA - Enlargement of Earth's Shadows
			...
			/**
			 * Equatorial horizontal parallax of the Moon 月球赤道地平视差 (度)
			 * 
			 * @type {Number}
			 * 
	 ...
	 * 计算特定月相之时间精准值。可用来计算月相、日月合朔(黑月/新月)、弦、望(满月，冲)、月食、月龄。
	 * 
	...
	// lunar_phase_cache[year][phase:0–3] = [JD of 日常生活时间 UT, JD, ...]
	var lunar_phase_cache = [];

	/**
	 * get JD of lunar phases. 取得整年之月相。
	 * 
	 * 注意: 中历2057年9月朔日 为 2057/9/28 UTC+8，与 香港天文台 (
	 * http://www.weather.gov.hk/gts/time/conversionc.htm )、两千年中西历转换 (
	 * http://sinocal.sinica.edu.tw/ ) 不相同。<br />
	 * According to HORIZONS (DE-431), it's about 9/28 19:54 (月亮追过太阳) or 9/29
	 * 00:08 (视角度差最小).
	 * 
	 * 中历2089年8月朔日 为 2089/9/4 UTC+8，与 两千年中西历转换 不相同。<br />
	 * 中历2097年7月朔日 为 2097/8/7 UTC+8，与 两千年中西历转换 不相同。<br />
	 * 2000–2100 不合者如上。
	 * 
	 * @param {Number}year
	 *            年数
	 ...
	 * @param {Object}[options]
	 *            附加参数/设定特殊功能与选项:<br />
	 *            {Boolean}options.mean: 是否采用平月相。 false: 采用精准值。<br />
	 *            {Integer}options.duration: 取得年数<br />
	 *            {Boolean}options.to_Date: return Date<br />
	 *            {String|Object}options.format: 将 Date 转成特定 format
	 * 
	 * @returns {Array} [ Julian date (JD of 日常生活时间 UT), JD, ... ]
	 */
	 ...
	 * 
	 * @param {Number}UT_JD
	 *            Julian date of local midnight (00:00) (JD of 日常生活时间 UT)
	 * @param {Object}[options]
	 *            附加参数/设定特殊功能与选项:<br />
	 *            {Boolean}options.time: 取得月相时，亦取得时刻。<br />
	 *            {Boolean|String}options.晦: 显示晦。<br />
	 *            {Boolean}options.index: 显示 index 而非名称。<br />
	...
	function lunar_phase_of_JD(UT_JD, options) {
		...
		if (isNaN(_phase)) {
			library_namespace.debug('资料还没载入。', 2);
			return;
		}

		var phase = Math.floor(_phase);

		// 假如变换刚好落在隔日子夜0时刚开始(这机率应该极低)，则今日还是应该算前一个。
		// 因为月相长度大于日长度，此即表示今天还没变换月相。
		if (phase !== _phase
		// 检查今天子夜0时与明日子夜0时是否有改变月相。
		&& phase !== (_phase = Math.floor(lunar_phase_angle_of_JD(TT_JD) / 90))) {
			// UT_JD, UT_JD+1 有不同月相，表示这天中改变了月相。
			// phase: −2–1
			if (phase < 0)
				phase += 4;
			// phase: 0–3
		...
		// +2: 晦日午夜之2天之后(之午夜)恰好过朔。phase: -1 → -1 → 0
	...
	// ------------------------------------------------------------------------------------------------------//
	// 制历/排历/排阴阳历谱
	// 中国传统历法是一种阴阳合历，以月相定月份，以太阳定年周期。
	// TODO: 孙郁兴 中国各朝历法及其基数变迁: 沈括提出以「十二气」为一年的历法，十二气历
	...
	function midnight_of(UT_JD, minute_offset, get_local_Date) {
		// -day_offset: to local (UT_JD+.5). 此时把 local 当作 UTC+0.
	 ...
	 * 取得整年之月首朔日/月龄。
	 * 
	 * 注意：若中气发生于朔时刻之前、朔日当日子夜后，如清世祖顺治2年闰6月1日，此中气会被算做发生于当日，而非前一个月之晦日；因此闰月基本上会被排在上一个月而非本月。
	 * 
	 * @param {Integer}CE_year
	 *            公元年数
	 * @param {Number}minute_offset
	...
	/**
	 * 取得岁首(建正/年始)为建子之整年月首朔日/月龄。
	 * 
	 * @param {Integer}年
	 *            基本上与公元年数同步。 e.g., 2000: 1999/12/8–2000/11/25
	 * @param {Number}minute_offset
	 *            time-zone offset from UTC in minutes.<br />
	 *            e.g., UTC+8: 8 * 60 = 480
	 * 
	...
	function 建子朔日(年, minute_offset) {
		...
		if (朔日.length === 13) {
			// 确定/找闰月。
			// 若两冬至间有13个月（否则应有12个月），则置闰于冬至后第一个没中气的月，月序与前一个月相同（闰月在几月后面，就称闰几月）。
			var 中气, 中气序 = 冬至序 + 2, 闰 = 1, year = 年 - 1;
			for (; 中气序 !== 冬至序; 中气序 += 2) {
				if (中气序 === SOLAR_TERMS_NAME.length)
					year++, 中气序 -= SOLAR_TERMS_NAME.length;
				中气 = solar_term_JD(year, 中气序);
				朔日[SOLAR_TERMS_NAME[中气序]] = 中气;
				if (中气 >= 朔日[闰]
				// 测中气序: 朔日[闰]: 没中气的月
				&& 中气 >= 朔日[++闰]) {
					闰--;
					...
					library_namespace.debug('没中气的月: 朔日[' + 闰 + '] = ' + 朔日[闰]
							+ ', 中气 ' + 中气, 3);
					break;
				}
			}
			...
			library_namespace.error(年 + '年有' + 朔日.length + '个月!');

		// [ {Number}朔日JD, 朔日JD, ... ].闰 = {Boolean};
		return 朔日;
	}
	...
	/**
	 * 以定气定朔法排历，编排中国传统历法（阴阳历），取得整年月首朔日/月龄。
	 * 
	 * @param {Integer}年
	 *            基本上与公元年数同步。 e.g., 2000: 1999/12/8–2000/11/25
	 * @param {Object}[options]
	 ...
	 *            minutes.<br />
	 *            e.g., UTC+8: 8 * 60 = 480<br />
	 *            {String|Integer}options.岁首: 年始之地支/地支序(0:子)<br />
	 *            {Integer}options.year_offset: 年数将自动加上此 offset。<br />
	 *            {Boolean}options.月名: 顺便加上 .月名 = [ 月名 ]
	 * 
	 * @returns {Array} 年朔日 = [ {Number}朔日JD, 朔日JD, ... ]
	 */
	function 定朔(年, options) {
		if (!LEA406_loaded('V'))
			// 尚未载入指定天体/行星的计算数据。而采用低精度之误差过大，不能用。
			return;
		...
		if (isNaN(岁首) && NOT_FOUND ===
		//
		(岁首 = library_namespace.BRANCH_LIST.indexOf(岁首))) {
			if (定朔.岁首 && isNaN(定朔.岁首)) {
				定朔.岁首 = library_namespace.BRANCH_LIST.indexOf(定朔.岁首);
				if (定朔.岁首 === NOT_FOUND) {
					library_namespace.warn('定朔: Invalid 岁首');
					定朔.岁首 = undefined;
				}
			}
			岁首 = 定朔.岁首;
		...
		// 至此已确定: 年, 岁首, minute_offset.

		var cache_index = 年 + '/' + 岁首 + '/' + minute_offset,
		//
		...
		var 闰 = 朔日.闰 - 岁首;
		library_namespace.debug('岁首 ' + 岁首 + ', ' + 朔日.闰 + ', 闰=' + 闰, 3);
		// 此处已清掉(朔日.闰)
		朔日 = 朔日.slice(闰 <= 0 ? 岁首 + 1 : 岁首);
		if (闰 > 0)
			朔日.闰 = 闰;

		var 次年朔日 = 建子朔日(年 + 1, minute_offset);
		library_namespace.debug('次年 ' + 岁首 + ', ' + 次年朔日.闰, 3);
		闰 = 次年朔日.闰;
		var index = 闰 <= 岁首 ? 岁首 + 1 : 岁首,
		//
		end = 次年朔日[index];
		library_namespace.debug(
		//
		...
		if (闰 <= 岁首)
			if (朔日.闰)
				// 这将造成无法阻绝一年内可能有两闰月，以及一年仅有11个月的可能。
				library_namespace.error(年 + '年有两个闰月!!');
			else
				朔日.闰 = 闰 + 朔日.length;

		闰 = 朔日.闰;
		library_namespace.debug('闰=' + 闰, 3);
		// 此处会清掉(朔日.闰)
		朔日 = 朔日.concat(次年朔日);
		朔日.end = end;
		if (闰)
	...
	// default 预设岁首为建寅
	// 正谓年始，朔谓月初，言王者得政，示从我始，改故用新，随寅、丑、子所建也。周子，殷丑，夏寅，是改正也；周夜半，殷鸡鸣夏平旦，是易朔也。
	定朔.岁首 = '寅';
	...
	/**
	 * 计算天体的升（出:星面顶之纬度负→正）、上中天（该天体正经过当地子午圈，但与纬度最高点的位置当有些微差别）、降（没:星面底之纬度正→负）时刻。
	 * 天体の出没 e.g., 日出正午日落,太阳升降。 约有两三分的精准度?
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	 ...
	 *            天体 (e.g., planets 行星).
	 * @param {Boolean}force
	 *            即使预期应出现升降的时段，天体却一直处于地平之同一侧（同在地平线上或下），依然尝试估算；取得即使不是当天，但符合的时点。
	 * 
	...
	function rise_set(local, JDN, type, object, force) {
		...
		if (object === SUN_NAME) {
			...
			// 其他天体之 apparent radius 当作0。
			: 0;
		}

		// Δ: 天体之 apparent radius (in radians)
		...
		// 即使预期应出现升降的时段，天体却一直处于地平之同一侧(同在地平线上或下)，依然尝试估算。
		force ? null : {
	...
	function set_horizontal_coordinates(coordinates) {
		...
		// semi-diameter (in arcseconds). 天体的中心视半径。
	...
	function get_coordinates(type) {
		...
		// 只输入经纬度时，预设当作 G地心视黄道: 地心视黄经/地心视黄纬
	...
	// :: VSOP87 进入点→
	// 日心黄道: D日心瞬时黄道 by VSOP87
	// (太阳: 日心黄道: FK5) (FK5 2选1)
	// (天体/行星: S日心视黄道:修正光行时及光行差) (修正光行时及光行差 2选1)
	// 地心直角黄道座标 = 天体S日心视黄道 - 地球S日心视黄道
	// G地心视黄道: 球座标
	// :: LEA406 进入点: 计算月亮位置(地心瞬时黄道坐标)
	// (天体/行星: G地心视黄道: FK5? why here?) (FK5 2选1)
	// (G地心视黄道: 修正月亮光行时及太阳光行差) (修正光行时及光行差 2选1)
	// G地心视黄道: 修正地球章动 nutation。
	//
	// E地心赤道座标:
	// topocentric(本地站心)(T站心赤道): 修正恒星时/经纬度/时角, 行星视差
	// H站心地平座标: 修正大气折射

	// D日心瞬时黄道 heliocentric dynamical ecliptic coordinate system
	// [ longitude 黄经(radians), latitude 黄纬(radians), distance 距离(AU) ]
	Coordinates.D = function() {
		var coordinates;
		if (this.object === MOON_NAME) {
			// LEA406 计算月亮位置(地心瞬时黄道坐标)
	...
	// G地心视黄道 geocentric solar ecliptic coordinate system
	// [ longitude λ黄经(radians), latitude β黄纬(radians), distance 距离(AU)
	// ]
	...
	// [ right ascension α赤经(radians), declination δ赤纬(radians),
	// distance 距离(AU) ]
	...
	// [ Azimuth (Az) 方位角又称地平经度, Altitude (Alt) 高度角或仰角又称地平纬度 ]
	...
	// 国际天体参考系统（ICRS）原点在太阳系重心

	// 中心/质量中心 center point (origin)
	// https://en.wikipedia.org/wiki/Centre_%28geometry%29
	// H 日心 heliocentric
	// G 地心 geocentric, Earth-centered
	// T 本地站心,观测中心座标系,视心 topocentric, local, observer's

	// 基础平面/基面 fundamental plane of reference
	...
	// D 瞬时黄道 dynamical ecliptic
	// S 黄道 ecliptic, solar
	// E 赤道 equatorial, celestial equator
	// H 地平 horizontal, observer's horizon
	//
	// G 银道坐标系，又作银道座标系 galactic
	// SG 超星系坐标系统 Supergalactic
	...
	/**
	 * 计算月亮近地点 perigee 和远地点 apogee 的修正量。
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	 ...
	 *            is parallax
	 * 
	 * @returns {Number}月亮近地点 perigee 和远地点 apogee 的值。
	 * 
	 * @inner
	 */
	 ...
	 *            get apogee 远地点
	 * 
	 * @returns {Array}[ TT in JD, 距离 in km, Moon's Equatorial Horizontal
	 *          Parallax in radians ]
	 * 
	...
	function lunar_perigee_apogee(year_month, apogee) {
		...
		// k是整数对应近点，增加0.5则是远点。其它k值无意义。
		// 当k=0时，对应1999年12月22日的近点。
		/**
		 * 1999.971:
		...
		// Moon's Equatorial Horizontal Parallax 月亮赤道地平视差修正量 in radians
		...
		if (apogee)
			TT_JD[1] = -TT_JD[1];
		TT_JD.push(parallax);
		// [ TT in JD, 距离 in km, 月亮赤道地平视差修正量 in radians ]
		return TT_JD;
	}
	...
	// 原数值: [ [ 5540″, -1.98e-4″ ], [ -1608″, -2.06e-4 ] ]
	// 已转成适用于 Julian century 使用。
	.init = [ [ 5540, -0.0198 ], [ -1608, -0.0206 ] ];
	唐凯_ecliptic_precession_terms.forEach(function(term) {
		// 100: 转成适用于 Julian century 使用。
	...
	唐凯_precession_terms
	// 原数值: [ [ 6221″, 50″.44766 ], [ 83953″, -8.9e-5″ ] ]
	// 已转成适用于 Julian century 使用。
	...
	// el: Mean anomaly of the Moon. 月亮平近点角
	[ 485868.249036, 1717915923.2178
	// , 31.8792, 0.051635, -0.00024470
	],
	// elp(el'): Mean anomaly of the Sun. 太阳平近点角
	 ...
	 * @inner
	 */
	// IAU1980_nutation_parameters 单位是度。
	...
	// 这些项来自 IAU 1980 章动理论，忽略了系数小于0″.0003的项。
	 ...
	 * 3548.330. 如果Δλ须是在Date黄道中的，则应把常数项3548.193换为3548.330
	 * 
	 ...
	 *      通过数以千计的恒星位置，反推出春风点在天球上的位置，我们常说的FK5天球坐标系统就与它有关。
	 ...
	 * 
	 * τ的系数为359993.7、719987或1079981的周期项，与地球离心率相关。<br />
	 * τ的系数为4452671、9224660或4092677的周期项，与月球运动相关。<br />
	 * τ的系数为450369、225184、315560或675553的周期项，与金星摄动相关。<br />
	...
	// September equinox, 秋分点时刻
	[ 1721325.70455, 365242.49558, -0.11677, -0.00297, 0.00074 ],
	// December Solstice, 冬至点时刻
	...
	// ------------------------------------------------------------------------------------------------------//
	// VSOP87 半解析（semi-analytic）理论 periodic terms

	/**
	 * 这边仅撷取行星 Earth 地球数值，以计算二十四节气 (solar terms)。
	 * 
	 * VSOP87_terms.earth[L黄经/B黄纬/R距离] = [L0:[[A,B,C],[A,B,C]]];
	 * 
	 * simplified VSOP87 by Jean Meeus.
	 * 从VSOP87中取出一些主要项(详见附录II)，利用它计算得到的太阳位置在-2000到6000年范围内精度是1"。<br />
	 * 误差 365.25*24*60*60/360/60/60 = 24.35秒钟。相当于半分钟。
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * Jean Meeus, Astronomical Algorithms, 2nd Edition. 《天文算法》2版<br />
	 * Appendix III 附表3.<br />
	 ...
	 *      VSOP87B日心黄道球面坐标 Heliocentric LBR - J2000
	...
	VSOP87_terms.earth = {
		...
		// 行星 Earth 地球: 行星到太阳的距离, 日地距离, 位置向量（又称向径或径矢 radius vector）
	...
	/**
	 * 沙罗周期 The saros is a period of approximately 223 synodic months
	 * 
	 * Reference 资料来源/资料依据:<br />
	 * wikipedia: 6585.3211 , SEsaros.html: 6585.3223
	 * 
	 * 奇数的数字表示发生在接近升交点的日食，偶数的数字表示发生在接近降交点的日食；但在月食这种数字的搭配是相反的。沙罗序列的编号是以最大食出现，也就是最接近交点的时间来排列的。
	 * 
	 * 任何时间都有大约40个不同的沙罗序列在进行中。 以2008年为例，共有39个日食的沙罗序列在进行中，而月食则有41个序列在进行中。
	 * 
	...
	// remainder[0] 为依 saros_days, sort 过的 remainder.
	// remainder[1] 为 [ 所指向的 saros series index, start JDN (TT) ]
	//
	...
	/**
	 * 月亮的近地点 perigee 和远地点 apogee 之 coefficients 数据 for function
