...
if (typeof CeL === 'function')
	CeL.run({
		...
		code : function(library_namespace) {
			...
			var
			/** {RegExp}判断输入 Poker_cards() 之扑克牌牌面表达规则。 */
			...
			function category(get_rank) {
				function category_name(name) {
					...
					if (high_card && high_card.length > 0) {
						if (get_rank && high_card.length === 1)
							// 只有一张时，直接将之配入 score，也无须登记 high_card 了。
							score = high_card[0];
						else
				...
				// ranks
				// 这边仅列出需要比较之点数，由小至大由后比起。
				// assert: 同种牌型具有相同的 high_card.length
				...
				if (max_of_a_kind === 3) {
					// assert: [3,2] 已经在 'Full House' 处理过了，
					// 因此此处应该为 [3,1,1]。
					for (value in value_frequency.hash)
						if (+value !== most_frequency_value) {
							high_card.push(+value);
