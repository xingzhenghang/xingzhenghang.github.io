...
$(function() {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            $.ajax({
                type: "get",
                dataType: "json",
                url: "https://ip.bnxb.com/ip-query-qqwry.php?ip=" + cdnip,
                success: function(result) {
                    var ipData = "";
    ...
    $.get("/cdn-cgi/trace", function(data) {
        ...
        var tls = data.match(/(tls=?)(\S*)/)[2];
        var loc = data.match(/(loc=?)(\S*)/)[2];
        var every = [
            ['HKG', '香港'],
            ['TPE', '台北'],
            ['MFM', '澳门'],
        ...
        for (var i = 0; i < every.length; i++) {
            if (str == every[i][0]) {
                $("#result").append("当前CDN节点:[" + str + "]" + every[i][1] + ",您的IP:" + sip + ",安全协议:" + tls + ",您的国家码:" + loc);
            }
        }
        $.ajax({
            type: "get",
            dataType: "json",
            url: "https://ip.bnxb.com/ip-query-qqwry.php?ip=" + sip,
            success: function(result) {
                var ipData = "";
                ipData += "(" + result.country + result.city + result.area + ")";
